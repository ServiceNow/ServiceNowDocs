---
title: View a Customer Dispute Management case record
description: View the details of an Customer Dispute Management \(CDM\) case record in the CSM/FSM Configurable Workspace so that you can review the customer dispute information.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/australia/telecom-media-technology/view-adr-case.html
release: australia
topic_type: task
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Use Customer DIspute Management, Use, Customer Service Problem Management, Telecommunications, Media, and Technology \(TMT\)]
---

# View a Customer Dispute Management case record

View the details of an Customer Dispute Management \(CDM\) case record in the CSM/FSM Configurable Workspace so that you can review the customer dispute information.

## Before you begin

Role required: sn\_telco\_adr\_mgmt.viewer

## Procedure

1.  Navigate to **All** &gt; **Workspaces** &gt; **CSM/FSM Configurable Workspace**.

2.  Select the list icon \(\[Omitted image "Lists.png"\] Alt text: List Icon.\).

3.  Navigate to **Customer Dispute Management** &gt; **All**.

4.  Select the CDM case record that you want to see the details.


**Parent Topic:**[Using Customer Dispute Management case](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/australia/telecom-media-technology/use-alternative-dispute-resolution-case.md)

**Related topics**  


[Create a Customer Dispute Management case]()

[Investigate customer disputes]()

[Resolve a customer dispute and record the dispute analysis]()

[Close a customer dispute]()

[Customer Dispute Management](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/australia/telecom-media-technology/alternative-dispute-resolution.md)

