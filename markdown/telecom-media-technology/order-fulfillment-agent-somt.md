...
To modify the Order Fulfillment AI agent, [Duplicate an agentic workflow](https://www.servicenow.com/docs/access?context=clone-aia-usecase&version=yokohama&pubname=yokohama-intelligent-experiences&ft:locale=en-US), and adjust the settings according to your requirements.

The Order Fulfillment AI agent adds tasks apart from the generated tasks from GAF. To activate the GAF, see [Activate Group Action Framework for Now Assist for Sales CRM for Telecommunications](../task/activate-group-action-framework-somt.md). The GAF and AI search are both used for order tasks.
