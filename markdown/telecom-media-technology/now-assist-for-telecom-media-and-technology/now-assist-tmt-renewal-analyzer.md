...
**Note:** The Support renewals and expansion agentic workflow is available in read-only mode. Before using the workflow, you must make a copy and adjust the settings according to your requirements. See Duplicate an agentic workflow for details.
...
For more details on configuring the data sources, see Set up the Data Context Engine.
...
When a contract is due to expire in 90 days, the customer success manager receives a notification and the Support renewals and expansion workflow is automatically triggered. The customer success manager can monitor the progress in the Now Assist panel and select one of the following scans:
