...
-   Service catalog API: For synchronizing technical specifications with other network domain specific provisioning catalogs. See .
-   Service order API: Service Order API: Customer order is handled by another application and Sales CRM for Telecommunications acts as a service order manager or when the decomposition and orchestration is handled by the marketplace owner and sends order to Sales CRM for Telecommunications for fulfillment. See .
-   Technical service qualification API: Checks availability of resources and services before an order is submitted for fulfillment. See .
