---
title: Capture initiator details form
description: The Capture initiator details form enables you to collect the complaint requester details for an Alternative Dispute Resolution \(ADR\) case.
locale: en-US
release: australia
product: Customer Service Problem Management
classification: customer-service-problem-management
topic_type: reference
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Reference, Customer Service Problem Management, Telecommunications, Media, and Technology \(TMT\)]
---

# Capture initiator details form

The Capture initiator details form enables you to collect the complaint requester details for an Alternative Dispute Resolution \(ADR\) case.

<table id="table_wym_qgy_f3c"><thead><tr><th>

Field

</th><th>

Description

</th></tr></thead><tbody><tr><td>

Consumer

</td><td>

Consumer who submitted the complaint.

</td></tr><tr><td>

Contact

</td><td>

Contact of the business customer who submitted the complaint.

</td></tr><tr><td>

Account

</td><td>

Business customer who submitted the complaint.

</td></tr><tr><td>

Complaint submitted by

</td><td>

Type of complaint stakeholder. Select one of the following.-   **Regulatory**

A statutory authority that oversees compliance and investigate or act on complaints.

-   **Legal guardian**

A person legally authorized to act on behalf of another individual in raising or managing a complaint.

-   **Customer**

Consumer or business customer.

-   **Legal firm**

A law firm that represents a party in handling a complaint.


</td></tr><tr><td>

Initiator name

</td><td>

Name of the complaint stakeholder.**Note:** If **Customer** is selected in the **Complaint submitted by** field, the field label changes to **Name**.

</td></tr><tr><td>

Preferred contact method of initiator

</td><td>

Mode of communication with the complaint stake holder.**Note:** If **Customer** is selected in the **Complaint submitted by** field, the field label changes to **Preferred mode of communication**.

</td></tr><tr><td>

Initiator email id

</td><td>

Email id of the complaint stakeholder.**Note:** If **Customer** is selected in the **Complaint submitted by** field, the field label changes to **Email id**.

</td></tr><tr><td>

Initiator mobile number

</td><td>

Mobile number of the complaint stakeholder.**Note:** If **Customer** is selected in the **Complaint submitted by** field, the field label changes to **Mobile number**.

</td></tr><tr><td>

Initiator address

</td><td>

Address of the complaint stakeholder.**Note:** If **Customer** is selected in the **Complaint submitted by** field, the field label changes to **Address**.

</td></tr><tr><td>

Regulatory body name

</td><td>

Name of the third party regulatory body based on the location.

</td></tr><tr><td>

Intake channel

</td><td>

Channel through which the complaint progress must be communicated.

</td></tr><tr><td>

External reference number

</td><td>

Reference number related to your complaint.

</td></tr></tbody>
</table>**Related topics**  


[Create an Alternative Dispute Resolution case](../task/create-adr-case-record.md)

