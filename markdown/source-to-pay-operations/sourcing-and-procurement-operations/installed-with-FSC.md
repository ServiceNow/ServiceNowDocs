...
</table>**Note:** An application-specific admin role enables a user to access a specific application, but does not grant the user any other admin rights. Your system-level admin on the production instance must grant the application-specific admin role to the appropriate users, and then remove the application-specific admin role from all the users with the system-level admin role. For more information, see [Application administration](https://www.servicenow.com/docs/access?context=application-administration&version=xanadu&pubname=xanadu-application-development&ft:locale=en-US).
...
To modify the user limit, you can perform the following steps:
