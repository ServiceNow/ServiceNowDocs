...
5.  In the Add to sourcing event dialog box, select whether you would like to add the sourcing request to an existing or new sourcing event.
    ...
    -   If adding to an existing sourcing event, select the sourcing event from the **Sourcing events** list. The **Supplier response close** field is auto-populated with the supplier response close date specified in the existing sourcing event that you're adding the sourcing request to.
    -   If adding to a new sourcing event:
