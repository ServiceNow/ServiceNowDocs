...
To view the Pipeline management tab in the Source-to-Pay Worksapce, you must have the pipeline\_management\_admin role and have the following applications installed:
...
Sum of all the Cost avoidance on the Closed Completed pipeline projects assigned to the logged in user, where the savings end date is greater than or equal to the current year.
...
Total estimated savings \(estimated hard savings + estimated cost avoidance\) from planned pipeline projects assigned to the logged-in user, where the estimated end date falls within the current year.
