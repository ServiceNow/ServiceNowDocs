...
**Note:** The Properties page displays the properties for Craft only if you have installed the [Craft.co integration for Supplier Lifecycle Operations](../concept/slo-glossary.md#) \(com.snc.sn\_supplier\_craft\) plugin. Click the question mark icon \(![Question mark icon.](../image/question-mark-icon.png)\) beside each property field to see the property name corresponding to that field.
