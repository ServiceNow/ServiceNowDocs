...
-   Configure the \[sn\_supplier.slm\_email\] system property and edit the Value field to contain supplier email address. For more information, see [Configure properties for Supplier Lifecycle Operations](../reference/config-prop-supp-mgmt.md).
...
-   Activate the Trigger Intent to Action inbound action. For more information, see [Enable intent to action workflow from inbound actions](https://www.servicenow.com/docs/access?context=enable-intent-to-action&version=yokohama&pubname=yokohama-platform-administration&ft:locale=en-US).
