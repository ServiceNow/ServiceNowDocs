---
title: Roles for ServiceNow Otto for Finance and Procurement
description: Grant fulfiller, requester, and supplier user roles to access generative AI skills for ServiceNow Otto for Finance and Procurement.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/source-to-pay-operations/now-assist-for-fsc-common/now-assist-common-roles.html
release: zurich
product: Now Assist for FSC Common
classification: now-assist-for-fsc-common
topic_type: reference
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Explore, ServiceNow Otto for Finance and Procurement, ServiceNow Otto applications for Finance and Supply Chain, Finance and Supply Chain]
---

# Roles for ServiceNow Otto for Finance and Procurement

Grant fulfiller, requester, and supplier user roles to access generative AI skills for ServiceNow Otto for Finance and Procurement.

## Roles installed

|Role title \[name\]|Description|Contains roles|
|-------------------|-----------|--------------|
|ServiceNow Otto for Finance and Procurement|
|sn\_fsc\_genai.now\_assist\_fulfiller|Fulfiller role to access generative AI skills for FSC Common|now\_assist\_panel\_user|
|sn\_fsc\_genai.now\_assist\_requester|Requester role to access generative AI skills for FSC Common| |
|sn\_fsc\_genai.now\_assist\_supplier|Supplier role to access generative AI skills for FSC Common.| |
|ServiceNow Otto for Accounts Payable Operations \(APO\)|
|sn\_ap\_gen\_ai.now\_assist\_fulfiller|Fulfiller role to access generative AI skills for Accounts Payable Operations.|sn\_fsc\_genai.now\_assist\_fulfiller|
|sn\_ap\_gen\_ai.now\_assist\_supplier|Supplier role to access generative AI skills for Accounts Payable Operations.|sn\_fsc\_genai.now\_assist\_supplier|
|ServiceNow Otto for Sourcing and Procurement Operations \(SPO\)|
|sn\_spend\_gen\_ai.now\_assist\_fulfiller|Fulfiller role to access generative AI skills for Sourcing and Procurement Operations.|sn\_fsc\_genai.now\_assist\_fulfiller|
|sn\_spend\_gen\_ai.now\_assist\_requester|Requester role to access generative AI skills for Sourcing and Procurement Operations.|sn\_fsc\_genai.now\_assist\_requester|
|ServiceNow Otto for Supplier Lifecycle Operations \(SLO\)|
|sn\_supplier\_gen\_ai.now\_assist\_fulfiller|Fulfiller role to access generative AI skills for Supplier Lifecycle Operations.|sn\_fsc\_genai.now\_assist\_fulfiller|
|sn\_supplier\_gen\_ai.now\_assist\_supplier|Fulfiller role to access generative AI skills for Supplier Collaboration Portal|sn\_fsc\_genai.now\_assist\_supplier|

**Parent Topic:**[Explore ServiceNow Otto for Finance and Procurement](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/source-to-pay-operations/now-assist-for-fsc-common/now-assist-fsc-exploring.md)

**Related topics**  


[Supporting information for ServiceNow Otto for Finance and Procurement]()

[ServiceNow Otto for Sourcing and Procurement Operations \(SPO\)](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/source-to-pay-operations/sourcing-and-procurement-operations/now-assist-spo.md)

[ServiceNow Otto for Supplier Lifecycle Operations \(SLO\)](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/source-to-pay-operations/now-assist-slo.md)

[ServiceNow Otto for Accounts Payable Operations \(APO\)](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/source-to-pay-operations/accounts-payable-operations/now-assist-apo.md)

