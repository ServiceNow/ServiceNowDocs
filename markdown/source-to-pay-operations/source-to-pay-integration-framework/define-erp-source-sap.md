---
title: Define ERP source configuration for SAP ECC and SAP S4 HANA
description: ERP source configuration determines the ERP source to which your ERP system connects. Map the integration payload with the SAP ECC and SAP S4 HANA tables.
locale: en-US
release: xanadu
...
# Define ERP source configuration for SAP ECC and SAP S4 HANA
...
By default, the base system includes one ERP source named SAP ECC and SAP S4 HANA, which refers to the parent connection for SAP ECC and SAP S4 HANA. If there are multiple SAP ECC and SAP S4 HANA instances, multiple ERP sources must be configured, and the child connections for these SAP ECC and SAP S4 HANA instances should be referenced.
...
2.  In the ERP Configurations view, select one of the following ERP source configuration based on your requirement:
...
5.  To create a new ERP source, perform the following steps:
    ...
    2.  On the form, fill the fields.
        ...
        ![image.sap-integration-new-erp-source]

    3.  Select **Submit**.

        A new ERP source is created. Verify that the connection alias is also unique for each ERP source.

        **Note:** SAP ECC and SAP S4 HANA integration can have multiple ERP sources. The Staging table displays the ERP source column, which you can use to distinguish which ERP the data belongs to.


## What to do next

By default, the SAP ECC and SAP S4 HANA base system provides 14 integration services. For other SAP ECC and SAP S4 HANA instances, you need to perform the following:

-   To modify each integration service, you can open the integration service and select **here** to. You can define element level mapping between SAP ECC and SAP S4 HANA table fields and payload elements.
