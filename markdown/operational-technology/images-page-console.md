---
title: Images page
description: The Images page contains the images related to your Discovery Console for OT assets.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/operational-technology/images-page-console.html
release: zurich
topic_type: concept
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Using the Discovery Console, Discovery Console for OT, Operational Technology Native Discovery components, Operational Technology Discovery, Operational Technology]
---

# Images page

The Images page contains the images related to your Discovery Console for OT assets.

On the Images page, you can view, add, and manage photos and screenshots captured during the Discovery process related to your assets on the Console.

**Note:** When you import any assets, image data aren't imported.

The Images page allows you to search for an image. The page has the following tabs:

-   **Gallery** tab: Displays images.
-   **Auto-Assignment** tab:

\[Omitted image "image.images-page"\] Alt text: Images page

## Gallery tab

The Gallery tab shows different views of the images:

-   All: View all images.
-   Screenshots: View only screenshot images
-   Photos: View only photo images
-   No Grouping
-   Grouping

On the Gallery tab, select the **Actions** button and select from the drop-down menu.

\[Omitted image "image-page-actions.png"\] Alt text: Image Gallery Actions menu

Select the add icon \[Omitted image "add-icon-msi.jpg"\] Alt text: to browse and upload images. You can also just drag and drop the image on the window.

## Auto-assignment tab

Under the **Auto-Assignment** tab, you can select the add icon \[Omitted image "add-icon-msi.jpg"\] Alt text: and open the **Edit Auto-Assignment Profile**.

To add an image, select the add icon \[Omitted image "add-icon-msi.jpg"\] Alt text:.

To delete an image, select the delete icon \[Omitted image "delete-image-msi-console.png"\] Alt text:.

**Note:** To capture screenshots during the Operational Technology Discovery process, the Full Web Extractor Auto Query needs to be selected and executed. This captures the screenshots over ports 443 and 80 and extracts the content from the web pages.

