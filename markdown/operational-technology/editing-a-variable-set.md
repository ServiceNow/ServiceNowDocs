---
title: Edit a Variable set
description: Edit a Variable set for an Auto Query scan created in the Discovery Console for OT.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/operational-technology/editing-a-variable-set.html
release: zurich
topic_type: task
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Variables page, Using the Discovery Console, Discovery Console for OT, Operational Technology Native Discovery components, Operational Technology Discovery, Operational Technology]
---

# Edit a Variable set

Edit a Variable set for an Auto Query scan created in the Discovery Console for OT.

## Before you begin

Role required: admin

## Procedure

1.  Navigate to the Variables page.

2.  Select a Variable set.

    **Note:** Editing the Custom - Global variable set allows you to globally override variable values defined in the Driver - Global variable set.

3.  Select the pencil icon \[Omitted image "edit-icon-ot-console.png"\] Alt text: for the Variable.

    The Edit Variable Set view opens.

4.  Edit parameters as needed.

    1.  **Identification**: Edit the name and description of the Variable.

    2.  **Sites**: Select a different site from the original selection if needed.

5.  In the Variables section, you can edit or delete the variables as needed.

    Select the pencil icon to edit the Variable. The Edit Variable window opens.

    \[Omitted image "image.edit-variable-window-2"\] Alt text: Edit variable window

6.  To hide the value of a Variable, slide the toggle next to Encrypted.

    **Note:** Encrypted Variable values in Variable Sets are obscured. Encrypted values display as dots when set to encrypted.

    \[Omitted image "image.encrypted-passwords"\] Alt text: Encrypted values


