...
|Task|Purpose|
|----|-------|
|[Run the OT Network Map Cache Handler scheduled job](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/operational-technology/industrial-workspace/run-ot-network-map-cache-handler-scheduled-job.md)|Configure this scheduled job to run after your Service Graph Connectors to populate the latest data to show in Node Map.|
|[Set the system property for the OT Network Map](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/operational-technology/industrial-workspace/set-the-system-property-ot-network-map.md)|Set the **Max CI Nodes per Site \(sn\_ot\_core.node\_map.max\_ci\_nodes\_per\_site\)** system property to determine the maximum number of CI nodes shown per site on the network map.|
