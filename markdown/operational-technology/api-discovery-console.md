...
description: On the Settings page, the select the API tab to view and use the active tokens, denied tokens, and endpoints needed to communicate with the Service Graph Connector \(SGC\) for ServiceNow OT Discovery.
locale: en-US
...
On the **Settings** page, the select the API tab to view and use the active tokens, denied tokens, and endpoints needed to communicate with the Service Graph Connector \(SGC\) for ServiceNow OT Discovery.

## API settings

The API settings are used to set the active tokens, denied tokens, and endpoints needed to communicate with the SGC for ServiceNow Discovery for Operational Technology \(OT\).
...
-   Sends status of the user license.
-   Imports all software programs that are identified on the devices and returning a paginated list from the SoftwaresAPI view.
...
-   Transfers images and screen shots from the Console to ServiceNow instance. This returns images in a paginated list with these filters:
    -   `UpdateOn`
    -   `CreatedOn`
    -   `ScreenShot`
    -   `Size`
...
For security reasons, a computer/system may not be directly connected to the internet or may be on an unsecured LAN. In such a situation, the Discovery for Operational Technology components are not able to push files or data to the MID Server and from there via the Service Graph Connector on to the ServiceNow instance.
...
For example, if you navigate to **Settings &gt; Exports** on the Discovery Console for OT, you can download data generated by the Discovery components as a JSON export file. You can schedule the frequency of these downloads setting the number of days and time of day. These files are saved to the `/opt/MSi/srv/apiexports` directory. From there you can move them to a physical media like a thumb drive. You can then copy or import the JSON files to the MID Server and on to the SGC and then to the ServiceNow instance.
