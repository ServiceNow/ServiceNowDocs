...
1.  Fill the following columns in the Microsoft Excel spreadsheet:
    ...
    **Note:** Column names cannot be changed. Extra columns can be added to the staging table. For more information about adding a new custom field mapping in the staging table, see [Add a custom field mapping in the staging table for Service Graph](add-new-column-mappings-excel-service-graph.md).
...
Users must not fill this column.
 ...
 For system properties, see [System properties that impact SG-OT Device Excel Import processing](system-properties-sg-ot-asset-excel-import.md).
...
-   For a listing and explanation of valid CI types, see [Operation Technology \(OT\) extension classes](https://www.servicenow.com/docs/access?context=cmdb-ci-class-models-operation-technology&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US).
