...
Be sure that the MID server user can create files to the sys\_attachment table and that the table can accept files of the “bundle” type.
 ...
 Linking or an application through a MID Server enables access to repositories behind a firewall. See [MID Server](https://www.servicenow.com/docs/access?context=mid-server-landing&version=zurich&pubname=zurich-servicenow-platform&ft:locale=en-US) for more information.
