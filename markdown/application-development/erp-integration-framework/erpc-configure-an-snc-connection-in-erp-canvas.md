...
-   Administrative access to the SAP system and the server where it is hosted.
...
2.  Search for and open the **ERP Canvas RFC** alias with the ID sn\_erp\_integration.ERP\_Canvas\_RFC.
...
5.  Restart the MID server to apply the changes.
    ...
    For more information, see [Manually start, stop, and restart a MID Server](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/servicenow-platform/mid-server/t_InstallMIDServerAsWinService.md).
...
6.  Verify that the SNC configuration is working correctly.
    ...
    3.  Point to the **Name** column and select the filter icon.
...
-   Confirm that the SAP clients are properly configured to use SNC. For more information, see [Configuring MID Server](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/servicenow-platform/mid-server/configure-mid-server.md).
