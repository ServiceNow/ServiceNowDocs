...
Canonical name. \(Mandatory if **SNC Connection** field is set to **true**.\)
...
Absolute path of the SAP Cryptography library file. \(Mandatory if **SNC Connection** field is set to **true**.\)
...
Enter one of the following numbers \(just the number, not the text\):-   0: Do not apply SNC to connections
...
Client Personal Security Environment \(PSE\) name. \(Mandatory if **SNC Connection** field is set to **true**.\)
