...
**Note:** You can use Build Agent to help you create and edit applications in ServiceNow Studio. For more information, see [Build Agent](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/application-development/build-agent.md).
...
2.  Select the Explorer tab icon \[Omitted image "sn-studio-explorer-icon.png"\] Alt text: in the activity bar and add an app in one of two ways.

    1.  If there are no applications in the Explorer yet, select **Create an app**.

    2.  If there are apps in the Explorer, select the add to workspace icon \[Omitted image "sn-studio-add-to-workspace-icon.png"\] Alt text: and select **Create an application**.
...
4.  Enter a name for the application and press Enter.

5.  Enter a description for the application and press Enter.

6.  For scoped applications, enter a scope name and press Enter.

    The scope name should fill automatically from the application name. The scope name must be unique on the instance, begin with x\_&lt;prefix&gt;, and be 18 characters or fewer. For more information, see [Namespace identifier](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/application-development/c_NamespaceIdentifier.md).

    If you receive an error because the scope name is not unique, begin the process again from the beginning.

7.  Enter a package name for the application and press Enter.
...
8.  Select a template that defines the default application structure and press Enter.

    -   Basic now-sdk boilerplate: An application with only the basic structure necessary for development in source code.
    -   now-sdk + AIUX Extension starter: An AIUX extension application that overlays pages onto an existing host experience.
    ...
    In the status bar at the bottom of your browser, watch the progress of the app as it builds.

    \[Omitted image "sn-studio-fluent-app-begin.png"\] Alt text: The status bar shows that your project is being created.

    \[Omitted image "sn-studio-fluent-app-progress.png"\] Alt text: App creation progress continues with different status messages.

    \[Omitted image "sn-studio-fluent-app-complete.png"\] Alt text: The app creation process finishes with a confirmation of completion.
