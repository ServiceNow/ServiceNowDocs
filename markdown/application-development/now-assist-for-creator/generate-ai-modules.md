...
# Generate modules using ServiceNow Otto
...
Use ServiceNow Otto to generate modules by providing prompts in natural language. The AI agent analyzes your prompt to identify the module type and key parameters, such as table names, views, roles, filters, and arguments. It then generates the requested modules.

## Procedure

1.  From the application navigator, select **All**, and then select **Create menu**.

    \[Omitted image "image.ui\_gen\_module\_gen\_create\_menu"\] Alt text: The Create menu button \(+\) highlighted in the UI Builder navigation panel.
...
8.  Select **Generate**.
    ...
    ServiceNow Otto generates the modules.
