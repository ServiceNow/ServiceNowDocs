...
Administrators must install the required applications, configure the agent in AI Agent Studio, enable the ServiceNow Otto panel, and activate the default ServiceNow Otto Panel- Platform assistant before developers can interact with the agent.

## Procedure

1.  [Install required applications for UI Builder Agent](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/application-development/now-assist-for-creator/configure-ui-builder-agent.md)

2.  [Enable UI Builder Agent in AI Agent Studio](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/application-development/now-assist-for-creator/configure-ui-builder-agent.md)

3.  [Enable the ServiceNow Otto panel](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/application-development/now-assist-for-creator/configure-ui-builder-agent.md)

4.  [Activate the ServiceNow Otto Panel- Platform \(default\) assistant](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/application-development/now-assist-for-creator/configure-ui-builder-agent.md)
...
Install ServiceNow Otto for Creator, UI Generation, and Conversational Studio from Application Manager before configuring the UI Builder Agent.
...
2.  In the search field, type `ServiceNow Otto for Creator` and press Enter.

3.  Select the **Installed** tab and verify that ServiceNow Otto for Creator \(`sn_now_creator`\) is listed and shows **Up to date**.
...
All three applications — ServiceNow Otto for Creator, UI Generation, and Conversational Studio — are installed and up to date on your instance.
...
Verify that you have installed all the required applications. See [Install required applications for UI Builder Agent](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/application-development/now-assist-for-creator/configure-ui-builder-agent.md).
...
8.  On the **Select channels and status** step, scroll down to **Activation status**, enable **This AI agent is active**, and then select**Test**.
    ...
    The system navigates to the Testing tab in AI Agent Studio. An alert banner appears indicating that you must enable the ServiceNow Otto panel before testing.


### Result

The UI Builder Agent setup wizard is complete. The agent configuration is confirmed, and you can enable the ServiceNow Otto panel to proceed with testing and activation.

## Enable the ServiceNow Otto panel

Enable the ServiceNow Otto panel in AI Admin Hub.
...
1.  On the AI Agent Studio Testing tab alert banner, select the **Edit this setting** link.
    ...
    Alternatively, navigate directly to **All** &gt; **AI Admin Hub** &gt; **Experiences**.

    The ServiceNow Otto panel page opens in AI Admin Hub, showing a summary of the panel and a **Turn on** button.

2.  In the **Turn on** section, select **Turn on**.

    A success dialog appears confirming that the ServiceNow Otto panel is turned on. The dialog shows that generative AI skills in the ServiceNow Otto panel are available. It also provides a link to the Assistants page in CI Admin console for further configuration.
...
The ServiceNow Otto panel is enabled on your instance and is available to users based on their designated permissions.
...
Activating AI Search turns on ServiceNow Otto in Virtual Agent.
...
## Activate the ServiceNow Otto Panel- Platform \(default\) assistant

Add a display experience to the ServiceNow Otto Panel- Platform \(default\) assistant, request AI Search activation, and activate the assistant in Assistant Designer.
...
The ServiceNow Otto Panel - Platform \(default\) assistant is the main system assistant in the Unified Navigation app shell. To use the UI Builder Agent through the ServiceNow Otto panel, you must assign at least one display experience to the assistant and turn it on.

### Procedure

1.  On the CI Admin Experience Assistants page, select **Manage assistants**.

    Assistant Designer opens, showing all available assistants including ServiceNow Otto Panel- Developer \(default\), ServiceNow Otto in Catalog Builder, and ServiceNow Otto Panel- Platform \(default\).

2.  On the ServiceNow Otto Panel- Platform \(default\) card, select **Edit**.
    ...
    The ServiceNow Otto Panel- Platform \(default\) assistant opens in Assistant Designer.
...
6.  Select **Save**.
    ...
    A success banner confirms that the ServiceNow Otto Panel- Platform \(default\) assistant is saved successfully.
...
7.  Select **Activate**.
    ...
    The status of ServiceNow Otto Panel- Platform \(default\) assistant in Assistant Designer changes from **Inactive** to **Active**.


### Result

The ServiceNow Otto Panel- Platform \(default\) assistant is active.

The UI Builder Agent is available to users through the ServiceNow Otto panel in UI Builder and across the platform.
