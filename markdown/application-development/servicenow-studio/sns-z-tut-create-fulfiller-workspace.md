---
title: Create the fulfiller workspace
description: Workspaces are a great option for departments. Workspaces can be tailored specifically for department needs and can simplify interacting with the platform.
locale: en-US
release: zurich
product: ServiceNow Studio
classification: servicenow-studio
topic_type: task
last_updated: "2025-07-31"
reading_time_minutes: 1
breadcrumb: [Tutorial part 1: Begin with an MVP, ServiceNow Studio tutorial, Explore, ServiceNow Studio, Developing your application, Building applications]
---

# Create the fulfiller workspace

Workspaces are a great option for departments. Workspaces can be tailored specifically for department needs and can simplify interacting with the platform.

## Before you begin

Role required: admin or delegated\_developer

## Procedure

1.  Navigate to **All** &gt; **App Engine** &gt; **ServiceNow Studio**.

2.  Select **Create** &gt; **File** &gt; **User Interface** &gt; **Workspace**.

3.  Select **Continue**.

4.  Select **Begin**.

5.  In the **Name** field, enter `Marketing Services`.

6.  Accept the other provided defaults for the workspace design and select **Continue**.

    ![Accept the defaults in the Marketing Services workspace.](../image/sns-z-tut-workspace.png)

7.  Select **Done**.


**Parent Topic:**[ServiceNow Studio tutorial part 1: Begin with an MVP](../concept/sns-z-tutorial-begin-with-mvp.md)

