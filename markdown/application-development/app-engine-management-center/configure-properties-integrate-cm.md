...
2.  Locate the following property: sn\_deploy\_pipeline.change\_management.default.template.
    ...
    2.  Select the update personalized list icon \(\[Omitted image "update-personalized-list-icon.png"\] Alt text: Update personalized list icon\) from the list header.

    3.  In the Available column, select **Template** and use the move icon \(\[Omitted image "app-tutorial-move-right-icon.png"\] Alt text: Move icon\) to add it to the Selected column.
