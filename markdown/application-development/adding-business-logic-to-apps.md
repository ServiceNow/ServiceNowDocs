...
In ServiceNow, two of the primary tools for adding server-side business logic are business rules and script includes. Learning about both, and how they work together, helps you build apps that are reliable, maintainable, and consistent.
...
Script includes are reusable JavaScript libraries that you define once and call from multiple places in your app. Unlike business rules, script includes don't fire automatically. You call them explicitly from a business rule, another script include, or an API. This makes them ideal for logic that needs to run in more than one context. For example, if several business rules have to calculate the same value, you write that calculation once in a script include and call it from each rule. This means that any future changes only have to be made in one place.
