...
**Note:** Once you choose an activity, you can select the information icon \(![Information icon](../../app-engine-studio/image/icon-information.png)\) to preview its Activity Definition record.
...
Brief description of the custom activity, which appears in the activity picker.
        ...
        -   See [Workflow Studio actions](https://www.servicenow.com/docs/access?context=flow-actions&version=xanadu&pubname=xanadu-build-workflows&ft:locale=en-US) for all standard actions.
        -   See [Workflow Studio flow logic](https://www.servicenow.com/docs/access?context=flow-logic&version=xanadu&pubname=xanadu-build-workflows&ft:locale=en-US) for a list of all flows.
...
3.  Specify which fields appear in the playbook activity's properties panel.
    ...
    **Note:** The fields that appear may have already been configured in the process of defining the activity. If so, follow these steps to verify that the available fields are correct.
    ...
    3.  Make the field appear for the activity’s properties in a Creator Studio by selecting **Always Show**.
        ...
        For more information, see [Add dynamic inputs to an activity](https://www.servicenow.com/docs/access?context=add-dynamic-inputs-to-activity&version=xanadu&pubname=xanadu-build-workflows&ft:locale=en-US).
