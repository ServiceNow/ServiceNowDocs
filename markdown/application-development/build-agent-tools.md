...
**Note:** You must have Australia Patch 2 and above to use the semantic search tool.
...
You can specifically prompt UI validation by asking Build Agent to `validate the UI built using the ATF cloud runner tooling`.The UI validation tool is available for Build Agent in both ServiceNow Studio and the ServiceNow IDE, though you must be on Australia Patch 3 or higher to use it in ServiceNow Studio.
