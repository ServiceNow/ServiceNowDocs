...
If none of the technicians are available or support the task's demand channel, the task is not scheduled and is shown in the run summary for the optimization run. For more information, see [Reviewing Schedule Optimization results](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/field-service-management/field-service-scheduling/viewing-task-assignments-from-so-runs.md).
