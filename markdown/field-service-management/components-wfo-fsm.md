...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/field-service-management/components-wfo-fsm.html
release: xanadu
...
|sn\_shift\_planning\_agent\_time\_work|Stores the agent's time worked, such as clock-in and clock-out times. If an agent logs in early or logs out late, the system uses the shift start and end times instead of the clock-in and clock-out times.|
...
Sets the percentage for a threshold in adherence. The agents who don’t qualify as above the defined threshold value \(70%\) are considered non-adherent agents. -   **Type:** Integer
-   **Default value:** 70%
...
Set the lower threshold percentage for flexibility in conformance. The agents who don’t qualify for the defined lower and upper conformance threshold values \(80-120\) are considered non-conformant agents. -   **Type:** Integer
-   **Default value:** 80%
...
Sets the upper threshold percentage for flexibility in conformance. The agents who don’t qualify for the defined lower and upper conformance threshold values \(80-120\) are considered non-conformant agents.-   **Type:** Integer
-   **Default value:** 120%
...
|--------------|-----------|
|Notify User Clock In|Counts the clock-in events whenever there’s a change in agent's Presence state.|
|Notify User Clock Out|Counts the clock-out events whenever there’s a change in agent's Presence state.|
...
Clock-out event generation \(For scenarios where the clock-out event doesn’t get triggered\)

</td><td>

Generates clock-out events every 4 hours when agents haven't clocked out. **Note:**

For example, say an agent's work shift times are from 8 a.m. to 5 p.m. and the **sn\_shift\_planning.auto\_clockout\_threshold** property value is set to 60 minutes. If the agent clocks in and fails to clock out, the scheduled job waits until 6 p.m. \(5 p.m.+60 minutes\) and if there’s no clock out until 6 p.m., the system generates a clock-out event with the agent's shift end time.
...
|Coaching Opportunity for low CSAT|Coaching opportunity for agents who worked on tasks that have a customer satisfaction score less than 4.|Task Report \[sn\_customerservice\_task\_report\]|
...
Number of days to read a knowledge article or complete training. The admin \(sn\_wfo.admin\) sets the number of days for the trainee to complete reading an article or complete training. The number of days is converted to the due date for the trainee to complete the training. It’s calculated from the current date, taking the trainee's time zone into consideration.-   **Type:** Integer
-   **Default value:** 5
...
You must add this system property to set the desired value. For more information, see [Add this system property](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/platform-administration/r_AvailableSystemProperties.md).

When you synchronize third-party learning content with your ServiceNow instance, if the word count of the content being pulled into your instance exceeds the value set for this property, the synchronization will fail. For more information, see [Integrate Coaching With Learning with third-party learning management systems](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/customer-service-management/workforce-optimization-for-customer-service/integrate-learning-third-party-configurable-wfo-cs.md).
 ...
 **Note:** When you synchronize third-party learning content with your ServiceNow instance, if the word count of the content being pulled into your instance exceeds the value set for this property, the synchronization fails. For more information, see [synchronize](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/customer-service-management/workforce-optimization-for-customer-service/integrate-learning-third-party-configurable-wfo-cs.md).
