...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/field-service-management/service-locations-work-orders.html
release: xanadu
...
If you do not want to use the auto-populated location in the **Location** field when creating a work order or work order task, you can add an ad hoc location. The default validation option for service locations determines whether you can use the Google map \(Using map\) or enter the address manually \(Without map or No validation\). For more information, see [Set the default validation option for service locations](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/field-service-management/workforce-optimization-for-field-service/set-default-validation-option-for-service-locations.md).
...
The address information is entered in the text field or by dragging and dropping the marker. The fields auto-populate based on the address entered in the search field on a map.
 ...
 The Google Maps API key or client ID must be set in order to use the Google Maps API for Business. For more information, see [Set up Google Maps API](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/platform-user-interface/configure-user-experiences/set-up-google-maps-api.md).
...
</table>**Parent Topic:**[Field Service with Service Locations](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/field-service-management/field-service-with-service-locations.md)
