---
title: Configure Data Collection for APM
description: Configure Data Collection for APM.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/impact/value-library/dc-apm-config.html
release: yokohama
product: Value Library
classification: value-library
topic_type: task
last_updated: "2025-01-30"
reading_time_minutes: 1
breadcrumb: [Impact Value Management Data Collection for APM, Data Collection Toolkit, Value Management, Impact Delivery Instance \(formerly Impact Digital Experience\), Impact]
---

# Configure Data Collection for APM

Configure Data Collection for APM.

## Before you begin

You must update Manual Indicators periodically \(monthly, for example\) using the scoresheet function.

Role required: admin, pa\_power\_user, pa\_admin, or pa\_data\_collector

## Procedure

1.  Navigate to **Performance/Platform Analytics &gt; Scoresheet**, and then select **Impact VM - Legacy APM systems run-rate**.

2.  Enter the data point in the relevant month cell.

    \[Omitted image "dct\_config\_apm.png"\] Alt text: Example with Jul 2024 cell selected with an Indicator score of 3,000.


