---
title: Edit permissions for CBS Knowledge Readers
description: Edit the Can Read user criteria to add knowledge readers in Core Business Suite.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/core-business-suite/cbs-perm-kb-read.html
release: brazil
topic_type: task
last_updated: "2026-09-10"
reading_time_minutes: 1
keywords: [CBS Knowledge Readers, permissions, user criteria, Core Business Suite]
breadcrumb: [Knowledge Base in Core Business Suite, Configure, Core Business Suite]
---

# Edit permissions for CBS Knowledge Readers

Edit the Can Read user criteria to add knowledge readers in Core Business Suite.

## Before you begin

Role required: sn\_cbs.admin

## About this task

You can perform the same steps to edit knowledge bases under each business unit module.

## Procedure

1.  Navigate to **Core Business Suite** &gt; **Configure**.

2.  Select **Knowledge** in Core Business Suite module under the Configuration Console.

3.  In the Manage Permissions section, select the default Can Read user criteria.

4.  On the CBS Knowledge Readers form page, fill in the fields and select **Save**.

    For a description of the form values, see [CBS Knowledge Readers form](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/core-business-suite/cbs-reader-ref.md).


## Result

The user criteria is updated with the specified knowledge readers.

**Parent Topic:**[Knowledge Base in Core Business Suite](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/core-business-suite/kb-cbs.md)

