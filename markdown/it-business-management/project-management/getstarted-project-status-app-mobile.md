---
title: Get started with Project Status app
description: Access project status and status reports on the Project Status mobile app to review your project status and collaborate with stakeholders regardless of your location.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/it-business-management/project-management/getstarted-project-status-app-mobile.html
release: yokohama
product: Project Management
classification: project-management
topic_type: task
last_updated: "2025-01-30"
reading_time_minutes: 1
breadcrumb: [Manage status of your projects using Project Status application, Use, Project Management, Project Portfolio Management, Strategic Portfolio Management]
---

# Get started with Project Status app

Access project status and status reports on the Project Status mobile app to review your project status and collaborate with stakeholders regardless of your location.

## Before you begin

-   Download the Mobile Agent mobile application on an iOS platform from the Apple App Store or on an Android platform from the Google Play Store.
-   [Activate the PPM Mobile plugin](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/it-business-management/project-management/activate-ppm-mobile.md).

Role required: admin

## Procedure

1.  Open the mobile app and tap the plus icon \(\[Omitted image "mobile-plus-icon.png"\] Alt text: Add new instance.\).

2.  Add a ServiceNow instance and enter the credentials to login.

    You do not need to include `service-now.com` at the end of the instance name.

3.  You can give a **Nickname** to your instance for easy identification.

4.  Tap **Project Status** \(\[Omitted image "proj-status-mob.png"\] Alt text: Project status icon.\) to get started with tracking your projects.


**Related topics**  


[Agent mobile app](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/mobile/mobile-platform/mobile-experience.md)

