---
title: Collaborate with stakeholders from Project Workspace
description: Add comments and attachments to the project or a project task and share them with other users of your organization. Collaborate and verify that your project is in line with your organizational priorities.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/it-business-management/project-workspace/collaborate-with-stakeholders-from-project-workspace.html
release: zurich
product: Project Workspace
classification: project-workspace
topic_type: task
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Manage projects, Project Workspace, Project Portfolio Management, Strategic Portfolio Management]
---

# Collaborate with stakeholders from Project Workspace

Add comments and attachments to the project or a project task and share them with other users of your organization. Collaborate and verify that your project is in line with your organizational priorities.

## Before you begin

Role required: it\_project\_manager

## About this task

Browse and upload files from your computer. Add comments and tag stakeholders for their feedback or notes.

## Procedure

1.  Open a project from the home page of Project Workspace.

    For information, see [Access the Project Workspace](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/it-business-management/project-workspace/access-new-project-workspace.md).

2.  Attach files to your project or project task.

<table id="choicetable_sj5_5hz_hsb"><thead><tr><th align="left" id="d138702e92">

Option

</th><th align="left" id="d138702e95">

Action

</th></tr></thead><tbody><tr><td id="d138702e101">

**Project**

</td><td>

1.  Select the Attachments icon \(\[Omitted image "icon-attachments.png"\] Alt text: Attachments icon.\) from the planning header.
2.  Upload a file from your computer.
    1.  Select **Select file**.

If you have attachment, select the Add file icon \(\[Omitted image "icon-add-file.png"\] Alt text: Add File icon.\).

    2.  Browse and upload a file.


</td></tr><tr><td id="d138702e145">

**Project task**

</td><td>

1.  Select a project task to open its details in the side panel.
2.  Navigate to the Attachment tab.
3.  Upload a file from your computer.
    1.  Select **Select file**.

If you have attachment, select the Add File icon \(\[Omitted image "icon-add-file.png"\] Alt text: Add File icon.\).

    2.  Browse and upload a file.


</td></tr></tbody>
</table>3.  Add comments or tag stakeholders to draw their attention to an attachment or information about the project.

    You can add comments directly at the project level or for a project task.

<table id="choicetable_fx3_gjz_hsb"><thead><tr><th align="left" id="d138702e198">

Option

</th><th align="left" id="d138702e201">

Action

</th></tr></thead><tbody><tr><td id="d138702e207">

**Project**

</td><td>

1.  Select the Activity icon \(\[Omitted image "icon-activity.png"\] Alt text: Attachments icon.\) from the planning header.
2.  Enter your comments in the text box.

If you want to address your comments to a user, @-mention their name in the comment directly.

3.  Select **Post Comments**.


</td></tr><tr><td id="d138702e240">

**Project task**

</td><td>

1.  Select a project task to open its details in the side panel.
2.  Navigate to the Activity tab.
3.  Enter your comments in the text box.

If you want to address your comments to a user, @-mention their name in the comment directly.

4.  Select **Post Comments**.


</td></tr></tbody>
</table>
**Parent Topic:**[Managing projects with Project Workspace](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/it-business-management/project-workspace/use-projects-pw.md)

