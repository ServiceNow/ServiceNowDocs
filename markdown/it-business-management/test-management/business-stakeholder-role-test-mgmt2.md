...
When the read-only roles for Test Management 2.0 plugin \(com.snc.tm2\_read\_roles\) and Test Management 2.0 plugin \(com.snc.test\_management.2.0\) are active in your ServiceNow instance. In this case, the user with the business stakeholder role \(sn\_tm2\_read\) has read access to the following tables.
