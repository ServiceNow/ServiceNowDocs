...
-   Work item table API \(awa\_work\_item\) to look up the queued work items waiting for assignment. For information on table API, see [Table API](https://www.servicenow.com/docs/access?context=c_TableAPI&version=xanadu&pubname=xanadu-api-reference&ft:locale=en-US).
