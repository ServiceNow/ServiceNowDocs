...
-   Define a [template](https://www.servicenow.com/docs/access?context=c_Templates&version=xanadu&pubname=xanadu-platform-administration&ft:locale=en-US) to assign a static field value for all records created by the record producer.
-   Define a script that uses any variable or server-side objects and functions to assign values.
    ...
    -   Use [RP.getParameterValue](https://www.servicenow.com/docs/access?context=RenderProperties-Scoped-API&version=xanadu&pubname=xanadu-api-reference&ft:locale=en-US) to return specified parameter values passed on the URL. The RP stands for Render Properties, not Record Producer.
