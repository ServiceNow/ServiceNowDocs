...
If selected, adds a title and an expandable header to the right of the variable set.**Note:** When a user requests the item, all [Check box](../reference/check-box.md) variables are grouped under a default title of **Options**. To use a custom title, insert a variable of type **Label**, with an **Order** value that puts it directly above the check box variables.
