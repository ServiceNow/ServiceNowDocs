...
**Note:** Prior to the Fuji release, activities were built with [Activity Definitions](https://www.servicenow.com/docs/access?context=t_ViewActivityDescriptions&version=xanadu&pubname=xanadu-build-workflows&section=r_ElmtsWkflwActDef&ft:locale=en-US). In Fuji, a new architecture was introduced using the Activity Designer. Orchestration Core activities, built originally as Activity Definitions, have been converted to use the new template format \(Activity Elements\) and the databus offered with the activity designer. Workflows in upgraded instances that use legacy activities continue to work normally.

For instructions on using activities to construct a workflow, see [Add an activity to a workflow](https://www.servicenow.com/docs/access?context=t_AddAnActivityToAWorkflow&version=xanadu&pubname=xanadu-build-workflows&ft:locale=en-US).
