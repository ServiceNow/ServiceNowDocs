...
-   **Application Service table \[cmdb\_ci\_service\_auto\]**

    The application service is typically the system that a caller identifies when they report an issue with an application.
    ...
    You might have several application services representing each deployment based on the environment \(development, QA, production\) and location or geography \(North America, Asia Pacific\).
