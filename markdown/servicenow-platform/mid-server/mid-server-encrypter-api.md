...
-   -   **CyberArk**: Data security is provided by [CyberArk integration configuration](https://www.servicenow.com/docs/access?context=c_CyberArkIntegrationConfiguration&version=zurich&pubname=zurich-platform-security&ft:locale=en-US), which moves sensitive data from the **config.xml** file to a secure CyberArk vault. This solution does not encrypt the data.
