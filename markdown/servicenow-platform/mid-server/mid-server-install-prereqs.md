...
</table>-   Verify that the host computer satisfies the [MID Server system requirements](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/servicenow-platform/mid-server/r_MIDServerSystemRequirements.md).
...
Enter the name of the [MID Server user](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/servicenow-platform/mid-server/t_SetupMIDServerRole.md) that you already created. The MID Server user must have the mid\_server role.
...
To upgrade the MID Server, see [MID Server upgrades](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/servicenow-platform/mid-server/c_UpgradeAndTestMIDServer.md) for procedures and requirements.
...
3.  The MID Server can also be uninstalled with the silent uninstall script.
    ...
    This is useful for uninstalling multiple MID Servers. For more information, see [Windows MID Server silent installation and uninstallation](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/servicenow-platform/mid-server/mid-server-install-prereqs.md)
...
Windows service credentials control the level of privilege on the device. The service account must meet the following requirements:

-   The account cannot be a local system or an administrator-level account \(local admin, domain admin, and so on\).
-   The account must have the **Log on as a service** right, which is required for an account to be used as the log on user for a service.
-   The account must have the **Increase a process working set** privilege, which is required for the MID Server to manage its memory allocation.

**Note:**

Windows service credentials are not the same as the MID Server user credentials, which allow communication between the MID Server and the instance. You must configure both of these credentials separately. See [Create the MID Server user and grant the role](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/servicenow-platform/mid-server/t_SetupMIDServerRole.md) for instructions on MID Server user credentials.
...
-   **MUTUAL\_AUTH**: enables mutual authentication. If this switch is enabled, **MID\_USERNAME** and **MID\_PASSWORD** are not required. See [MID Server unified key store](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/servicenow-platform/mid-server/mid-unified-keystore.md) for more information.
...
[Validate](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/servicenow-platform/mid-server/t_ValidateAMIDServer.md) the MID Server to prepare it for use.
...
Ensure that your JRE version is supported. See [MID Server system requirements](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/servicenow-platform/mid-server/r_MIDServerSystemRequirements.md) for details.
