...
6.  To configure the MID Server manually, edit the `config.xml` file with a text editor such as WordPad:
    ...
    4.  Enter connection information for the proxy server.
        ...
        -   **mid.proxy.username**
        -   **mid.proxy.password**
7.  Run `start.bat` to start the MID Server.

8.  On the instance, in the **Related Links**, select **Validate**.
...
**Note:** In a future release, non-admin accounts will be mandatory for the MID Server, and support for using an administrative account will be deprecated.
...
1.  Run the MID Server service as LocalSystem or as a user with admin rights.

2.  Create a file entitled `GrantStartStopPerm.ps1` with the following code.
...
3.  Start the PowerShell console as an administrator.

4.  Load the file by using the dot-source function from the `GrantStartStopPerm.ps1` file.

    For example: `C:\Users\mid_account\Desktop> . .\GrantStartStopPerm.ps1`

5.  Run PowerShell sourced function with non-admin account and the service name as input.
...
6.  Add the non-admin account name to the MID Server parameter **mid.windows\_host.file\_permissions.allow\_list** in the MID Server host's `config.xml` file.

    This step adds the non-admin account to the allow list. See [MID Server parameters](../reference/mid-server-parameters.md#section_security) for more information about **mid.windows\_host.file\_permissions.allow\_list** as well as instructions for adding a parameter to the `config.xml` file.

7.  Restart the MID Server.

    The new file permission enforcement rules take effect when the MID Server starts.

8.  Switch the MID Server to the non-admin service account.
...
For more information about managing the allow list and file permission enforcement, see [File permission enforcement for Windows MID Servers](mid-non-admin-permission.md).
