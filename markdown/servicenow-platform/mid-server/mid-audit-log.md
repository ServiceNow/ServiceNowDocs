...
Discovery supports JEA profiles for WinRM. The MID Server command audit log records the JEA profile of the discovery command, if it is available. See [Microsoft Just Enough Administration \(JEA\) for Discovery](https://www.servicenow.com/docs/access?context=microsoft-jea-discovery&version=xanadu&pubname=xanadu-it-operations-management&ft:locale=en-US) for more information on JEA profiles.

By default, the table is rotated every seven days. For more information, see [Table Rotation](https://www.servicenow.com/docs/access?context=c_TableRotation&version=xanadu&pubname=xanadu-platform-administration&ft:locale=en-US).
