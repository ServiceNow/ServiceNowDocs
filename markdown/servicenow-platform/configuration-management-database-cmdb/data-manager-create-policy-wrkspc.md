...
-   Verify that any custom subflow that you want to associate with a policy exists:
    ...
    4.  Select **Create new subflow** to [create a new subflow in Workflow Studio](https://www.servicenow.com/docs/access?context=subflows&version=zurich&pubname=zurich-build-workflows&ft:locale=en-US).
...
-   When [Asset Management](https://www.servicenow.com/docs/access?context=c_AssetManagement&version=zurich&pubname=zurich-it-service-management&ft:locale=en-US) is activated, check if there's an asset record associated with that CI before retiring the CI. Check the associated asset record, if there's one, and verify that its asset state \(install\_status\) is **Retired**.
...
The length of time for retaining archived CIs in the archive table before they're deleted.
 ...
 During the specified retention period, you can use the ServiceNow AI Platform® data archiving feature to [restore archived CIs](https://www.servicenow.com/docs/access?context=managing-archived-data&version=zurich&pubname=zurich-platform-administration&ft:locale=en-US).
...
-   For Attestation policies, attestation tasks are assigned to users as specified, and those tasks appear in the [CMDB Workspace](../concept/cmdb-workspace.md)and [Service Graph Workspace store app](../concept/sg-workspace.md) when those users log in.
...
-   Users log into [CMDB Workspace](../concept/cmdb-workspace.md)or to [Service Graph Workspace store app](../concept/sg-workspace.md) to [review and process attestation tasks](review-data-manager-attes-task.md) assigned to them.
-   Users log into [CMDB Workspace](../concept/cmdb-workspace.md)or to [Service Graph Workspace store app](../concept/sg-workspace.md) to [review and process non-attestation tasks](data-manager-review-task-wrkspc.md) assigned to them.
