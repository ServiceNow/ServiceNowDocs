...
2.  To run a simulation of an existing payload:
    ...
    3.  Select **Use Enhanced Identification** to apply the [identifyCIEnhanced](https://www.servicenow.com/docs/access?context=c_IdentEngineScriptAPI&version=zurich&pubname=zurich-api-reference&ft:locale=en-US) API for enhanced CI identification, instead of using the identifyCI API.
...
2.  After finalizing the payload, use the [createOrUpdateCI\(\)](https://www.servicenow.com/docs/access?context=c_IdentEngineScriptAPI&version=zurich&pubname=zurich-api-reference&ft:locale=en-US) API to execute the payload by IRE which will result in actual updates to the CMDB.
