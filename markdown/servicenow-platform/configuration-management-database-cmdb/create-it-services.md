...
Service Mapping, if activated, can automatically discover and map application services as described in [Application service mapping](https://www.servicenow.com/docs/access?context=c_DefineMapBusinessServices&version=xanadu&pubname=xanadu-it-operations-management&ft:locale=en-US). A discovered application service contains the CIs and the connections between them that Service Mapping discovered and mapped.

You can also create an application service by using the [createOrUpdateService - POST](https://www.servicenow.com/docs/access?context=application-service-api&version=xanadu&pubname=xanadu-api-reference&ft:locale=en-US) REST API.
...
3.  In the **Provide Basic Details** tab:
    ...
    1.  Fill out the fields for Basic Details.
        ...
        ![Basic details tab for new application service](../image/app-services-tab1.png)
...
4.  On the **Populate the Application Service** tab:
    ...
    2.  On the Choose a Method page, select a **Service Population Method**, and then follow the respective link to complete the specific population method:
        ...
        -   **Top Down Discovery**: [Populate application services using top-down discovery](../../configuration-management/task/populate-app-service-top-down.md)
        ...
        -   **Tags**: [Populate application services using tags](../../configuration-management/task/populate-app-service-tags.md)
        -   **Manual**: [Populate an application service using the Manual method](../../configuration-management/task/populate-app-service-manual.md)
    ...
    3.  To add another method to populate the application service, click **Add Method** on the Service Population Methods page.
        ...
        -   You can click a card for a Converted Business Service method to see details about the service conversion, such as the conversion type. For more information, see [Convert business services to application services](../concept/convert-bus-to-app-svc-intro.md#).
...
-   If the service population method is **Tags**, **Top Down Discovery**, or **Manual**, and click **View Map** to [view the application service map](../../configuration-management/task/app-service-map-base-system.md) where you can:
    -   [Link application services](https://www.servicenow.com/docs/access?context=link-services-to-services&version=xanadu&pubname=xanadu-it-operations-management&ft:locale=en-US)
    -   [View CI attributes in an application service map](https://www.servicenow.com/docs/access?context=t_ViewCIDetails&version=xanadu&pubname=xanadu-it-operations-management&ft:locale=en-US)
    -   [View the change history of application services](https://www.servicenow.com/docs/access?context=t_ViewCIChanges&version=xanadu&pubname=xanadu-it-operations-management&ft:locale=en-US)
    -   [Compare two versions of an application service](https://www.servicenow.com/docs/access?context=t_CompareBS&version=xanadu&pubname=xanadu-it-operations-management&ft:locale=en-US)
