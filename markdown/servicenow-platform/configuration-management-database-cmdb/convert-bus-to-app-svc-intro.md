...
-   \(Optional\) On a change request form, view affected dynamic services. For example, after you add an affected CI that is associated with a dynamic service:
    ...
    For more information about affected CIs on a change request, see [Associated CIs on a change request](https://www.servicenow.com/docs/access?context=c_AffectedCIsAndImpactedServices&version=zurich&pubname=zurich-it-service-management&ft:locale=en-US).
