---
title: Create incidents and change requests from a requested item
description: You can quickly open an incident or a change request from a requested item. It also maintains a relationship between the requested item, and its associated incidents or change requests.
locale: en-US
release: australia
product: Self-service
classification: self-service
topic_type: task
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Service catalog requests, Self-Service, Self-service, Manage service capabilities, Extend ServiceNow AI Platform capabilities]
---

# Create incidents and change requests from a requested item

You can quickly open an incident or a change request from a requested item. It also maintains a relationship between the requested item, and its associated incidents or change requests.

## Before you begin

The administrator must enable the **Create Change** and **Create Incident** UI actions for the Requested Item \[sc\_req\_item\] table. The administrator must set the **Active** field to `true` when editing these UI actions .

Role required: itil

## Procedure

1.  Navigate to **All** &gt; **Self-Service** &gt; **Requested Items**.

2.  Open a requested item.

3.  Click the Additional actions menu icon ![Additional actions menu icon](../image/AdditionalActions.png) and select **Create Change** or **Create Incident**


**Parent Topic:**[Service catalog requests](../topic/p_ServiceCatalogRequests.md)

**Related topics**  


[Record Producer](../../../product/service-catalog-management/concept/c_RecordProducer.md)

[Service catalog requests](../topic/p_ServiceCatalogRequests.md)

