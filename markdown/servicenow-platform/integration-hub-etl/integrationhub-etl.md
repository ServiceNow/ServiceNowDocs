...
-   [Robust Transform Engine \(RTE\)](https://www.servicenow.com/docs/access?context=robust-import-set-transformers&version=zurich&pubname=zurich-integrate-applications&ft:locale=en-US): Used to transform raw source data that is stored in staging tables, into the data that is mapped and integrated into the CMDB. RTE uses ETL transform maps that were created for the integration during data transformation.
...
To process nested data payloads, you must first ensure that the data source that is used for the integration, is set with the [Data in single column](https://www.servicenow.com/docs/access?context=r_DataSourceFileTypeFields&version=zurich&pubname=zurich-integrate-applications&ft:locale=en-US) option. With that setting, you can correctly represent nested data in a JSON payload which IntegrationHub ETL then processes as nested data, rather than as flat data.
