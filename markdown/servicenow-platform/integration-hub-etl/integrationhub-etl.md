...
To process nested data payloads, you must first ensure that the data source that is used for the integration, is set with the [Data in single column](https://www.servicenow.com/docs/access?context=r_DataSourceFileTypeFields&version=xanadu&pubname=xanadu-integrate-applications&ft:locale=en-US) option. With that setting, you can correctly represent nested data in a JSON payload which IntegrationHub ETL then processes as nested data, rather than as flat data.
