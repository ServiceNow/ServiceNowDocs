...
-   **Flow Dependencies**
    ...
    Returns all the server to server connections that were discovered using the Netflow collector. The script builds a graph based on data in the \[sa\_flow\_server\_comm\] table. This table contains pairs of services represented by an IP and a listening port that are communicating with each other. For more information, see Data collection and discovery using Netflow and Data collection and discovery using VPC Flow Logs.
...
[bundle-itom.data-collection-netflow-mapping]
