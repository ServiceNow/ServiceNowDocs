...
You can then use the e-signature template in task forms to request electronic signatures from users. In the HR Service Delivery application, for example, you can add HR tasks to HR cases or configure HR task templates to request electronic signatures per the selected e-signature template. See [E-signature for HR](https://www.servicenow.com/docs/access?context=hr-e-signature&version=australia&pubname=australia-employee-service-management&ft:locale=en-US) for HR-specific examples.
