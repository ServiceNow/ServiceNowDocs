...
1.  On the trigger of the trouble ticket event, the system invokes the appropriate business rule, and stamps the event type.
    ...
    To learn more about the business rule that you must add to your ServiceNow instance, see [Add a business rule for a new trouble ticket event](https://www.servicenow.com/docs/access?context=prd_evt_not-dev_gd-add_bus_rule&version=yokohama&pubname=yokohama-api-reference&ft:locale=en-US).
...
4.  The topic picker mechanism determines the topics, which are compatible with the event type. Topic picker performs the following steps to check the compatibility of the topics:
    ...
    To learn more details about how to customize the existing topic picker mechanism, see [OpenMessageBusEventPublisherOOB - Scoped](https://www.servicenow.com/docs/access?context=OpenMsgBusEvtPubScopedAPI&version=yokohama&pubname=yokohama-api-reference&ft:locale=en-US).
...
[Handling the external events using Telecommunications API notification](https://www.servicenow.com/docs/access?context=telecommunications-api-notification&version=yokohama&pubname=yokohama-telecom-service-ops&ft:locale=en-US)
