---
title: Australia Patch 4m release notes
...
# Australia Patch 4m release notes
...
New: A new 'Specify document type' field has been added to the Document Verification Task, letting users indicate the type of document being verified.
...
-   Fixed: For XCC \(External Content Connector\): Agent prompt quality improvements.
...
Changed: Express List accessibility improvements. The Express List is now fully accessible to users with low vision, expanding the user base and ensuring compliance with international accessibility standards and regulations.; Consolidated ACLs for access control management. Access control lists have been consolidated to better follow security best practices, reducing risks and simplifying management.; Link View topology visualization accessibility. Link View topology visualization now meets accessibility standards while maintaining usability and performance.

 Fixed: The Express List layout has been corrected to properly adapt for narrow viewports and no longer breaks at certain screen sizes.; Unintended double scrollbars in accessibility reflow mode have been eliminated.; The Express List columns now display correctly in Japanese and CJK languages, resolving translation and layout issues.; The Express List live list functionality now properly triggers and updates in real time.; The 'update available' banner no longer appears incorrectly when resuming the live list.; Saving an Alert Management Rule with 'Create Incident Advanced' now works in all scenarios.; Operators with more than five assignment groups can now see their created Enrich automations in the Enrich module.; Alerts manually removed from a group are no longer automatically re-added.; A broken documentation link in the alert automation configuration has been fixed.; Dynatrace Monitor setup instructions in the Launchpad have been updated to match the current Dynatrace console.; The 'Tags' label in the Launchpad is now correctly translated across contexts.; Concatenated strings causing grammatical issues in certain languages have been separated for correct localization.; Launchpad forms now match the approved design at 400% zoom.; Dotted connection lines in Link View now appear when connected CIs are returned from the backend.; The Simulation UI now displays 'This automation' or the saved automation name instead of 'MIXED' or 'CMDB' labels for Mixed or CMDB group types.; The 'All Time \(Last 180 days\)' option now shows as selected in the time range picker and is correctly translated.; Service health percentages now display correctly in locales using a comma as the decimal separator.; Ellipsis menu items in the Log Viewer now display in the user's language setting.; Backspace behavior in Extended mode is now correct.; The evt\_mgmt\_admin role now has write access to Express List properties.
...
Fixed: Enhanced the user interface for the Change models and Risk configuration pages.
...
-   Defect fixes Fixed unused icon import in knowledge\_article component.
...
-   Fixed: AI forecasted status and AI rationale fields from goal insights are updated on the grid in realtime without grid refresh.
...
Model default change to 3P and model version updates.
...
-   Prompt Library for the Premium Chat Assistant. Admins can view out-of-the-box default prompts with read-only filters and manage their own prompts in the Prompt Library, backed by a new schema and Create Script API in the NAVA app.
-   Post-chat survey support for Premium Chat. Admins can configure a Post-chat Survey Topic and Closing Topics for Premium Chat, and Closing Topics for Enhanced Chat. Existing survey and closing topics are migrated automatically.
-   Additional semantic search sources for Synthesized Response. Admins can add semantic search sources through the 'Add search source' dropdown, with display marking that indicates which sources are and are not used in the Synthesized Response.
-   Search-profile override for the Enhanced Chat Dynamic Window. Admins can toggle an Assistant Search Profile override in the Dynamic Window experience, enabled per Assistant through a new Agentic Orchestration configuration attribute, with portal split configuration and the corresponding flag passed through to mobile.
-   Premium Chat support for Mobile Custom Apps. Custom Apps on mobile can now be set to either the Enhanced or Premium Chat experience.
-   Citations for additional search source types. The Requestor experience now shows citations for additional types of search sources.
-   AI Agent Studio Assistant hidden from selection. The AI Agent Studio Assistant is no longer surfaced in the Assistant list.
-   PRB2024249: Following up a non-conversational catalog item with the same term no longer submits the request without URLs.
-   PRB2039583: Corrected Prompt Library text in Assistant Designer.
-   PRB2038815: The Prompt Library toggle now triggers auto-save correctly.
-   PRB2038884: Restored the missing tooltip for Conversational Catalog Items.
-   PRB2038480: Promoted assets no longer mis-map an Agentic Workflow to AI agent type; the Add-asset dropdown now shows the correct Agentic Workflow option.
-   PRB2038346: OOTB prompts now correctly show 'ServiceNow' as the creator.
-   PRB2030411: Resolved multiple UI issues in the Display Experience portal.
-   PRB2032277: The review page in the create flow no longer omits configurations.
-   PRB2032733: Custom greeting and live-agent topics are no longer silently lost during the June 2026 upgrade migration when default settings already exist.
-   PRB2030773: Fixed a localization warning caused by improper handling of a parameterized message in the static greeting.
-   PRB2030421: Resolved multiple UI issues in the Display Experience channel.
-   PRB2032989: Chat experience tabs are no longer mislabeled as 'Legacy' when Premium Chat is unavailable in Assistant Designer.
-   PRB2024688: Fixed layout issues for longer strings on smaller windows and icon rendering for non-English languages.
-   PRB2035197: Premium display experience now defaults to Otto branding.
-   PRB2033795: Granular feedback options are now translated correctly.
-   PRB2034709: Inactivated the Search Q and amp;A agent in Now Assist in Virtual Agent.
-   PRB2029072: Fixed the NAP Developer assistant entry in the All assets and gt; Add from library modal.
-   PRB1998145: Voice calls in the Testing UI now connect reliably on the first attempt.
-   PRB2030310: Resolved copy and font-size issues when enabling chat features in Virtual Agent.
-   PRB2029063: The All assets and gt; Add from library modal now shows the correct count.
-   PRB2033251: Custom greeting topics now correctly convert OOB NAVA and NAP greeting topics to tools on load.
-   PRB2029106: Fixed a header UI issue on the Promoted asset tab.
-   PRB2030701: Resolved issues with secondary-language support when phonemes are deleted or updated.
-   PRB2027372: Fixed RTL-language layout issues under Now Assist Voice Deployment in Assistant Designer.
-   PRB2027159: Translated previously untranslated strings under Now Assist Voice Deployment in Assistant Designer.
-   PRB2022882: Disambiguated 'min' in the Now Assist Voice Deployment designer.
-   PRB2032619: Hid the Custom option from the SIP CCaaS provider list.
-   PRB2027179: Fixed RTL-language layout issues under the Assistant tab in Assistant Designer.
-   PRB2026588: Corrected the tooltip that incorrectly showed 'Sort' for field labels.
-   PRB2013063: The Assistant test panel now refreshes when testing multiple assistants sequentially.
-   PRB2027467: Label, value, and other options are now disabled until a type is selected when adding a chat experience.
-   PRB2032643: The org-chart widget now loads on mobile.
-   PRB2032268: Labels now populate correctly on the modal for 'chat' type Premium Chat branding.
-   PRB2027171: Fixed RTL-language layout issues under the Assistant tab in Assistant Designer.
...
-   New: A new web search tool capability enabled Build Agent to find answers from the public internet when its internal knowledge sources don't have them. This tool needs to be toggled on from the settings screen of the Build Agent chat panel.
...
This release focused on improving the quality and accuracy of AI Specialists responses as well as consolidating feedback management.
 ...
 Memory bank to give AI Specialists context of resolutions that lead to positive outcomes AI Specialists now have an updated memory type that enables them to reference past resolutions that led to a positive outcome on assigned records. This is disabled by default for this release, but can be enabled by administrators via system properties.
...
-   New: AI-powered HAM advisor dashboard summarization: Leverage the summary section in HAM advisor dashboard to get precise recommendations and insights on what actions to take to get your CMDB ready to drive HAM business outcomes.
...
New: Build Agent is adding a web search tool capability so it can find answers from the public internet when its internal knowledge sources don't have them. This tool needs to be toggled on from the settings screen of the Build Agent chat panel.
...
Widget editor Support for new karuna runtime.

 Preview the canvas by user / role.

 Widget editor Defect fixes.
...
This release includes an improvement of firewall task management AI to accept multiple rule configurations for a single task.
...
-   Fixed: Resolved an issue where the Create Demand modal in Portfolio Planning did not work.
...
-   New: Autonomous AI Agent that scans incident clusters and automatically generates draft knowledge base articles and problem records based on configurable incident count and severity thresholds.
...
-   Added support for the following models for all AI features: Google Gemini 3.5.
-   FlashOpenAI GPT 5.1.
-   OpenAI GPT 5.4 Mini.
...
-   MarkdownToHtml incorrectly truncated URLs containing balanced parentheses - for example, SharePoint URLs containing \(SRM FORM\) - causing the href attribute to cut off at the first closing parenthesis and leaving the remainder of the URL as plain text in Now Assist synthesized answer rendering.
...
-   New: Create and document governance, risk, and compliance issues with guided assistance from the employee center.
...
-   New: Signature workflows on the Docusign envelope now support adding signatories with different roles.
...
-   Fixed: Selecting 'Assign to me' button on legal requests displays error message when a request is already assigned.
...
-   Fixed: Resolved incorrect sorting of creator skill names in the metrics dashboard. Non-numeric skill columns now sort alphabetically by name for easier identification and analysis.
