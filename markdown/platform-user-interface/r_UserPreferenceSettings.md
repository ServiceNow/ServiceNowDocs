...
Indicates whether the JavaScript editor and formatting controls are enabled or disabled for the user. The JavaScript editor icon \(![JavaScript editor icon](../image/jscript-icon.png)\) and formatting controls are available only in instances where the Syntax Editor plugin is active. This record updates automatically when a user clicks the icon to enable or disable script editing. For more information, see JavaScript syntax editor.**True** = enable script editor.
