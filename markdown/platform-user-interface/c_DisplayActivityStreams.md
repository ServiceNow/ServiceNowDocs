...
The activity stream information appears in a flyout window, and is the same information that appears in the [activity formatter](https://www.servicenow.com/docs/access?context=c_ActivityFormatter&version=zurich&pubname=zurich-platform-administration&ft:locale=en-US) for a record. The short description appears as the title for the entry. Activity stream items on the task table use the task number as the title. The information in the flyout window updates automatically with audit and journal entries. Click the **&gt;&gt;** icon at the top to close the activity stream.
