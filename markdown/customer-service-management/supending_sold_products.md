---
title: Suspending sold products
...
Suspend single or multiple root-sold products, and create combined suspension and resumption orders of products and services at the same time to improve agent efficiency.

|Task|Description|
|----|-----------|
|Suspend a single sold product|[Suspend a single sold product](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/customer-service-management/supending_sold_products.md)|
|Suspend multiple sold products|[Suspend multiple sold products](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/customer-service-management/supending_sold_products.md)|
...
4.  In the Sold Products related list, select the root sold product that you want to suspend.

    **Note:** You can do a **Suspend** action on the sold product only if it is in the **Active** state. The **Suspend** option is disabled if you select a child sold product.
...
An order with the suspend action is created for the suspend order line items. If you specify an end date and time, a resume order line item is created as a part of the same order. When you update the suspend order line item to **Complete**, the sold product state automatically updates to **Suspended**. When you move the state of the resume order line item to **Complete**, the sold product state updates to the **Active** state.
...
4.  In the Sold Products related list, select the sold products that you want to suspend.
    ...
    **Note:** You can do a **Suspend** action only on root sold products with a product offering and if it is in the **Active** state. The **Suspend** option is inactive if you select a child sold product.
...
6.  In the Suspend sold products window, in the **Start date and time** field, enter the start date and time that you want to start the suspension.
    ...
    You can see the selected sold products in the account.
...
An order with the suspend action is created. Suspend order line items are created in an synchronized process. If you specify an end date and time, a resume order line item is also created as a part of the same order.
