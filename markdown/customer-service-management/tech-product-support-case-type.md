...
breadcrumb: [Technology Product Support Case application, Customer service case types, Case management, Organize agent workspaces, Configuring Customer Service Management, Customer Service Management]
---
...
State flows are customized transitions from a state to another, such as when an agent moves a technology product support case from Work in Progress to Awaiting Info.
...
The Technology Product Support Case application includes list views for CSM/FSM Configurable Workspace and for Core UI that appears in the application navigator.
...
Service definitions enable customers to detect and request the services that they need and agents to create cases of the appropriate type to support those requests.
...
A record producer is a specific type of catalog item that enables end users to create task-based records from a service catalog. The Technology Product Support Case application includes the following record producer: **Create a Technology Product Case**.
...
The case action status feature enables agents to detect cases that need attention.
...
Cases in the Resolved state can be closed automatically if customers don’t act.

</td></tr></tbody>
</table>## Roles

User roles available with the Technology Product Support Case application address the diverse skill sets needed to resolve complex technical issues. These roles enable you to research technical issues, collaborate with development teams, and implement advanced troubleshooting techniques.
...
A case that is being created but isn’t yet submitted.
...
A case moves from Draft to New when it’s submitted or when the case intake is completed.
...
The customer accepts the proposed solution or the agent selects the **Close case** action. The case can also be auto- closed if there’s no response from the customer within a specified about of time.The case record is read only when the state is Closed.
...
A case that has been canceled by a user.
...
|The case is canceled|Tech Case canceled notification|
...
Select a product from the list of available products. This list includes the products sold for the logged-in user.
...
The auto- Close Resolved Cases feature is available with the technology product support case type. Cases in the Resolved state can be closed automatically if customers don’t act.

**Note:** This feature is active by default.

A case can remain in the Resolved state for 10 days before it’s automatically closed. Administrators can change this default value in the **Auto Close Resolved Cases** Flow Designer flow.

Customers receive the following notifications before a case is automatically closed:

-   First notification: sent after three days
