...
Gauge the timing and regularity of implementation of a schedule for a work plan. Planned work schedules have triggers like the duration, meter, condition, and script to determine and ensure that the work plan milestones are met. To create a planned work schedule, see Configure a work schedule.
...
Describe the planned maintenance activities that must be performed for an install base item and the order in which they must occur. To create a planned work schedule template, see Associate a work order template to a work schedule.
...
Work orders and work order tasks are related and created only if there's an existing work order for the install base item. Fill in the **Parent** field on the work order task form with the name of the work order. To create a work order task, see Create a work order task.
