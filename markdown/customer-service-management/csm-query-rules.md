...
This change applies to Query business rules \(QBRs\) and read ACLs on CSM related tables. Each data table that used QBRs before Quebec now has a new QBR that uses the new logic. Tables with read ACLs that used filters from the CSQueryBRUtilOOBConstants script include now have one more read ACL that uses the filters from the Query Rule table.
