...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/customer-service-management/csm-cases-case-tasks-overview.html
release: xanadu
topic_type: concept
...
Customer service agents create cases to identify questions or issues, gather information, and track the activities related to resolution. Agents also uses cases to communicate with customers and other users.
...
-   [Customer service case form](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/customer-service-management/r_CustomerServiceCaseForm.md)

## Case tasks

Case tasks are created for additional work that needs to be completed as part of resolving a customer service case. These tasks are frequently assigned to users such as middle or back office agents. Case tasks are stored in the Case Task \[sn\_customerservice\_task\] table.
...
**Note:** Case tasks can also be created from other records. If a task is created from a non-case record, such as an incident, problem, change, or request, the **Parent** field is updated with the number of the parent record. The **Account**, **Contact**, or **Consumer** fields are not updated.
...
-   [Case task form](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/customer-service-management/csm-case-task-form.md)
...
-   If yes, the related party is not removed from the case and the system displays a message that lists the associated case tasks.

## Case task integration with Service Definitions

The [service definitions](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/customer-service-management/csm-service-definitions.md) feature enables you to create connections between products, services, and case types.
...
This user can work on case tasks that they have created and that are assigned to them or to their assignment groups.Case task agents have read access to the **Account**, **Contact**, **Consumer**, and **Parent case** fields for case tasks that are assigned to them or to their assignment groups.
...
End users and requestors can access case tasks from the Customer and Consumer Service Portals by selecting **My Lists** &gt; **My Case Tasks**.

End users and requestors can view and update case tasks and the fields that they have access to \(case tasks that have the **Visible to customer** field enabled\). These users have read access to the **Parent case**, **Account**, **Contact** and **Consumer** fields.
...
These users can see the following fields on the Case Task lists and forms: **Account**, **Contact** and **Consumer**. If an end user is viewing a case task from a list of case tasks, the Case Task form also displays the **Parent case** field.

## Scheduled job for case tasks

Case task enhancements in the Xanadu release add several fields to the case task record. After upgrading to the Xanadu release, you can run a scheduled job that populates these fields for active case tasks.
