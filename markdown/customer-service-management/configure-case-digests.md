...
-   **Post case review**: when the work on a case has been completed, agents can a post case review document that captures details about the issue, such as the root cause, mitigation plan, and preventive actions.
...
Assignment rules automatically assign tasks to users and groups when the rule conditions are met. Two rules are provided with the case digests feature that assign the post case review and case action summary records to the case agent:-   Auto assign the CAS to case agent
