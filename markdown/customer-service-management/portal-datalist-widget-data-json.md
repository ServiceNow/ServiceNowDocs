...
The name of the view that defines the list of columns or fields to appear on the widget.For more information on these views, see [Create and delete views](https://www.servicenow.com/docs/access?context=create-delete-view&version=xanadu&pubname=xanadu-platform-user-interface&ft:locale=en-US).
...
The portal page that displays the details of any record accessed in the widget.This parameter is optional.

By default, this field is set to the Case \(csm\_ticket\) page.

</td></tr><tr><td>

pre\_defined\_filters

</td><td>

Display cases which are in New, Closed, Open, or Awaiting Info state in any selected category. These filters appear as tabs in the widget. Only Open and Closed case are included by default in the JSON code. You can change the names of these filters by changing the value of **filter\_name**.This parameter is optional.
...
Defines the values from the defined table for the column parameter. The available values are:-   Label: Name of a value
...
Used to filter the records from the specified table using a query defined in label and query parameters. Ranges to define theFor example, you can display case records that have been updated in the last month, last 3 months, or last 6 months.The range configuration is:
