...
-   The Customer Service NLU Model for Virtual Agent Conversations \(com.sn\_csm.nlu\) plugin is automatically enabled when you enable the Customer Service Virtual Agent Conversations plugin \(com.sn\_csm.virtualagent\).
    ...
    -   GetHelpWithOrder
    -   SubmitRequest
    Each intent corresponds to a [chatbot topic](../task/csm-virtual-agent-conversation.md). To customize an intent, you can import an existing intent from the Customer Service NLU for Virtual Agent model and then link the NLU model with the associated intent for the topic.
