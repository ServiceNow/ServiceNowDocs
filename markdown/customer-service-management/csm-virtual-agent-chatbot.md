...
-   [Publish](https://www.servicenow.com/docs/access?context=publish-virtual-agent-topic&version=xanadu&pubname=xanadu-conversational-interfaces&ft:locale=en-US) the following read-only pre-defined virtual agent chatbot topics and topic blocks:
...
-   The Customer Service NLU Model for Virtual Agent Conversations \(com.sn\_csm.nlu\) plugin is automatically enabled when you enable the Customer Service Virtual Agent Conversations plugin \(com.sn\_csm.virtualagent\).
    ...
    -   GetHelpWithOrder
    -   SubmitRequest
    Each intent corresponds to a [chatbot topic](../task/csm-virtual-agent-conversation.md). To customize an intent, you can [import](https://www.servicenow.com/docs/access?context=import-nlu-intent&version=xanadu&pubname=xanadu-intelligent-experiences&ft:locale=en-US) an existing intent from the Customer Service NLU for Virtual Agent model and then [link the NLU model with the associated intent for the topic](https://www.servicenow.com/docs/access?context=va-NLU&version=xanadu&pubname=xanadu-conversational-interfaces&ft:locale=en-US).

-   Enable ServiceNow NLU in the [general settings](https://www.servicenow.com/docs/access?context=va-conversation-settings&version=xanadu&pubname=xanadu-conversational-interfaces&ft:locale=en-US).
