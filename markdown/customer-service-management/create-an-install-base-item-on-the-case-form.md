...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/customer-service-management/create-an-install-base-item-on-the-case-form.html
release: xanadu
topic_type: task
...
4.  Perform one of the following actions before selecting an install base item on the Create Case form.
    ...
    For a description of the field values, see [Case task form](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/customer-service-management/csm-case-task-form.md).

    |Field|Action|
    |-----|------|
    |**Account**|Select an account that has to be associated to the install base.|
    |**Consumer**|Select a consumer that has to be associated to the install base.|
