---
title: Configure activity types for the Customer Activity view
description: Create an activity type to display in the activity feed on the Customer Activity view.
locale: en-US
...
Create an activity type to display in the activity feed on the Customer Activity view.
...
Activity types are displayed in the Customer Activity view in Agent Workspace.

\[Omitted image "activity-type.png"\] Alt text: Agent Workspace displaying activity types in Customer Activity view. In this example, cases are highlighted on the Customer Activity tab.

## Procedure

1.  Navigate to **All** &gt; **Customer Central** &gt; **Feed Configuration** &gt; **Activity Types**.
...
5.  In the notification message, select the link to the business rule.
    ...
    **Note:** The business rule is required to create a record in a central activity table each time the action is performed. The central activity table improves the performance of rendering the feed during runtime. Create a business rule for each activity type.
