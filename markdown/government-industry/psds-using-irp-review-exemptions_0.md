---
title: Review a case for exemptions using Information Request Playbook
description: Review a case for exemptions.
locale: en-US
release: zurich
topic_type: task
last_updated: "2026-03-30"
reading_time_minutes: 1
breadcrumb: [Review request details, Information Request Playbook, Playbooks and solutions, Use, Public Sector Digital Services \(PSDS\)]
---

# Review a case for exemptions using Information Request Playbook

Review a case for exemptions.

## About this task

Several exemptions may apply to an information request case. .

## Before you begin

Role required: admin

## Procedure

1.  Select any exemptions that apply to the case, and select **Mark as Complete**.

2.  If no exemptions apply to the case, select **No exemption code\(s\) applicable**, then select **Mark as Complete**.


