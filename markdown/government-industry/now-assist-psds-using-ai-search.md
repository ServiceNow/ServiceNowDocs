...
For more information on search application configurations, see [Search application configurations](https://www.servicenow.com/docs/access?context=defining-search-app-cfgs-ais&version=zurich&pubname=zurich-platform-administration&ft:locale=en-US). For more information on how to set up and customize AI Search for use with Now Assist for Public Sector Digital Services \(PSDS\), see [Configure AI search for Now Assist for Public Sector Digital Services \(PSDS\)](now-assist-psds-configuring-ai-search-toggle.md).
