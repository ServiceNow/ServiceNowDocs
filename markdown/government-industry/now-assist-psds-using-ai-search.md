...
For more information on search application configurations, see [Search application configurations](https://www.servicenow.com/docs/access?context=defining-search-app-cfgs-ais&version=xanadu&pubname=xanadu-platform-administration&ft:locale=en-US). For more information on how to set up and customize AI Search for use with Now Assist for Public Sector Digital Services \(PSDS\), see [Enabling and configuring AI Search with Now Assist for PSDS](now-assist-psds-enabling-configuring-ai-search.md).
