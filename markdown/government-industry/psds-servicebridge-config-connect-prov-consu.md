...
A provider record must have been created. See [Set up a Service Exchange provider record](https://www.servicenow.com/docs/access?context=service-bridge-v2-new-provider&version=yokohama&pubname=yokohama-service-bridge&ft:locale=en-US).
...
3.  Select **Save**.
    ...
    An email is generated and sent to the Consumer Contact specified during registration if email sending is enabled. If not, the admin must either copy the link from the work notes and manually send it to the consumer admin. The Consumer Contact must follow the steps listed in the [Registering with a provider](https://www.servicenow.com/docs/access?context=service-bridge-v2-register&version=yokohama&pubname=yokohama-service-bridge&ft:locale=en-US) to complete the registration process on the consumer instance.
