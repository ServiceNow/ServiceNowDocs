...
**Note:** To use the Mobile Agent, the required plugins must be enabled by an administrator. If you're an administrator, see [Configure Mobile Agent for Service Request Playbook](psds-configure-mobile-agent-for-service-request-playbook.md) to get the information on configuring the Mobile Agent. For instructions on how to get started with the app after an administrator configures these settings, see [Get started with the Customer Service mobile application](https://www.servicenow.com/docs/access?context=install-customer-service-mobile-app&version=yokohama&pubname=yokohama-customer-service-management&ft:locale=en-US).
