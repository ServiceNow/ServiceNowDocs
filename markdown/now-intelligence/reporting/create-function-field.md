...
2.  Select the report that you want to add a function field to or create a report to configure a function field on.

    When you configure a function field on a table, it is available anywhere you use the table on the ServiceNow AI Platform, including Platform Analytics experience visualizations on the same table. For example, a function field that calculates the age of open incidents is available for all reports and data visualizations on the incident table.

3.  Select **Edit** to open the Report Designer.

4.  Open the **Configure** tab and select **Configure function field**.

5.  Enter the field name in the search bar to see if someone has already created the function field.
...
9.  Select the Operation.

    The **Syntax** text box shows the name of the function preceded by `glidefunction:`.

    \[Omitted image "create-ff-01.png"\] Alt text: Create function field window showing Label, Return type, and Add Operation. The subtract operation is highlighted.
...
11. When the syntax is correct, you can save the function field.
    ...
    \[Omitted image "create-ff-02.png"\] Alt text: Create function field window with the Add field option and the Syntax is correct message. The Child incidents field is highlighted.
