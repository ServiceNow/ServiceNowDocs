...
Unshared reports don’t appear in lists of reports a user has access to or in other options where a report can be opened or selected. When users can see a dashboard the report has been added to, but not the report itself, a message indicates that the the report has limited visibility.
...
The ACL is considered for each list row that might be displayed in a report. For more information, see [Report\_view access control](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/now-intelligence/reporting/report-view-access-control.md).
...
On upgrade to Brazil, all public Core UI reports are unpublished to enhance security awareness and control. The glide.report.published\_reports.enabled property on all Core UI reports is set to false. Users with the report\_admin role can navigate to the sys\_report\_public table to view unpublished reports or enable sharing with logged in users inside the platform. Only customer support can enable the public capacity. For more information, see .
