...
Additional steps may be necessary if you are moving a dashboard on a scoped application. For more information, see [Update set id 'global' is different than update set scope id](../../dashboards/concept/troubleshooting-dashboard-update-set.md#).
...
3.  Verify that there’s a portal page record associated with each tab.
    ...
    1.  Select the context menu icon \(![](../../dashboards/image/icon-context-p.png)\) and select **Launch Dependency Assessment**.

        ![Dependency assessment example highlighting dashboard and dashboard tab tiles](../../dashboards/image/move-dp-update-da-ex.png)

    2.  Point to the corner of each dashboard tab's tile and select the info icon \(![Admin console tree view info button](../image/admin-console-treeview-info.png)\) to see the associated portal page. ![Dependency assessment example with portal page highlighted](../../dashboards/image/dependency-assess-portal-page.png)

4.  Point to the corner of the dashboard's tile and select the info icon \(![Admin console tree view info button](../image/admin-console-treeview-info.png)\).

    Select **Open Record** to open the dashboard form.

    The dashboard's tile has the same name as the dashboard itself.

5.  Select the context menu icon \(![](../../dashboards/image/icon-context-p.png)\) and select **Unload Dashboard**.
...
8.  On the target instance, make sure that all the tabs of the dashboard have associated portal pages.
    ...
    **Note:** If you can’t find the associated portal page in the target instance, follow these steps to move each missing portal page through the update:
    ...
    2.  Filter the list to show only the portal page with the sys\_id you copied when following the procedure in [Validate that tabs are moved to a target dashboard](../../dashboards/task/validate-dashboard-tabs-moved.md).
    3.  Right-click the record and select **Unload Portal Page**.

        ![Portal page record context menu with the item Unload portal page selected.](../image/unload-portal-page.png)

        The portal page for the current tab is added to the current update set.

    4.  Move the update set to another instance using standard update set functionality. For more information, see [Retrieve an update set](https://www.servicenow.com/docs/access?context=t_RetrieveAnUpdateSet&version=xanadu&pubname=xanadu-application-development&ft:locale=en-US).
