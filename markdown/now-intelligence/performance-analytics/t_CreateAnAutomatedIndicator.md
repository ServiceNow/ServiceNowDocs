...
**Tip:** Generally avoid collecting records with aggregates other than Count and Sum. Users rarely want to see the records that go into a Min or Max, and the number of records in a Count Distinct may differ from the score. Consider collecting records for indicators with these other aggregates only if a user asks for them.
 ...
 You must enable this option for the main indicator of a Spotlight group. Otherwise, the Spotlight group cannot evaluate a snapshot of collected records. For more information, see [Evaluating a snapshot or platform data](create-spotlight-group.md#).
...
A check box to indicate if the value should be aggregated based on a script. This option is available only if **Aggregate** is not set to **Count**. Clear the **Scripted** check box to aggregate the values in a field.
...
The field to perform the aggregate operation on. This field appears only if **Aggregate** is not **Count** and **Scripted** is not selected.
...
</table>    For example, the indicator Summed age of open incidents uses the Sum aggregate. Because it uses this aggregate, it must have either a field or a script specified. In this case, it is scripted and uses the Incident.Age.Hours script.

    ![The Incident.Age.Hours script on the Summed age of open incidents indicator](../image/auto-ind-scripted.png)

9.  Expand the context menu and click **Save**.
...
1.  If the indicator source for this indicator uses a business or fiscal calendar, and this indicator is the first one created that uses that calendar, you are prompted to set the periodicity.
...
2.  In the **Additional Conditions** tab, add conditions to limit the set of records that the indicator evaluates.
    ...
    **Warning:** To avoid a [data collection job](../concept/performance-analytics-glossary.md#) completing with errors, follow these limitations:
...
The process for generating a fiscal calendar automatically creates a fiscal calendar group. For business calendars, the creator of the business calendar must also create the business calendar group, manually. If you have any questions, contact the admin responsible for creating business calendars. For more information, see [Create a Business Calendar Group](https://www.servicenow.com/docs/access?context=create-business-calendar-group&version=xanadu&pubname=xanadu-platform-administration&ft:locale=en-US).
