...
3.  In the **Group** field, use the lookup icon \(![](../image/icon-search.png)\) to select a group to add the dashboard to.
...
4.  Open the form of the dashboard group to modify its permissions.
    ...
    Dashboard groups use standard platform permissions. For more information, see [Access control list rules](https://www.servicenow.com/docs/access?context=access-control-rules&version=xanadu&pubname=xanadu-platform-security&ft:locale=en-US).
