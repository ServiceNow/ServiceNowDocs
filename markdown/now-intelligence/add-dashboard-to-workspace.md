...
3.  Create a page in the workspace using the Dashboards page template, or open an existing page created from the template.
    ...
    For more information, see [Create a page from a template](https://www.servicenow.com/docs/access?context=reuse-page-definitions&version=xanadu&pubname=xanadu-application-development&ft:locale=en-US) and [Add Platform Analytics pages to a configurable workspace](add-analytics-center-to-experience.md#).
...
8.  Set the other configuration options as they are described in the [Dashboard component reference on the Developer site](https://developer.servicenow.com/dev.do#!/reference/next-experience/xanadu/now-components/sn-dashboard/overview).
