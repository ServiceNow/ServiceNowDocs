...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/now-intelligence/process-mining/set-activity-def.html
release: yokohama
...
5.  Select **New** in the Activities area.
    ...
    \[Omitted image "set-activity-def.png"\] Alt text: Set activity definition
...
These recommendations come from the process configuration set for the table on which the project is based. They are derived from the Process perspectives section of the Process details page in the Process Configuration Builder.

For more information on Process perspectives, see [Configure process details](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/now-intelligence/process-mining/process-details.md).

The recommendation is provided even if you do not specify anything in the process configuration for the table. If nothing is configured on the Process details page, we recommend the fields that have high frequency of change.
...
\[Omitted image "act-def-recom.gif"\] Alt text: Recommendations in activity definition based on the setting in process configuration
...
This field is available only if you choose **Field Values \(Default\)** or **Date Time** in the **Select type** field.If you want to see the list of related fields, select the Related Fields icon \(\[Omitted image "act-def-rel-list.png"\] Alt text: Related list\), and select a field.
...
This field is not available if you select **Contextual activity**.The value in this field decides which events must be displayed first on the process graph when multiple activity definitions have the same timestamp.
...
\[Omitted image "ad-listofvalues.png"\] Alt text: List of values
...
-   **Group unique values and display as a single activity step**: If you select this option, the process map reflects the group of changes for the activity as one change item.
 ...
 \[Omitted image "dont-show-changes-group.png"\] Alt text: Changes are not grouped
...
</table>    **Note:** To edit an activity definition, select the activity definition from the list and make edits. To delete an activity definition, select the activity definition from the list and select the Delete icon \(\[Omitted image "act-def-delete.png"\] Alt text: Delete icon\).
...
9.  Select improvement opportunities to set improvement opportunities.
    ...
    For more information, see [Set improvement opportunities](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/now-intelligence/process-mining/improve-opportunities.md)


**Parent Topic:**[Scoping your analysis](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/now-intelligence/process-mining/scope-analysis.md)
