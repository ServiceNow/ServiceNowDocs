---
title: Run Process Mining evaluation project
description: Run Process Mining evaluation project to familiarize with improving your process with Process Mining capability.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/now-intelligence/process-mining/run-evaluation-project.html
release: yokohama
product: Process Mining
classification: process-mining
topic_type: task
last_updated: "2025-01-30"
reading_time_minutes: 1
breadcrumb: [Evaluating Process Mining, Exploring Process Mining, Process Mining, Platform Analytics]
---

# Run Process Mining evaluation project

Run Process Mining evaluation project to familiarize with improving your process with Process Mining capability.

## Before you begin

Role required: None

The only criteria to run a Process Mining evaluation project is to have access to the table. You can run the evaluation project even without access to the table, however, then it would be an empty project.

You can run an evaluation project from Platform Analytics or Process Mining Workspace.

## About this task

You can run an evaluation project from Platform Analytics or Process Mining Workspace.

## Procedure

1.  To run an evaluation project from Platform Analytics:

    1.  Navigate to **All** &gt; **Platform Analytics** &gt; **KPIs**.

    2.  Select a KPI.

    3.  Select **Run process analysis**.

        If the KPI is related to ITSM, CSM, or HR, then the sample mining starts.

        You will be able to view the records of the last 7 days and a maximum of 3600 records.

        **Note:** To know more about the evaluation projects available in Process Mining, see [Process Mining evaluation project for Incident Management](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/now-intelligence/process-mining/evaluation-pm-inci-manag.md), [Process Mining evaluation project for Customer Service Management](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/now-intelligence/process-mining/evaluation-pm-csm.md), and [Process Mining evaluation project for Human Resources](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/now-intelligence/process-mining/evaluation-pm-hr.md).

2.  To run an evaluation project from Process Mining Workspace:

    1.  Navigate to **Workspaces** &gt; **Process Mining Workspace**.

    2.  From the details button for the evaluation project, select **Run sample mining**.


**Parent Topic:**[Evaluating Process Mining](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/now-intelligence/process-mining/evaluate-pm.md)

