...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/now-intelligence/process-mining/generate-process-map.html
release: xanadu
...
Process Mining extracts a maximum limit of 500,000 records for a single table. If that limit is reached with the table you selected, add additional filters to refine the data set. This limit will decrease depending on the number of tables you have configured on your project. For example, if you configure two tables, a parent table \(like incident\) and a child table \(like task sla\), the limit calculated on the incident table will be 250,000 records.
...
-   You can attempt a data refresh from the Analyst Workbench on a project that has been successfully mined from its workflow record. An error icon \[Omitted image "extract-error-icon.png"\] Alt text: displays when project data does not refresh successfully. To investigate for details, view the **Extract Data Log** tab from the Project Definition form.
