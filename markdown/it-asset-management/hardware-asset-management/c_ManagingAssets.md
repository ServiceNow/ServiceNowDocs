...
## Asset and CI synchronization through MAC address

Synchronization between asset and CI can be achieved using the **MAC address** \(mac\_addr\) field in the Asset \[alm\_asset\] table in addition to the Serial number. A CI can have multiple MAC addresses. The Network Adapter \[cmdb\_ci\_network\_adapter\] table stores all the MAC addresses and also has a reference field to the CI \[cmdb\_ci\] table. Therefore, the relationship between MAC address and CI is maintained.
...
-   **glide.create\_alm\_asset.async**
    ...
    **Note:** If you upgrade to Yokohama from Orlando or earlier, the default value of this property is **false**. Before updating this property, review your processes that depend on an asset value present on a CI and make any necessary changes to account for delayed asset creation.
...
-   **glide.asset.create\_ci\_with\_ire**
    ...
    **Note:** This property is applicable to CI classes that are extended from the hardware CI class \[cmdb\_ci\_hardware\].
    ...
    The default value of this property is false if you upgrade to Yokohama from Orlando or earlier. Before updating this property, review the customizations on the Serial number field and any integrations or flows that use an existing serial number to create an asset.
