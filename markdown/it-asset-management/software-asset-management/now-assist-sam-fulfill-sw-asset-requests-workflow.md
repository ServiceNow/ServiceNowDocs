...
3.  Assign Microsoft 365 licenses based on availability:
    ...
    2.  If the licenses aren't available, the AI agent checks for available entitlements.
        -   If the entitlement is available, the AI agent creates a reservation order using the existing entitlement. It generates a catalog task to replicate that reservation within the Microsoft 365 Admin Center.
