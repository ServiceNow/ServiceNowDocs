...
-   Activate the File-based Discovery \(com.snc.discovery.file\_based\_discovery\) plugin on your ServiceNow instance. See File-based Discovery for more information on file-based Discovery.
...
-   If you want to track and manage IBM licensing in public cloud environments, configure one or more for the following services to automatically scan for and update the CMDB with cloud infrastructure changes every 30 minutes:
    -   For public clouds hosted by AWS, configure the AWS Config service. See Configure the AWS Config service to send event notifications to the ServiceNow instance for detailed instructions.
    -   For public clouds hosted by Microsoft Azure, configure the Microsoft Azure Alert service. See Configure the Microsoft Azure Alert service to auto-update the CMDB for detailed instructions.
    -   For public clouds hosted by Google Cloud Platform \(GCP\), configure the Google Cloud Logging service. See Configure the Google Cloud Logging service to auto-update the CMDB for detailed instructions.
