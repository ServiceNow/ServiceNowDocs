...
If you haven't used task rate cards, then expense lines do not get created. When you upgrade to Zurich, you can automatically create expense lines in bulk for all your closed tasks. This is only a one time process that runs on active task rate cards with a populated task rate value. Using time worked will not result in creation of expense lines if the time worked records are not present for tasks.
