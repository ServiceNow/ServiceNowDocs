...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/order-management/common\_salesforce\_integration\_issues.html
release: brazil
topic_type: concept
...
Another worthwhile step when investigating any integration issue is to confirm that the steps in the [Set up environments linked to Salesforce orgs](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/order-management/installation-and-setup-guide-for-environments-linked-to-salesforce-org.md) were correctly completed.

**Note:** If partner users are experiencing any of these issues, or you utilize RFQ APIs, first see the [Partner users/RFQ API sharing settings](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/order-management/common_salesforce_integration_issues.md) section at the end of this topic.

## Salesforce: “SBQQ: Too many SOQL queries: 101”

\[Omitted image "cpq-error-SBQQ-too-many-SOQL-queries.png"\] Alt text: Salesforce issues
...
\[Omitted image "cpq-additional-settings-enable-large-configurations.png"\] Alt text: Salesforce issues
...
\[Omitted image "cpq-error-attempt-to-de-reference-a-null-object.png"\] Alt text: Salesforce issues
...
\[Omitted image "cpq-pricebook-entry-missing.png"\] Alt text: Salesforce issues

\[Omitted image "cpq-pricebook-entry-inactive.png"\] Alt text: Salesforce issues
...
\[Omitted image "cpq-additional-settings-nested-bundles-for-external-configurator.png"\] Alt text: Salesforce issues
...
\[Omitted image "cpq-logik-admin-settings-push-settings.png"\] Alt text: Salesforce issues
...
-   Check the permissions of the integration user against the list in [Required permissions for the integration user](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/order-management/integration_user_required_permissions.md).

## Saving does not create child quote lines

\[Omitted image "cpq-shopping-cart-product-logik-environment.png"\] Alt text: Salesforce issues

CPQ BOM has the product CPQ environment.

\[Omitted image "cpq-edit-quote-product-logik-environment-configurator.png"\] Alt text: Salesforce issues
...
-   Confirm that the BOM items have the correct BOM types as defined in CPQ Admin:
    ...
    \[Omitted image "cpq-logik-admin-settings-bom-types.png"\] Alt text: Salesforce issues
...
-   Check whether the parent configurable product has the related Dynamic feature defined:
    ...
    \[Omitted image "cpq-logik-environment-configurator-dynamic.png"\] Alt text: Salesforce issues

    If so, also confirm that the option selection method is also dynamic:

    \[Omitted image "cpq-edit-dynamic-option-selection-method.png"\] Alt text: Salesforce issues
...
\[Omitted image "cpq-settings-editor-plugins-quote-calculator-plugin.png"\] Alt text: Salesforce issues
...
\[Omitted image "cpq-error-JSON-string-exceeds-heap-size-limit.png"\] Alt text: Salesforce issues
...
\[Omitted image "cpq-error-contact-blocked.png"\] Alt text: Salesforce issues

Solution:

-   If this is happening for only one user:
    -   Confirm that they have Admin access. See [User access](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown).
    -   Check whether they are running into browser issues. See [Common browser issues while using CPQ](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/order-management/common_browser_issues_while_using_logik_io.md).
-   If this is happening for a group of users:
    -   Confirm that their profile and permission group have the proper SFDC permissions. See [What to do if receiving an Insufficient Privileges or blank screen when launching a configuration](https://customer.help.logik.io/servicedesk/customer/portal/4/article/1616314402).
    -   Confirm that CPQ has been added to Trusted URLs in the Setup section, with iFrame enabled. While this isn't necessary in all cases, we've seen this addition help organizations with CPQ users from multiple different countries, or if they are using Subscription Manager/RLM.
        ...
        \[Omitted image "cpq-setup-trusted-URLs.png"\] Alt text: Trusted URLs screen

    -   If this is happening for every user, check whether the URL is correctly entered in Custom Settings.

        \[Omitted image "cpq-salesforce-custom-settings.png"\] Alt text: Custom settings
...
\[Omitted image "cpq-salesforce-insufficient-privileges.png"\] Alt text: Information message
...
-   If this is happening only for one Blueprint or when launching from one quote, check whether there are any errors in the Logs tab of the CPQ Admin.
    ...
    If you find no errors in the logs, review [Common error messages and causes](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/order-management/common_error_messages_and_causes.md). This may be due to an issue with the On Configure/Reconfigure script of the Blueprint, or with a twinned field that is expected from the quote.

-   If this is happening for only one user, check whether they are experiencing browser issues. See [Common browser issues while using CPQ](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/order-management/common_browser_issues_while_using_logik_io.md).
-   If this is happening for a group of users, check to see whether their profile and permission group have the proper SFDC permissions. See [What to do if receiving an Insufficient Privileges or blank screen when launching a configuration](https://customer.help.logik.io/servicedesk/customer/portal/4/article/1616314402) .
-   If this is happening for every user, confirm that the URL is correct in Salesforce CPQ. Navigate to Setup &gt; Installed packages &gt; Salesforce CPQ &gt; Configure &gt; Additional Settings &gt; External Configurator URL &amp; Third Party Configurator.
    ...
    \[Omitted image "cpq-salesforce-external-configuration-url.png"\] Alt text: External configurator

    This is usually one of the first steps done when integrating a CPQ environment with an SFDC org. The URL will usually take the form of `https://<domain-name>.<sector>.ServiceNow CPQ.io/ui/configure`, but it can vary based on whether you use VisualForce pages to launch the configurator.
...
\[Omitted image "cpq-salesforce-sharing-settings.png"\] Alt text: Sharing settings
...
This error can occur when you try to uncheck the **LGK\_\_IsConfigurable\_\_c** field on a Product2 record. To resolve the issue, from the Salesforce app launcher, visit the ServiceNow CPQ.io Admin Custom Settings page and enable the **Skip Configurable Product Setup Trigger** setting.
