...
In order to display ServiceNow CPQ configuration data in out-of-the-box Salesforce CPQ quote documents, you must first map the data from your Configuration Line Itemsʼ extended info to custom fields on their associated quote lines. This can be accomplished by using the configuration line item to quote line” flow that is included with ServiceNow CPQ. For more information, see [Use case: Configuration line item to quote line flow](use-case-configuration-line-item-to-quote-line-flow.md)
