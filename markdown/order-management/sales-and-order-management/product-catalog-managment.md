...
-   Product offer eligibility: Filter your product catalogs, product offering categories, and product offerings so that agents sell customers only the products for which they’re eligible. To learn more about setting up product eligibility, see [Configuring product offer eligibility](configuring-sales-eligibility.md).
...
Starting with Product Catalog Management v. 17.0.0, agents and customers can use AI Search queries in the product catalog interface to find relevant product offerings quickly, using more then keyword searches. AI Search supports logical operators and wild-card characters in queries and provides type-ahead search suggestions. For more information on setting up this feature, see [Configuring AI Search for the product catalog](configure-ai-search-prod-catalog.md).
