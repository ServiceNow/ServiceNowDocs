...
Although the ServiceNow CPQ rules engine is engineered to process rules quickly, not all rules perform the same. Therefore, the administrator plays an essential role in optimizing the performance of the ServiceNow CPQ UI. Use the following rules of thumb when you build rules:

-   Simple rules are best. The ServiceNow CPQ rules engine can quickly run tens of thousands of rules whose conditions have defined logical expressions and whose actions are defined without scripting.
