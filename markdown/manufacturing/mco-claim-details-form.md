---
title: Claim details form
description: The Claim details form enables the dealer to enter the claim details for the repair work performed.
locale: en-US
release: australia
topic_type: reference
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Repair form, Reference, Manufacturing Commercial Operations]
---

# Claim details form

The Claim details form enables the dealer to enter the claim details for the repair work performed.

|Field|Description|
|-----|-----------|
|Asset|Asset or product on which repair action is performed.|
|Requested by|User who is raising the repair claim request.|
|Consumer|Customer information that is auto-populated based on the install base.|
|Description|Short description on the repair claim.|
|Dealer|Dealer who is raising the repair claim request.|
|Reported date|Date on which the claim is being raised.|
|Work order|Claim work order number that is a string value.|

**Parent Topic:**[Repair form](repair-form.md)

**Related topics**  


[Roles and components of Partner Relationship Management](https://www.servicenow.com/docs/access?context=roles-and-components-of-partner-relationship-management&version=australia&pubname=australia-order-management&ft:locale=en-US)

[Submit a repair claim for warranty](../task/mco-submit-repair-claim.md)

[Submit a repair claim for recall](../task/mco-submit-repair-claim-for-recall.md)

