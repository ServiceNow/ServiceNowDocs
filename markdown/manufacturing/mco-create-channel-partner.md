---
title: Create channel partner
description: Create a channel partner to streamline and help manage the sales process of a product or a service for an enterprise.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/manufacturing/mco-create-channel-partner.html
release: zurich
topic_type: task
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Set up dealer, Configure, Manufacturing Commercial Operations]
---

# Create channel partner

Create a channel partner to streamline and help manage the sales process of a product or a service for an enterprise.

## Before you begin

Role required: admin

## Procedure

1.  Navigate to **All** &gt; **Workspaces** &gt; **CSM/FSM Configurable Workspace.**

2.  Select the List icon.

3.  Navigate to **Partner Relationship Management** &gt; **Channel Partners**.

4.  Select **New**.

5.  On the Channel partner form, fill in the fields.

    For a description of the field values, see [Channel partner form](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/manufacturing/mco-cp-form.md).

6.  Select **Save**.


**Parent Topic:**[Set up dealer](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/manufacturing/set-up-dealer.md)

**Related topics**  


[Register Partner Staff on workspace](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/order-management/register-partner-staff-on-workspace.md)

[Create Channel Partner record](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/order-management/create-channel-partner-record.md)

