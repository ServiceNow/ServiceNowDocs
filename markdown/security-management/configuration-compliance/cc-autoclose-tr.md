...
breadcrumb: [Automatically closing stale test results in Configuration Compliance, Configuration Compliance remediation, Configuration Compliance, Unified Security Exposure Management, Security Operations]
---
...
3.  Click **Submit**.
    ...
    The scheduled job, **Auto-Close Stale Test Results**, runs daily. This job identifies the TRs based on the conditions that you provided and transitions the ones that match to **Closed-Stale**.

    After TRs are marked as **Closed-Stale**, if the scanner finds these TRs again, the test results are reopened and the states transition to**Open**.

    If a TR is marked as **Closed-Stale**, and the scanner finds that it has been fixed, the TR transitions to**Closed-Fixed**. States also roll up to remediation tasks as described in the following list:
