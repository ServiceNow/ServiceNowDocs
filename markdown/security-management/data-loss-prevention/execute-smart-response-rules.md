...
5.  Click **Submit**.
    ...
    **Note:** You can also configure the approval rule\(s\) for the Response Options by following the procedure explained in the [Create Approval Rules](../../data-loss-prevention/task/configure-approval-rules.md) section. When an approval rule is configured for the Smart Response Rule, the approval flow will be triggered first and after receiving all the approvals, the Smart Response Option flow will be triggered. The state of the DLP Incident will be updated as per the target state of the Response Option mapping after successful execution of the response option.
