---
title: Create security annotations for observables
description: You can select a single or multiple observables and apply security annotations to them using the Actions on selected rows choice menu.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/security-management/create-security-annotations-multiple.html
release: yokohama
topic_type: task
last_updated: "2025-01-30"
reading_time_minutes: 1
breadcrumb: [Security annotations, Security Operations common functionality, Security Operations]
---

# Create security annotations for observables

You can select a single or multiple observables and apply security annotations to them using the **Actions on selected rows** choice menu.

## Before you begin

-   Requires the Threat Intelligence plugin and activation. The Threat Intelligence plugin is available as a separate subscription.

Role required: admin

## Procedure

1.  Navigate to **All** &gt; **Threat Intelligence** &gt; **Observables**

2.  Select one or more observables in the list.

3.  From the **Actions on selected rows** drop-down menu, choose **Add security annotation**.

    \[Omitted image "SecurityAnnotMultiObs.png"\] Alt text: Creating multiple security annotations for observables.

4.  Enter an annotation.

    Annotations can be in any text format.

5.  Click **Submit**.


**Parent Topic:**[Security annotations](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/security-management/security-annotations.md)

**Related topics**  


[Create security annotations for CIs]()

[Create security annotations for users]()

[View security annotations reports]()

