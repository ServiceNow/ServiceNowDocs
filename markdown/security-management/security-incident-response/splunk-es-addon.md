...
Create a **Manual event forwarding** profile to forward events on-demand from your Splunk Enterprise Security console to create a Security Incident Response \(SIR\) on the ServiceNow instance. For more information, see [Create and name an event profile for the Splunk Enterprise Event Ingestion integration](splunk-event-ingest-create-profile.md).
