...
-   **Include in standard model and configuration panel**: when set to view in the standard modal and the configuration panel, it appears in the PAD configuration panel as well as in the properties modal. For more information, see [Activity definitions](https://www.servicenow.com/docs/access?context=activity-definitions&version=xanadu&pubname=xanadu-build-workflows&ft:locale=en-US)
