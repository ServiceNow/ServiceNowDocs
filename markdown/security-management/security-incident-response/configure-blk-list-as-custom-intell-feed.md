...
description: The firewall administrator must configure the Custom Intelligence Feed corresponding to the Block List created in ServiceNow AI Platform.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/security-management/security-incident-response/configure-blk-list-as-custom-intell-feed.html
release: xanadu
...
The firewall administrator must configure the Custom Intelligence Feed corresponding to the Block List created in ServiceNow AI Platform.
...
If Firewall administrator is not using ServiceNow AI Platform, the NOW Security Incident Administrator can provide the required information via an Email. Alternately, if firewall administrator is using ServiceNow AI Platform, the Change Request associated with Block List can be assigned to firewall administrator. This Change Request has the required information to configure Custom Intelligence Feed.
...
4.  Execute the command to add Custom Intelligence Feed.
    ...
    -   When prompted for ServiceNow AI Platform user password with API Access.
