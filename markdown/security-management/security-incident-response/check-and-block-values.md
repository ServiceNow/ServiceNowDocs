...
**Note:** You must manually enter the input variables for this activity and then [publish the workflow](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/build-workflows/legacy-workflow/work-on-workflows.md). If the workflow is not published, the input variables will not be saved for non-admin users.
