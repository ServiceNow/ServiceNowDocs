...
**Note:** You must manually enter the input variables for this activity and then [publish the workflow](https://www.servicenow.com/docs/access?context=work-on-workflows&version=yokohama&pubname=yokohama-build-workflows&ft:locale=en-US). If the workflow is not published, the input variables will not be saved for non-admin users.
