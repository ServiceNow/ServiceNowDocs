---
title: View a Security Incident Response runbook
description: Runbooks give you access to procedures related to tasks you are working on.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/security-management/security-incident-response/view-runbook.html
release: xanadu
product: Security Incident Response
classification: security-incident-response
topic_type: task
last_updated: "2024-08-01"
reading_time_minutes: 1
breadcrumb: [View information in a security incident, Managing security incidents and inbound requests, Security Incident Response, Enterprise security case management applications, Security Operations]
---

# View a Security Incident Response runbook

Runbooks give you access to procedures related to tasks you are working on.

## Before you begin

Role required: sn.si.knowledge\_admin

## Procedure

1.  Navigate to **All** &gt; **Security Incident** &gt; **Manual Runbook** &gt; **View Runbook Documents**.

2.  Select a runbook from the list.

    \[Omitted image "viewRunbook.png"\] Alt text: View runbook

3.  To create a runbook, click **New**.

    For more information, see [Create a runbook](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/security-management/security-incident-response/setup-assistant-reference.md).


