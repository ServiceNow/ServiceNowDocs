...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/security-management/security-incident-response/run-phishing-playbook.html
release: xanadu
...
3.  In the Flow page, click the more icon \[Omitted image "cj-sir-flow-more-icon.png"\] Alt text: More icon, make a copy of the flow and open it for your use.
...
4.  Click the **Trigger** icon.
    ...
    In the first step, you define or set the trigger for the flow. Specify the conditions for the trigger and how often you want the flow to execute the trigger.\[Omitted image "cj-sir-flow-phishing-trigger.png"\] Alt text: Automated phishing playbook flow: trigger
...
5.  Click the **Update Security Incident Record** link.
    ...
    \[Omitted image "cj-sir-flow-phishing-action1.png"\] Alt text: Automated phishing playbook flow: step 1

    The Update Security Incident Record is the first step in the flow. Click the annotation icon \[Omitted image "cj-sir-flow-annotation.png"\] Alt text: Annotation icon to add a note to the security analyst indicating that a phishing incident has occurred, and the automated phishing playbook flow has begun executing.
...
6.  Proceed with the step 2 in the flow, and click the **Create Response Task** link.
    ...
    \[Omitted image "cj-sir-flow-action2.png"\] Alt text: Automated phishing playbook flow: step 2
...
7.  In step 3, the flow collects additional details of the affected user \(typically the user who submitted the incident\) to ensure that it can send a notification successfully.
    ...
    You can specify conditions to check if the affected user's status is active and the user is able to receive notifications.\[Omitted image "cj-sir-flow-phishing-action3.png"\] Alt text: Automated phishing playbook flow: step 3
    ...
    \[Omitted image "cj-sir-flow-phishing-action3-1.png"\] Alt text: Automated phishing playbook flow: step 3.1
...
8.  In step 4, after the email has been sent, the response task is marked as closed.
    ...
    \[Omitted image "cj-sir-flow-phishing-action4.png"\] Alt text: Automated phishing playbook flow: step 4
...
9.  In step 5, all the observables involved in the incident \(such as email subject, email address from which the phishing email was sent, phishing URL\) or observables belonging a selected category \(hash, file, or domain\) are collected to perform additional automated actions in the subsequent playbook steps.
    ...
    \[Omitted image "cj-sir-flow-phishing-action5.png"\] Alt text: Automated phishing playbook flow: step 5

    Click the action designer icon \[Omitted image "cj-sir-flow-actiondesign-icon.png"\] Alt text: Action designer icon to see a detailed view of the action.

    To view the [Action Designer](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/security-management/security-incident-response/action-phishing-playbook.md) page, expand a step in the flow and click the action designer icon.

10. In step 6, an automated response task is created.

    This task captures the beginning of process of getting the reputation of all observables and performing enrichment with configured integrations.\[Omitted image "cj-sir-flow-phishing-action6.png"\] Alt text: Automated phishing playbook flow: step 6
...
11. In step 7, two subflows are called:
    ...
    \[Omitted image "cj-sir-flow-phishing-action7.png"\] Alt text: Automated phishing playbook flow: step 7
    ...
    \[Omitted image "cj-sir-flow-phishing-action7-1.png"\] Alt text: Automated phishing playbook flow: step 7.1.1
...
12. In step 8, after the subflows have been completed, the response task is marked as Closed.
    ...
    \[Omitted image "cj-sir-flow-phishing-action8.png"\] Alt text: Automated phishing playbook flow: step 8

13. In step 9, the Confirm Threat from Observable Triage subflow is called.

    This subflow is used to confirm the presence of a threat indicator in the incident. If the threat is confirmed, an 'IOC Detected' flag is added to the incident.\[Omitted image "cj-sir-flow-phishing-action9.png"\] Alt text: Automated phishing playbook flow: step 9
...
14. When the threat is confirmed, in step 10, you update the security incident and add a note indicating that threat containment tasks will be launched.
    ...
    \[Omitted image "cj-sir-flow-phishing-action10.png"\] Alt text: Automated phishing playbook flow: step 10
...
15. Step 11 is an automated response task that captures the start and completion of the task used to assess the impact of the phishing emails.
    ...
    \[Omitted image "cj-sir-flow-phishing-action11.png"\] Alt text: Automated phishing playbook flow: step 11

16. In step 12, the Assess Phishing Email Impact subflow is called.

    This subflow is used to search for users who have received the phishing email using supported implementations.\[Omitted image "cj-sir-flow-phishing-action12.png"\] Alt text: Automated phishing playbook flow: step 12

17. In step 13, the task is marked as closed to indicate that the Assess Phishing Email Impact subflow has been executed.

    \[Omitted image "cj-sir-flow-phishing-action13.png"\] Alt text: Automated phishing playbook flow: step 13

18. Step 14 is used to retrieve all observables that have been marked as malicious.

    \[Omitted image "cj-sir-flow-phishing-action14.png"\] Alt text: Automated phishing playbook flow: step 14
...
19. Step 15 is an automated response task that captures the start and completion of the sighting search of observables task.
    ...
    \[Omitted image "cj-sir-flow-phishing-action15.png"\] Alt text: Automated phishing playbook flow: step 15

20. In step 16, the Run Sightings Search on Observables subflow is called.

    This subflow performs a sightings search using the configured implementation.\[Omitted image "cj-sir-flow-phishing-action16.png"\] Alt text: Automated phishing playbook flow: step 16
...
21. In step 17, the task is marked as closed to indicate that the Run Sightings Search on Observables subflow has been completed.
    ...
    \[Omitted image "cj-sir-flow-phishing-action17.png"\] Alt text: Automated phishing playbook flow: step 17
...
22. After you have identified the malicious observables, in step 18, you update the security incident record to indicate that the containment actions will now commence.
    ...
    \[Omitted image "cj-sir-flow-phishing-action18.png"\] Alt text: Automated phishing playbook flow: step 18

23. Step 19 is an automated response task that captures the start and completion of the block requests task.

    \[Omitted image "cj-sir-flow-phishing-action19.png"\] Alt text: Automated phishing playbook flow: step 19

24. In step 20, the Create Block Requests subflow is called.

    This subflow is used to block malicious observables.\[Omitted image "cj-sir-flow-phishing-action20.png"\] Alt text: Automated phishing playbook flow: step 20

25. In step 21, the task is marked as closed to indicate that the Create Block Requests subflow has been completed.

    \[Omitted image "cj-sir-flow-phishing-action21.png"\] Alt text: Automated phishing playbook flow: step 21

26. In step 22, the Eradicate Phishing Emails subflow is called.

    This subflow is used to delete phishing emails from user mailboxes.\[Omitted image "cj-sir-flow-phishing-action22.png"\] Alt text: Automated phishing playbook flow: step 22
...
27. After the phishing emails have been deleted, in step 23, you update the security incident record to indicate that the incident status needs to be reviewed.
    ...
    \[Omitted image "cj-sir-flow-phishing-action23.png"\] Alt text: Automated phishing playbook flow: step 23

28. In the last step, the flow creates an automated response task.

    This task is used to send a reminder to the security analyst to save all the incident response actions for future reviews. \[Omitted image "cj-sir-flow-phishing-action24.png"\] Alt text: Automated phishing playbook flow: step 24
...
\[Omitted image "cj-sir-flow-execute.png"\] Alt text: Automated phishing flow: execution

-   **[View automated phishing response playbook flow action designer](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/security-management/security-incident-response/action-phishing-playbook.md)**  
...
-   **[View the automated phishing response playbook subflow designer](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/security-management/security-incident-response/subflow-phishing-playbook.md)**  
...
**Parent Topic:**[Playbook for Automated Phishing](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/security-management/security-incident-response/flow-designer-and-phishing-response.md)
