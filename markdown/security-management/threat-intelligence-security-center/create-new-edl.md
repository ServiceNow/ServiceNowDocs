...
The URL link the Palo Alto Networks firewall administrator uses for configuration in the Palo Alto Networks firewall is automatically generated and displayed.**Note:** To access this EDL URL on the Palo Alto Networks Next-Generation Firewall \(NGFW\), ensure that user credentials are configured with the appropriate role `sn_tisc_pa.api_account_access`.
