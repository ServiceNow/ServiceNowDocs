---
title: ESG Workspace users
description: The ESG Workspace displays the relevant data for the environmental, social, and governance pillars and enables you to take appropriate actions according to your role. These roles include the ESG administrator, ESG program manager, ESG reporting and disclosure manager, ESG metrics manager, and ESG data owner.
locale: en-US
release: xanadu
topic_type: concept
last_updated: "2024-08-01"
reading_time_minutes: 5
breadcrumb: [Explore, Environmental, Social, and Governance Management]
---

# ESG Workspace users

The ESG Workspace displays the relevant data for the environmental, social, and governance pillars and enables you to take appropriate actions according to your role. These roles include the ESG administrator, ESG program manager, ESG reporting and disclosure manager, ESG metrics manager, and ESG data owner.

The ESG Workspace is highly configurable and role-driven. Being role-driven means that the ESG Workspace is customized or unique for each user or role in your organization.

The following table lists the key tasks that each role performs in your organization.

<table id="table_qq2_y2l_hrb"><thead><tr><th>

User

</th><th>

Tasks

</th></tr></thead><tbody><tr><td>

ESG Administrator

</td><td>

Set up the ESG Management application so that it can be used by the users. -   [Create material topics](../task/create-material-topics.md)
-   [Create ESG goals](../task/create-esg-goal.md)
-   [Set targets for goals](../task/create-esg-target.md)
-   [Add a goal to a material topic](../task/associate-a-goal-to-mt.md)
-   [Approve a material topic](../task/approve-a-mt.md)
-   [Manage entities in ESG Management](entities-in-esg.md)
-   [Create a new emission activity](../task/create-an-emission-activity.md)
-   [Create an emission factor](../task/create-an-emission-factor.md)
-   [Manage all integrations](integrate-esg.md)
-   [Create a new schedule for calculating the due date of the metric data task](../task/create-a-new-schedule.md).
-   [Manage metrics](../../metrics/concept/esg-metrics.md).
-   [Create unit family, units, and unit conversions](../task/create-a-unit-family.md).

</td></tr><tr><td>

ESG program manager

</td><td>

-   [Create material topics](../task/create-material-topics.md)
-   [Create ESG goals](../task/create-esg-goal.md)
-   [Set targets for goals](../task/create-esg-target.md)
-   [Add a goal to a material topic](../task/associate-a-goal-to-mt.md)
-   [Approve a material topic](../task/approve-a-mt.md)
-   [Manage entities in ESG Management](entities-in-esg.md)
-   [Manage metrics](../../metrics/concept/esg-metrics.md)

</td></tr><tr><td>

ESG reporting and disclosure manager

</td><td>

[Create disclosures for internal and external stakeholders](../task/create-disclosure.md)

</td></tr><tr><td>

ESG metrics manager

</td><td>

-   [Create an automated metric definition](../../metrics/task/create-metric-definition.md)
-   [Create a manual metric definition](../../metrics/task/create-manual-metric-definition.md)
-   [Create a calculated metric definition](../../metrics/task/create-composite-metric-definition.md)
-   [Create a metric](../../metrics/task/create-a-metric.md)
-   [Integrating ESG Management with Watershed](integrate-esg-with-watershed.md)

</td></tr><tr><td>

ESG data owner

</td><td>

-   [Provide data for the metrics](../../metrics/task/provide-data-for-metric-data-task.md)
-   [Provide responses for multiple metrics](../task/provide-data-for-multiple-metrics.md)

</td></tr><tr><td>

ESG Risk Manager**Note:** This role is installed if you activate the sn\_esg\_risk\_mgmt plugin

</td><td>

-   [Create a risk statement](https://www.servicenow.com/docs/access?context=t_CreateRiskState&version=xanadu&pubname=xanadu-governance-risk-compliance&ft:locale=en-US)
-   [Generate a risk from a risk statement](https://www.servicenow.com/docs/access?context=t_GenRiskFromRiskFrameProfile&version=xanadu&pubname=xanadu-governance-risk-compliance&ft:locale=en-US)
-   [Create a risk assessment scope and initiate an assessment](../task/create-a-risk-asmt-scope-and-initiate-asmt.md)

</td></tr></tbody>
</table>For more information on the roles, see [Components installed with ESG Management](../reference/components-installed-with-esg.md).

**Parent Topic:**[Exploring ESG Management](esg-new-explore.md)

