---
title: Insurance Underwriting Operations release notes
description: Version history for the Insurance Underwriting Operations on the ServiceNow Store.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/store-release-notes/store-rn-fso-financial-services-insurance-underwriting.html
release: store
topic_type: reference
last_updated: "2021-04-15"
reading_time_minutes: 1
breadcrumb: [ServiceNow Store - Financial Services version history release notes, ServiceNow Store version history release notes]
---

# Insurance Underwriting Operations release notes

Version history for the Insurance Underwriting Operations on the ServiceNow Store.

**Important:** For details on system requirements and family compatibility, view the application listing on the [ServiceNow Store](https://store.servicenow.com/sn_appstore_store.do#!/store/home) website.

## Version history

-   **Version 1.0.1 - April 2021**
    -   New:
        -   Role and persona for underwriter
        -   Task type to handle insurance underwriting tasks
        -   Dedicated agent workspace for underwriter

**Parent Topic:**[ServiceNow Store - Financial Services version history release notes](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/store/markdown/store-release-notes/store-rn-fso-highlight.md)

