---
title: Safe Workplace suite Professional release notes
description: Version history for the Safe Workplace suite Professional on the ServiceNow Store.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/store-release-notes/store-rn-hr-sw-suite-professional.html
release: store
topic_type: reference
last_updated: "2025-07-31"
reading_time_minutes: 4
breadcrumb: [ServiceNow Store - HR Service Delivery version history release notes, ServiceNow Store - Employee Service Management version history release notes, ServiceNow Store version history release notes]
---

# Safe Workplace suite Professional release notes

Version history for the Safe Workplace suite Professional on the ServiceNow Store.

**Important:** For details on system requirements and family compatibility, view the application listing on the [ServiceNow Store](https://store.servicenow.com/sn_appstore_store.do#!/store/home) website.

**Note:** To automatically install all the Safe Workplace applications at once, download the ServiceNow® Safe Workplace suite application.

## Version history

-   **Version 1.25.2 - August 2025**

    Changed: Minor ACL updates

-   **Version 1.25.1 - February 2023**

    Changed: Updated version numbers for the Safe Workplace Suite Professional dependency applications.

-   **Version 1.25.0 - November 2022**

    Changed: Updated version numbers for the Safe Workplace Suite Professional dependency applications.

-   **Version 1.24.0 - August 2022**

    Changed: Updated version numbers for the Safe Workplace Suite Professional dependency applications.

-   **Version 1.23.0 - May 2022**

    New: Localization for features

-   **Version 1.22.0 - March 2022**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.21.0 - February 2022**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.20.0 - January 2022**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.19.0 - January 2022**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.18.1 - December 2021**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.17.0 - November 2021**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.16.0 - November 2021**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.15.0 - September 2021**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.14.0 - September 2021**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.13.0 - September 2021**

    Changed: Updated version numbers for the Safe Workplace Professional suite's dependency applications

-   **Version 1.12.0 - August 2021**

    Changed: Updated version numbers for the Safe Workplace Professional suite's dependency applications

-   **Version 1.11.1 - August 2021**

    Changed: Updated version numbers for the Safe Workplace Professional suite's dependency applications

-   **Version 1.10.0 - July 2021**

    Changed: Updated version numbers for the Safe Workplace Professional suite's dependency applications

-   **Version 1.9.0 - July 2021**
    -   Changed: Updates to the translation engine
    -   Fixed: Minor bugs
-   **Version 1.8.0 - June 2021**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.7.0 - May 2021**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.6.0 - April 2021**

    New: Localization for features from the April 15 release

-   **Version 1.5.0 - April 2021**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.4.0 - March 2021**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.3.0 - March 2021**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.2.0 - February 2021**

    Changed: Updated version numbers for the Safe Workplace suite Professional's dependency applications

-   **Version 1.1.0 - February 2021**

    Fixed: Corrected plugin name from 'Safe Workplace Pro suite' to 'Safe Workplace suite Professional'

-   **Version 1.0.0 - January 2021**

    The ServiceNow Safe Workplace suite Professional helps automate the complex steps for returning to the workplace in a safe and efficient process. It helps establish trust with your employees through proactive communications, minimizes potential outbreaks with health screenings, and provides mobile-friendly self‑service experiences to make reservations for shifts, workspaces, and visitors.


## Notice regarding use by organizations

All decisions in connection with the implementation of this application are at the sole decision of the Organization utilizing this application. Organizations agree that use of the application is not a representation by ServiceNow regarding the application’s compliance with any law or regulation and any suggested language provided out of the box with the application does not constitute legal advice by ServiceNow.

Organizations remain solely responsible for complying with their legal obligations under applicable law, including \(but not limited to\) data protection and employment laws, and should modify any language within the templates provided to meet the Organizations' specific requirements.

## Notice regarding use by government agencies

ServiceNow is offering this application to government agencies and their authorized users, not to government employees in their individual capacities. Use of the application does not modify any existing, or future entitlements or payment obligations for ServiceNow software or applications otherwise purchased by the government agency. ServiceNow shall not be responsible for any implementation or configuration costs associated with use of the application unless separately purchased. Government customers are solely responsible to confirm with the agency’s Ethics Office or its authorized representative that acceptance and usage of the application is permissible.

All decisions in connection with the implementation of this application are at the sole decision of the government agency utilizing this application. Agencies remain solely responsible for complying with their legal obligations under applicable laws and regulations, including \(but not limited to\) data protection and employment laws and regulations, and should modify any language within the templates provided to meet the agency’s specific requirements.

**Parent Topic:**[ServiceNow Store - HR Service Delivery version history release notes](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/store/markdown/store-release-notes/store-rn-hr.md)

