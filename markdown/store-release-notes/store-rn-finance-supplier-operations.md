---
title: Supplier Operations release notes
description: Version history for the ServiceNow Supplier Operations application on the ServiceNow Store.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/store-release-notes/store-rn-finance-supplier-operations.html
release: store
topic_type: reference
last_updated: "2026-09-10"
reading_time_minutes: 1
breadcrumb: [ServiceNow Store - Supplier Lifecycle Operations version history release notes, ServiceNow Store - Source-to-Pay Operations version history release notes, ServiceNow Store version history release notes]
---

# Supplier Operations release notes

Version history for the ServiceNow® Supplier Operations application on the ServiceNow Store.

**Important:** For details on system requirements and family compatibility, view the application listing on the [ServiceNow Store](https://store.servicenow.com/sn_appstore_store.do#!/store/home) website.

-   **Version 8.0.0 - September 2026 \(Australia\)**

    Changed: Migration of code to Fluent

-   **Version 5.0.0 - September 2026 \(Zurich\)**

    Changed: Migration of code to Fluent

-   **Version 7.0.0 - June 2026 \(Australia\)**
    -   Fixed:
        -   Improved access controls for better security.
        -   Fixed translation of user interface text to ensure accurate display of product capabilities across multiple languages, resolving issues reported.
-   **Version 4.0.0 - June 2026 \(Zurich\)**
    -   Fixed:
        -   Improved access controls for better security.
        -   Fixed translation of user interface text to ensure accurate display of product capabilities across multiple languages, resolving issues reported.
-   **Version 1.9.1 - June 2026**
    -   Fixed:
        -   Improved access controls for better security.
        -   Fixed translation of user interface text to ensure accurate display of product capabilities across multiple languages, resolving issues reported.
-   **Version 3.0.0 - March 2026**

    Streamlines and optimizes supplier management processes, enabling organizations to efficiently onboard, monitor, and collaborate with their suppliers.


**Parent Topic:**[ServiceNow Store - Supplier Lifecycle Operations version history release notes](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/store/markdown/store-release-notes/store-rn-slm-highlights.md)

