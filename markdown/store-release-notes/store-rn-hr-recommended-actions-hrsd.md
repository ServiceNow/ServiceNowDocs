---
title: Recommended Actions for HRSD release notes
description: Version history for the ServiceNow Recommended Actions for HRSD application on the ServiceNow Store.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/store-release-notes/store-rn-hr-recommended-actions-hrsd.html
release: store
topic_type: reference
last_updated: "2026-07-09"
reading_time_minutes: 1
breadcrumb: [ServiceNow Store - HR Service Delivery release notes, ServiceNow Store - Employee Service Management release notes, ServiceNow Store release notes]
---

# Recommended Actions for HRSD release notes

Version history for the ServiceNow® Recommended Actions for HRSD application on the ServiceNow Store.

**Important:** For details on system requirements and family compatibility, view the application listing on the [ServiceNow Store](https://store.servicenow.com/sn_appstore_store.do#!/store/home) website.

-   **Version 1.0.3 - July 2026**

    Fixed: RCA issues in recommended actions.

-   **Version 1.0.1 - March 2026**

    Recommended Actions in HRSD helps HR Agents in suggesting the actions they can take next to fulfill the case.


**Parent Topic:**[ServiceNow Store - HR Service Delivery release notes](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/store/markdown/store-release-notes/store-rn-hr.md)

