---
title: Manufacturing Repair Claim Management release notes
description: Version history for the Manufacturing Repair Claim Management application on the ServiceNow Store.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/store-release-notes/store-rn-manufacturing-repair-claim-mgmt.html
release: store
topic_type: reference
last_updated: "2026-07-09"
reading_time_minutes: 1
breadcrumb: [ServiceNow Store - Operational Technology release notes, ServiceNow Store release notes]
---

# Manufacturing Repair Claim Management release notes

Version history for the Manufacturing Repair Claim Management application on the ServiceNow Store.

**Important:** For details on system requirements and family compatibility, view the application listing on the [ServiceNow Store](https://store.servicenow.com/sn_appstore_store.do#!/store/home) website.

## Version history

-   **Version 3.1.2 - July 2026**

    No release notes.

-   **Version 3.0.3 - July 2026 \(Zurich\)**

    No release notes.

-   **Version 2.7.0 - June 2026**

    No release notes.

-   **Version 2.6.0 - June 2026 \(Zurich\)**

    No release notes.

-   **Version 2.5.2 - April 2026**

    No release notes.

-   **Version 2.4.2 - April 2026 \(Zurich\)**

    No release notes.

-   **Version 2.1.2 - March 2026**

    No release notes.

-   **Version 2.0.2 - March 2026 \(Zurich\)**

    No release notes.

-   **Version 1.1.1 - August 2025**
    -   New: Create repair claim, add asset details, add claim job details, submit. Review and approve/reject/partially approve/send back claims.
    -   https://docs-preview.corp.service-now.com/bundle/zurich-release-notes/page/release-notes/manufacturing/manufacturing-commercial-operations-rn.html
-   **Version 1.0.1 - August 2025**
    -   The ServiceNow Manufacturing Commercial Operations \(MCO\) product empowers you to optimize sales, support, and service operations enhancing operational efficiency, reducing costs, and boosting productivity.
    -   The Manufacturing Recall Claim Management application from ServiceNow enables customers to:
        -   Manage recall campaigns efficiently by streamlining collaboration with dealers to resolve claims. Dealers are supported in submitting repair claims for work performed as part of recall campaigns. OEM assessors benefit from a playbook-guided experience that helps them review and resolve claims effectively.
        -   Oversee warranty repair claims through seamless collaboration with dealers. Dealers can submit, edit, and track claims related to warranty repairs, while OEM assessors follow a guided playbook process to evaluate and resolve claims consistently.

**Parent Topic:**[ServiceNow Store - Operational Technology release notes](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/store/markdown/store-release-notes/store-rn-manufacturing-highlights.md)

