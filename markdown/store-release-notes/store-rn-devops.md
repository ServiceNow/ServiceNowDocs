---
title: DevOps Data Model release notes
description: Version history for the DevOps Data Model application on the ServiceNow Store.
locale: en-US
...
Version history for the DevOps Data Model application on the ServiceNow Store.
...
-   **Version 4.1.0 - August 2024**
    -   Improved the DevOps Data Model to support the following DevOps Change Velocity features:
        -   Changed:
...
-   **Version 3.0.0 - November 2023**
    ...
    -   Fixed:
        -   The Pipeline UI remained in the forever loading state when there were more than 20 pipelines.
        -   Window Server had a compatibility issue with Jenkins.
        -   Inbound events remained in the waiting state when the task execution URL had a comma\(,\) in it for Jenkins multi-branch pipelines.
        -   Tools were getting disconnected even for tools with valid credentials because of rate limit in the system.
        -   Encoded query to return the task execution URL for multi-branch Jenkins pipelines wasn't working.
        -   The orchestration task associated with a pipeline was not getting renamed when a pipeline was renamed in ADO and re-discovered in ServiceNow.
        -   Assignment group was not getting set in a change request when a template was added in a change step and the corresponding pipeline was run.
        -   Inbound events were failing while connecting to a GitHub private repository using GitHub Apps OAuth.
        -   Incorrect step names were generated when using the Matrix Strategy to build out step names in the GitHub workflow.
        -   The Sonar step in ADO release pipelines was not working for pull request pipelines.
        -   Callback request was failing for custom changes in GitHub Actions.
        -   Security fix for ADO webhooks.
        -   MID Server cluster support for connection alias of DevOps Tool wasn't working.
...
-   **Version 1.36.0 - November 2022**
    ...
    -   Fixed:
        -   The DevOps insights dashboard can now filter by Business Application.
            ...
            -   Jenkins Orchestration events randomly go into error state.
...
-   **Version 1.35.2 - September 2022**
    ...
    -   Added multi-branch pipeline support when registering a changeset or snapshot to pipeline execution \(requires DevOps Config\)
        -   Requires Jenkins plug-in for ServiceNow DevOps v1.35.2
    Fixed: MID server assigned to connection alias not picked when creating change.
...
-   **Version 1.32.0 - November 2021**
    -   New
        ...
        -   Table Archiving Policies: Archive Rules added to auto archive data which is older than 9 months \(configurable, can be configured for all archive rules using one property\) Destroy Rules are added which will purge the archived data older than 3 years. Above Archive and Destroy Rules are applied to tables consisting of data – PipelineExecution, StepExecution, TaskExecution and all related tables.
    -   Fixed
        -   Agile Development 2.0: When user with scrum\_product\_owner and scrum\_story\_creator roles creates a user story, the corresponding workitem is not created in DevOps
...
-   **Version 1.31.0 - October 2021**
    -   New
        ...
        -   Self-service Onboarding
            -   This release includes two new Service Catalog Items to enable a self-service model for onboarding teams to ServiceNow DevOps:
                -   Tool Onboarding: Allows teams to create a new ServiceNow DevOps tool instance, including tool Creation, Connection, Discovery, and Configuration. Once completed, the tool is ready for App onboarding.
                -   App Onboarding: Allows teams to create a new ServiceNow DevOps App \(or configure an existing one\), including pipeline stage discovery, controlling tracking, and \(optionally\) enabling Change Control for a step.
        -   With these two catalog items, development teams can configure their projects for use with ServiceNow DevOps from start to finish in minutes, completely self-sufficient and on their own schedule.
        -   New DevOps Change Velocity App
            -   A new "DevOps Change Velocity" app has been introduced to host the existing DevOps Change Acceleration feature. Installing DevOps Change Velocity will automatically install or upgrade all ServiceNow DevOps App dependencies, so it will no longer be necessary to install them individually. In conjunction with this, the existing "DevOps" app has been renamed to "DevOps Data Model". This is an organizational measure designed to improve the application install experience; it does not change existing ServiceNow DevOps functionality in any way.
        -   Automatic Change Control enablement for ADO and Jenkins
            -   When DevOps Change Control has been enabled in an Azure DevOps or Jenkins pipeline, the corresponding step in ServiceNow will automatically have Change Control enabled. This improvement works with the Self-service Onboarding feature to simplify and streamline the onboarding process.
...
-   **Version 1.28.0 - July 2021**
    -   New:
        ...
        -   Jenkins Integration
            -   Improved security, you can now use API key to create to Jenkins tool in ServiceNow DevOps The change request rejected/cancelled reason is sent back to the respective Jenkins pipeline so that you will know why the change was rejected/cancelled.
    -   Fixed:
        ...
        -   GitHub Enterprise - Unable to configure a repository due to incorrect webHookResourcePath
        -   Layout of pipeline UI page is broken in IE11 Jenkins Multibranch pipeline events stuck in Waiting state when branch name has '/' and has Junit tests Multibranch pipeline with a space doesn't work
        -   Azure DevOps:
            ...
            -   When the credentials change/expire after the tool is created, state of import requests for Discover action are success instead of error
            -   In a single pipeline execution, the revert commit is persisted prior to the original commit thereby not removing the revert commit from the Run Commits
...
-   **Version 1.27.0 - June 2021**
    ...
    -   Fixed:
        -   Race condition causing incorrect
        -   DevOps Inbound Event waiting reason and incomplete pipeline executions DevOps change creation fails with legacy change management plugin
...
-   **Version 1.26.1 - May 2021**
    ...
    -   Fixed:
        ...
        -   Test tool integration expects the toolID in the query parameters to be an orchestration tool id
        -   DevOps tag updates are not processed
...
-   **Version 1.23.0 - February 2021**
    ...
    -   Fixes:
        ...
        -   Inbound event generates error when processing work item for ADO tool with a project that has a space in the name. When multiple ADO pipelines \(all with tests\) are run in parallel, inbound events get stuck in waiting state with exceptions.
        -   App Onboarding payload is not stored in the encrypted field.
        -   Inbound Stage completion notifications from Azure DevOps are not processed correctly resulting in a timeout.
...
-   **Version 1.19.1 - October 2020**
    -   New:
        -   DevOps Pipeline Steps are now autocreated as the Jenkins pipeline runs. You no longer need to manually create the Pipeline Steps in ServiceNow or configure the notifications in Jenkins. Change is still configured manually in the desired steps in Jenkins and in ServiceNow DevOps.
...
-   **Version 1.18.0 - September 2020**
    -   New: DevOps Pipeline Steps are now autocreated as the orchestration tool's pipeline runs. You no longer need to manually create the Pipeline Steps and map them to the orchestration tool. Change should still be manually configured on the desired steps.
-   **Version 1.17.0 - August 2020**
    -   New: OAuth and GitHub apps are now supported with the GitHub integration
    -   Fixed:
        -   Fixed the issue where the Step name on Step Execution is empty until the entire execution is complete
        -   Improved performance for git commit processing
        -   Fixed the issue where Azure DevOps commit records are not created when the associated work item is not found
        -   Corrected handling of Azure DevOps pipeline runs with empty JUnit tests
-   **Version 1.16.0 - July 2020**
    -   New:
        -   DevOps Pipeline UI updates to include artifacts
            -   Ability to see Artifact versions created or released on the Pipeline UI
            -   Accessibility improvements
        -   Improved demo data to contain pipeline UI, tool, insight examples
        -   Ability to create Orchestration tools that utilize event-based discovery and generic change acceleration callbacks
        -   GitLab Coding and Orchestration Tool integrations via the DevOps Integration App
-   **Version 1.14.0 - May 2020**

    -   New:
        -   Customer-Managed Change - DevOps now provides greater flexibility in configuring Change Request flows, for example, support for new change types.
        -   Improved handling of out-of-sequence Azure DevOps pipeline notifications.
        -   Historical import for Azure DevOps Work Items is now supported.
    -   Fixed:
        -   Work item deleted events are now handled correctly for Azure DevOps.
        -   Commit notification handling has been corrected.
        -   Pipeline UI has been updated to include new states \(canceled by user\) in step executions.
        -   Corrected problem where only 200 records are imported due to max limit property.
        -   Tool creation note "Tool creation can take up to a minute" is now translatable.
        -   Added support for components/controls provided without states and/or properties \(expandable/collapsible content\).
        -   Addressed problem where layout does not adapt to changes in text size settings up to 200%.
    **Note:** This version of the app can only be installed on instances running New York or later.

-   **Version 1.10.0 - January 2020**
    -   New: Improvements to the Pipeline UI
-   **Version 1.9.0 - December 2019**
    -   New: The user created integration framework now supports connect and discover actions.
-   **Version 1.8.0 - November 2019**

    Improved tool creation

    -   There is a new 'Create' button that will:
        -   Create a DevOps tool, create connection, alias, and credentials, and will automatically connect to the tool
    -   App Pipeline UI
        -   From change requests that were created by DevOps, there is a Related Link to view the Pipeline UI for that execution.
        -   From App or Step Execution, you can follow the Related Link to the last execution on the pipeline UI.
-   **Version 1.4.0 - July 2019**
    -   New:
        -   A REST API that allows anyone to integrate code, plan, and orchestration tools
        -   Jenkins jobs in folders can now be discovered
        -   Change Acceleration works on MID Server
-   **Version 1.3.0 - June 2019**
    -   Name changed from "Enterprise DevOps" to "DevOps".
...
-   **Version 1.1.1 - April 2019**
    ...
    -   High-level pipelines allow more than one App to be a part of a pipeline.
    -   Model the Jenkins jobs to pipeline stages at the App level \(App stage\).
    -   Track upstream and downstream job relationships.
    Change Acceleration

    -   Create Change Requests for job runs that are under change control. Uses Configuration Items set on the App Stage.
    -   Jenkins plugin will wait for Change Request to be moved to ‘Implement’ state before executing the Jenkins job run. After the job run, the Change Request will be updated with implementation details and closed.
    Insights

    -   Change Acceleration insights provide visibility into the Change Management process and its efficiency.
    -   System Health insights allow admins to keep the pipeline operating effectively.
    ITBM Agile 2.0 Integration

    -   Automatically sync ITBM products, epics, stories, fixes, and releases into ServiceNow® DevOps.
    -   Historical data import.
    Jira Integration

    -   Automatically sync Jira projects, epics, stories, bugs, tasks, and releases into ServiceNow® DevOps.
    -   Historical data import.
    GitHub Integration

    -   Support [GitHub.com](https://github.com/) and on-prem Enterprise Github.
    -   Process commits and branches using web hooks on selected repositories.
    -   Automatically push web hook when configuring repository.
    -   Optionally track file changes.
    -   Track committer and repository quality metrics.
    -   Historical data import.
    Jenkins Integration

    -   Use the [Jenkins notification plugin](https://plugins.jenkins.io/notification) to send updates to ServiceNow® DevOps.
    -   Download and install the [ServiceNow® DevOps plugin for Jenkins](https://support.servicenow.com/kb_view.do?sysparm_article=KB0745378). There are instructions in the KB article.
    -   Import Jenkins freestyle pipeline jobs and map them to ServiceNow® DevOps pipelines.
