---
title: Amazon DynamoDB spoke release notes
description: Version history for the Integration Hub Amazon DynamoDB spoke on the ServiceNow Store.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/store-release-notes/store-integrationhub-rn-amazon-dynamodb.html
release: store
topic_type: reference
last_updated: "2022-09-01"
reading_time_minutes: 1
breadcrumb: [ServiceNow Store - Integration Hub version history release notes, ServiceNow Store - ServiceNow AI Platform Capabilities version history release notes, ServiceNow Store version history release notes]
---

# Amazon DynamoDB spoke release notes

Version history for the Integration Hub Amazon DynamoDB spoke on the ServiceNow Store.

**Important:** For details on system requirements and family compatibility, view the application listing on the [ServiceNow Store](https://store.servicenow.com/sn_appstore_store.do#!/store/home) website.

## Version history

-   **Version 1.0.1 - September 2022**

    Fixed: Improve installation performance of spoke

-   **Version 1.0.0 - June 2020**

    Provides actions to automate the management of tables, backups, and streams in Amazon DynamoDB.


**Parent Topic:**[ServiceNow Store - Integration Hub version history release notes](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/store/markdown/store-release-notes/store-integrationhub-landing.md)

