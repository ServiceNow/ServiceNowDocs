---
title: Display the help tips on the forms
description: Display the help tips on the forms by updating the preferences. Help tips provide extra details about the fields and their related actions.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/governance-risk-compliance/display-help-tips-on-forms.html
release: yokohama
topic_type: task
last_updated: "2025-07-31"
reading_time_minutes: 1
breadcrumb: [Configuring Digital resilience third-party registers, Maintaining Digital resilience third-party registers, Manage, Operational Resilience, Governance, Risk, and Compliance]
---

# Display the help tips on the forms

Display the help tips on the forms by updating the preferences. Help tips provide extra details about the fields and their related actions.

## Before you begin

Role required: sn\_oper\_res.manager

## Procedure

1.  Navigate to **** &gt; **Preferences** in the user profile.

    The user profile is shown in the example.

    \[Omitted image "user-profile-pref.png"\] Alt text: User profile.

2.  Select **Display**.

3.  Enable the **Show help tips on forms** option.

    The option is displayed in the example.

    \[Omitted image "show-help-tips-option.png"\] Alt text: Show the option.

    The help tips are displayed on the form as shown in the example.

    \[Omitted image "help-tip-on-the-forms.png"\] Alt text: Help tip.


**Parent Topic:**[Configuring Digital resilience third-party registers](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/governance-risk-compliance/conf-dg-resi-party-regi.md)

