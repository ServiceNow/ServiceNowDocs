...
Before calculating the recovery time objective \(RTO\) and recovery point objective \(RPO\) in the business impact analysis \(BIA\), verify that all the required BCM applications are installed. The illustration depicts the BCM dependency tree which shows how the parent applications are consumed by the child applications.
