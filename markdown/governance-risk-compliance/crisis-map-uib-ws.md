...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/governance-risk-compliance/crisis-map-uib-ws.html
release: xanadu
topic_type: concept
...
The Crisis map application consists of the following panels:

-   Alerts panel
-   Map view with the locations of the alerts

    **Note:**

    To enable the search functionality in Crisis map, you must set the **sn\_bcm\_map.use\_google\_places\_lib** system property. For more information on setting the property, see [Properties installed with BCM](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/governance-risk-compliance/properties-bcm.md).

    To enable Crisis map functionality in the GRC: Crisis map application, you must set up Google maps. For more information, see [Set up Google Maps API](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/platform-user-interface/configure-user-experiences/set-up-google-maps-api.md).


A typical view of the Crisis map application is shown in the following example.

\[Omitted image "crisis-map-uib-ws.png"\] Alt text: Integrating alerts in Crisis map.

For more information on the feeds, alerts, and events in Crisis map, see [Crisis map interface](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/governance-risk-compliance/threats-feeds-alerts-crisis-map.md).

## Tasks for a BCM administrator

If you are the BCM administrator, you must perform the following setup tasks for Crisis map:

-   Enable threat feed integrations that are provided by the base system.
-   Configure additional threat feeds into the Crisis Management application. The configurations alert the crisis managers about the threats and aids them to take an appropriate action on time.

For more information on the setup tasks for Crisis map, see [Setup for Crisis map](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/governance-risk-compliance/crisis-map-admin-tasks.md) in the configuration section.

## Alerts panel in Crisis map

The **Alerts** panel in Crisis map displays the following details:

-   **Search field**: You can search for an alert by using specific keywords.
-   **Show**: You can select this option to display the following types of alerts:
    -   **Active Alerts**
    -   **Dismissed Alerts**
-   **Sort by**: You can select this option to sort the alerts. Select one of the following options:
...
You can open or dismiss an alert in the alerts panel. You can also subscribe to an alert by selecting the bell icon as shown in the following example.

\[Omitted image "alerts-panel-crisis-map.png"\] Alt text: Single alert panel.

## Alerts panel

When you select an alert in the **Alerts** panel, it launches a zoomed-in view with the alert details such as category, event, and so on as shown in the following example.

\[Omitted image "single-alert-zoomed-in-view.png"\] Alt text: Focussed view for an alert.

You can view the following details of a single alert in the Alerts panel:
...
After you have set up Google maps, you can view the locations of the alerts in the Map view as shown in the following example.

\[Omitted image "layers-icon-in-map-view-crisis-map.png"\] Alt text: Locations of the alerts in the Map view.

You can view the following details in the Map view.
...
Use the Layers icon to view only the selected or all assets that can be toggled on and off on the map. As a crisis manager, when viewing the active alerts you can make an informed decision by concentrating on a single asset that can be critical at that moment from the rest. The following assets are displayed:
...
</table>**Parent Topic:**[BCM Configurable Workspace](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/governance-risk-compliance/bcm-workspace.md)
