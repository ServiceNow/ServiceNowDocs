...
-   **Request process: Ask to start a new engagement, reassess, or offboard an existing one**
    ...
    1.  An employee at your organization requests due diligence for a third-party engagement.
        ...
        6.  The system sends out an email notification to the Due diligence request assignment group. A member of the group can assign a TPR manager or TPR assessor to act as the owner of the request. This action triggers a task for the TPR manager.
...
-   **IRQ process: Scope the risk**
    ...
    1.  The TPR manager reviews and approves the IRQ.
        1.  In the Vendor Management Workspace, the TPR manager reviews and approves the IRQ by from a list on the system.

            The TPR administrator can create custom IRQs for an organization. The content of each IRQ is created and maintained by the person who is responsible for the business area, the geography, or third party that the questions are intended for. To define an IRQ, the TPR administrator adds sample questions to a questionnaire template and modifies the questions as needed. The sample questions are provided in the base system. Defining an IRQ is a no-code process.
    ...
    2.  Internal users respond to the IRQ:
        1.  In the Employee Center, any user that is interested in the engagement opens and responds to the IRQ.

            Several answers generate further clarifying questions. The answers can determine which external questionnaires are auto-generated and added to the sets that goes to the TP and engagement contact.

            For example, if an IRQ assessor responds that the third party interacts with government officials as part of the engagement, then an anti-bribery questionnaire that is appropriate to the third party's country \(ABAC in the USA or FCBA in the EU, and so on\) is included.
    ...
    **Note:** After the IRQ process has entered the IRQ in progress state, you can request risk intelligence reports associated with your due diligence request. For more information, see [Using risk intelligence reports and scores](../../grc-vendor-risk/concept/tprm-riskintel-using.md) and [Request a risk intelligence report associated with a due diligence request](../../grc-vendor-risk/task/tprm-riskintel-request-dd.md).
...
-   **TP element collection process: Collect third-party element information \(Optional\)**
    1.  The TPR manager or due diligence request owner starts the third-party element collection.
        ...
        2.  The TPR manager or owner navigates to the collection task tab and assigns the relevant third-party element collection questionnaires to the external assessment for collecting elements.
        ...
        3.  The TPR manager or owner reviews and approves the questionnaires and they’re sent to the engagement.
    2.  The system sends out an email notification to persons at the engagement. The messages notify the engagement contact to respond to the third-party element collection questionnaires.
    3.  In the Third-party portal, the engagement contacts either respond to the questionnaires directly or assign the tasks to other contacts at the organization.

    4.  The engagement contact returns the completed questionnaires.
    ...
    11. In the Vendor Management Workspace, the TPR manager or owner opens the engagement and manually adds elements as entities on the Entities tab.
    12. After all third-party element entities are assigned to an engagement, you can start the due diligence process.
-   **Due diligence process: Collect information to generate a risk score**
    ...
    7.  The system changes the state of the request to **Ready for TPRM approval** and sends alerts to the TPR managers.
    ...
    **Note:** If the third-party risk manager or owner selects the **Skip contract risk process** option during the due diligence process, the assigned contract negotiator isn’t notified and the Contract risk process state is skipped. An approver and owner can update the **Skip contract risk process** selection up until the approval process is completed. For more information about this process, see [Due diligence request process management](../reference/tprm-ws-dd-mgt-pg-details-tab.md).
