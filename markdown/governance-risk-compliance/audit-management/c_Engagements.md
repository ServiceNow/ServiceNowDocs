...
**Note:** The Entity-based access provides a framework for a more granular approach to the management of data access to objects associated with an entity. Administrators can grant access to an entity's related records by adding users or user groups, or by using entity user fields for entity-based access configuration. For more information, see [Entity Based Access](../../grc-common/concept/entity-based-access.md). When a user is qualified based on these configurations and has the minimum required roles, they will have access to the following tables:
...
-   **[Use the Audit Engagement Workbench to visually manage engagements](c_AuditEngagementWorkbench.md)**  
