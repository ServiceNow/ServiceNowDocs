...
1.  You must have the Audit manager role \(sn\_audit.manager\) and the Word template reader role \(sn\_irm\_shared\_cmn.word\_template\_reader\). You must also have access to the Audit Workspace.
...
For more information, see [Content configuration](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/governance-risk-compliance/grc-common-functions/configuring-audit-word-based-templates.md) and [Create Word template record for an audit engagement](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/governance-risk-compliance/audit-management/configure-audit-word-template.md).
...
-   [Create Word Report Templates](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/governance-risk-compliance/audit-management/audit-word-template-form.md)
-   [Create Word Template Categories](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/governance-risk-compliance/audit-management/audit-word-template-category.md)

-   If you selected the **Classic report template** option in the **Template type** field while creating the engagement record, the reports are generated as a Knowledge Base. The input template is in HTML, script, or XML. For more information, see [Create an audit report template](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/governance-risk-compliance/audit-management/create-audit-report-temp.md).
-   If you selected the **Word report template** option in the **Template type** field while creating the engagement record, the reports are generated as Word files.
...
2.  As an audit manager, audit supervisor, or auditor you can then associate the Word report template to the engagement record and generate the audit report. You must also have the template reader \(sn\_irm\_shared\_cmn.word\_template\_reader\) role.
...
9.  If you hadn’t selected the **Cloud enabled** option in the [Word template form](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/governance-risk-compliance/audit-management/audit-word-template-form.md), then select the attachment \(\[Omitted image "attachment-icon.png"\] Alt text: Attachment icon.\) icon to download the report.
...
10. If you had selected the **Cloud enabled** option in the Word template, then select the Cloud files tab for the cloud-based report.
    ...
    2.  View the contents of the audit engagement report in the cloud and share the report with your collaborators.
        ...
        For details on cloud document management, see [Manage your documents and work papers with Audit Management as cloud files](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/governance-risk-compliance/audit-management/manage-cloud-docs-using-onedrive-int.md).
    ...
    By default, the property is set as false. This means that the previous Word report as a sys attachment is deleted from the engagement record when the report is generated again. The engagement has only the latest report attached to the record.
    ...
    To reset the property, navigate to **All** &gt; **Audit** &gt; **Administration** &gt; **Properties**. See, [Properties installed with Audit Management and Advanced Audit](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/governance-risk-compliance/audit-management/r_InstallWAudit.md).
