---
title: Manage and address risks
description: Review and manage all risks stemming from risk statements having the content source of NIST 800-53.r4.
locale: en-US
release: zurich
product: GRC Common Functions
classification: grc-common-functions
topic_type: task
last_updated: "2025-07-31"
reading_time_minutes: 1
breadcrumb: [Assess controls, risks, issues, and remediation tasks, Process overview, \(Deprecated\) NIST RMF Use Case Accelerator, GRC use case accelerators, Common GRC features, Governance, Risk, and Compliance]
---

# Manage and address risks

Review and manage all risks stemming from risk statements having the content source of NIST 800-53.r4.

## Before you begin

**Note:** Starting with version 10.1.0, the NIST RMF Use Case Accelerator will be supported only for customers who currently use the product. New and existing customers should consider using the GRC: Continuous Authorization Monitoring application. For details, [Continuous Authorization and Monitoring](../../grc-cam/reference/grc-cam-landing-page.md).

Role required: sn\_irm\_nist\_rmf.security\_accessor or sn\_irm\_nist\_rmf.risk\_executive

## Procedure

1.  Navigate to **All** &gt; **NIST RMF** &gt; **Assess** &gt; **Risks**.

2.  Review each risk and their related risk statements.

3.  Manage and address any risks following the standard approach outlined in the Risk Management application.


**Parent Topic:**[Assess controls, risks, issues, and remediation tasks](../concept/rmf-assess.md)

