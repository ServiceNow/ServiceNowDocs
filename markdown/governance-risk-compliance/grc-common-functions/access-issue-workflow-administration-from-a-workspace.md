---
title: Access issue workflow administration from a workspace
description: Reach issue workflow configuration from within a product workspace, such as Risk Workspace, instead of navigating to the base GRC admin console.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/australia/governance-risk-compliance/grc-common-functions/access-issue-workflow-administration-from-a-workspace.html
release: australia
product: GRC Common Functions
classification: grc-common-functions
topic_type: task
last_updated: "2026-09-01"
reading_time_minutes: 1
breadcrumb: [Issue workflows, Common GRC features, Governance, Risk, and Compliance]
---

# Access issue workflow administration from a workspace

Reach issue workflow configuration from within a product workspace, such as Risk Workspace, instead of navigating to the base GRC admin console.

## Before you begin

Role required: sn\_grc\_issue\_mgmt.issue\_workflow\_reader

## About this task

Any workspace with GRC Issue Administration enabled shows the same issue workflow list you would otherwise reach from the base GRC admin console, without leaving the workspace.

## Procedure

1.  Navigate to **All** &gt; **Risk** &gt; **Risk Workspace**.

    You can open the workspace that you want.

2.  Select the Lists icon.

3.  Expand **Issue Administration** and select **Issue workflows**.


## Result

The issue workflow list opens, showing the same workflows available from the base GRC admin console.

**Parent Topic:**[Issue workflows](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/australia/governance-risk-compliance/grc-common-functions/issue-workflows.md)

