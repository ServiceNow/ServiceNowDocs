...
Network interfaces are used for the interconnection of signaling, or for the Internet Protocol \(IP\) or ATM networks. When you create a network interface record, it creates a corresponding configuration item \(CI\) record in the Network Interface \[cmdb\_ci\_interface\] table. To learn more, see [Data model for Telecommunications Network Inventory](../concept/uses-for-network-inventory-data.md).
