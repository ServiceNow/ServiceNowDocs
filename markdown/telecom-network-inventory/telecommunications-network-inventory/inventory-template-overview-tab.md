---
title: Inventory Template Overview tab
description: The Inventory Template Overview tab is a design-time verification surface on inventory template records. It shows the full template hierarchy as a tree and runs validation against the resolved hierarchy. No CIs are created from actions you take in this view.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/telecom-network-inventory/telecommunications-network-inventory/inventory-template-overview-tab.html
release: zurich
...
# Inventory Template Overview tab

The Inventory Template Overview tab is a design-time verification surface on inventory template records. It shows the full template hierarchy as a tree and runs validation against the resolved hierarchy. No CIs are created from actions you take in this view.
...
-   **The detail pane on the right** shows the attributes of the selected tree node: Name, Parent, Inventory model, Relationship type, Default field values, and Name Pattern where one is defined.
...
The tree traverses the inventory template's full composition graph. Related templates that have children are rendered as expandable nodes — selecting the chevron reveals those children, which may themselves be expandable.

The hierarchy can extend several levels deep. For example, the ASR9006 Edge Template nests five levels: slots, card templates, sub-slots, interface card templates, and ports — all beneath the equipment root.
...
For full details on validation behavior, see [Name pattern validation](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/telecom-network-inventory/telecommunications-network-inventory/naming-pattern-validation.md).
