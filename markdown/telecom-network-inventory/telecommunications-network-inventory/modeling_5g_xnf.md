...
![image.fiveg-network-model]

-   Business Application: The business application table stores all network functions. The business application records the labels of all classes proposed by 3GPPTM\* as a type of managed network function. To learn more, see [Define network function details](../task/create_business_applications.md).
-   Application Service: The application service stores the instance of the corresponding network function, which is associated with the business applications to indicate its type, such as DU or CU-CP and so on. Each instance of a function has a record in the application service. You can create a relationship with a business application and network interface. To learn more, see [Define network service instance details](../task/create_application_services.md).
