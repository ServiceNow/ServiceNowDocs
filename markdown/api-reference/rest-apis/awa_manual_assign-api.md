...
The sys\_id of the work item to assign to an available agent. Located in the Work Items \[awa\_work\_item\] table.The work item must be unassigned and in the **Pending Accept** or **Queued** state. For more information, refer to Check unassigned task work items.
...
Required. Sys\_id of the available agent to receive the work item. Agents are users with the awa\_agent role in the User \[sys\_user\] table.For information on how to determine if an agent is available, refer to Agent Inbox controls.
...
-   "Work item &lt;work\_item\_sys\_id&gt; cannot be assigned" – Work item provided cannot be assigned because it is in **Accepted** or **Canceled** state. Refer to Check work items and AWA events.
