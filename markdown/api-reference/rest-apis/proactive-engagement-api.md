...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/api-reference/rest-apis/proactive-engagement-api.html
release: xanadu
product: REST APIs
...
For more information, see [Proactive Engagement](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/it-service-management/proactive-engagement/proactive-engagement-landing-page.md)
...
Required. An object containing configuration item \(CI\) and user information used to detect issue details. **Note:** All parameters within this object are optional. You must pass at least one parameter within the object to identify the user or device."
...
Sys\_id of the CI device on which the issue was detected. Located in the Computer \[cmdb\_ci\_computer\] table.Data type: String
...
Sys\_id of the user for whom the issue was detected. Located in the User \[sys\_user\] table.Data type: String
...
User name of the user for whom the issue was detected. Located in the User \[sys\_user\] table.Data type: String
...
Optional. The user-defined ID to assign to the created issue. An ID is automatically generated if this parameter is not specified.Data type: Number
...
Optional. Parameters to pass to the action that will be executed on the device. The input parameters sent are passed to the configured resolution-remedial action such as a sub flow, flow action, or CI action. Data type: Object
...
Optional. The sys\_id of the process to terminate or restart. Data type: String
...
Optional. Details that could be useful for a manual investigation if the Power Usage Effectiveness \(PUE\) resolution fails. The investigative details are copied over to the incident which is created as a fallback when the PUE resolution fails.Data type: Object
...
How much CPU usage is used on the device. Data type: Number \(parsed as a string\)
...
How many processes are running on the device.Data type: Number \(parsed as a string\)
...
Required. Issue code to associate with the problem. The issue code must be available and deployed in the instance. Located in the Issue Registry \[sn\_pren\_issue\_registry\] table. The API returns an error if an empty or invalid issue is provided.Data type: String
...
Required. The unique code for the provider. This code has to match **provider\_code** field in **sn\_pren\_provider** table on the instance.Data type: String
...
The experience ID of the created experience issue. Generated from the **experience\_id** request parameter. The experience issue is created in the Experience Issues \[sn\_pren\_experience\_issue\] table.If **experience\_id** is not passed, the resultant ID is always the sys\_id of the created record.
