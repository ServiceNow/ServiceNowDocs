...
For additional information on installing and configuring the Virtual Agent Bot Integration API, see [Virtual Agent API](https://www.servicenow.com/docs/access?context=virtual-agent-api&version=xanadu&pubname=xanadu-conversational-interfaces&ft:locale=en-US).
...
-   AGENT: Switches the conversation from VA to Live Agent.
-   END\_CONVERSATION: Ends the chat conversation. The **message.text** parameter should be empty when using this action.
-   START\_CONVERSATION: Starts a chat conversation.
-   SWITCH: Switch the conversation to a topic that matches what is specified in the **intent.id** or **topic.name** parameters.
...
Flag that indicates whether the conversation is between bots, such as between a primary bot and secondary bot.
...
Unique client session identifier of the application/service that the end user is using to communicate with the ServiceNow bot. This ID helps identify the set of request/responses for a specific user conversation. The format of this parameter is determined by the implementer.Data type: String
...
NLU Intent ID of the system topic to switch the conversation to. Located in the CS Topic Language \[sys\_cs\_topic\_language\] table. The **intent.id** is resolved based on the language passed in the chat session. If no language is passed when starting a conversation, it obtains the language from the user's session/profile.Data type: String
...
Required if **attachment** parameter isn't specified. Message text. The language of the message should be passed in the **contextVariables** parameter.Data type: String
...
Name of the topic to switch to. Located in the name field of the Virtual Agent topics \[sys\_cs\_topic\] table.Data type: String
...
Sys\_id of the topic to switch to. Located in the sys\_id field of the Virtual Agent topics \[sys\_cs\_topic\] table.Data type: String
...
List of objects that describe the elements of the response message body.Data type: Array
...
Type of action message dispatched by the secondary bot to the primary bot. For example, typing indicator or agent wait time.Valid values:
...
Indicates the sensitivity of the associated data.Possible values:
...
List of objects that describe the options available for a picker control.Data type: Array
...
Type of body element. The possible values depend on the value in the **body.group** parameter. These values map back to the controls defined in the Virtual Agent Designer. For more information, see [Getting started with Virtual Agent Designer](https://www.servicenow.com/docs/access?context=conversation-designer-virtual-agent&version=xanadu&pubname=xanadu-conversational-interfaces&ft:locale=en-US).Possible values:
...
Flag that indicates whether the ServiceNow Virtual Agent has successfully completed the conversation.
...
Flag that indicates whether the message was typed in or was an option selection \(selected\).Valid values:
...
Flag that indicates that the ServiceNow Virtual Agent wants to hand control back over to the primary bot.Valid values:
...
The following example shows how to use the START\_CONVERSATION action to start a conversation with Virtual Agent. This call directs the user to the greeting topic.
