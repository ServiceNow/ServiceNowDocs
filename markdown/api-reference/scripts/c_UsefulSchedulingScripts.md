...
Description: A business duration calculates the open to close duration on an incident based on the particular [Creating and using schedules](https://www.servicenow.com/docs/access?context=c_UseSchedules&version=australia&pubname=australia-platform-administration&ft:locale=en-US). If there is no schedule specified, the script will simply use the first schedule returned by the query.
