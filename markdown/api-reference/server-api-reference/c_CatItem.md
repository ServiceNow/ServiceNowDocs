...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/api-reference/server-api-reference/c\_CatItem.html
release: xanadu
...
|Name|Type|Description|
|----|----|-----------|
|requestForUsers|Object|Array of user sys\_ids to check whether the associated user can acquire the current item and that the item can be requested on behalf of them. User sys\_ids are located in the Users \[sys\_user\] table.|

|Type|Description|
|----|-----------|
|Array|List of user names \(Name column from Users \[sys\_user\] table\) for whom the item cannot be requested for by a delegate.|

This example shows how to obtain a list of user names for whom the item cannot be requested for by a delegate.
