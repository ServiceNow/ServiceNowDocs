...
keywords: [ServiceBridge, Service Bridge, PSBPublicAPIUtil, ServiceBridge, Service Bridge, PSBPublicAPIUtil, ServiceBridge, Service Bridge, PSBPublicAPIUtil, ServiceBridge, Service Bridge, PSBPublicAPIUtil, ServiceBridge, Service Bridge, PSBPublicAPIUtil]
...
There are two types of Service Exchange configuration records that you can perform operations on using this script include:
...
For additional information on Service Exchange configuration records, see [Create configuration revisions](https://www.servicenow.com/docs/access?context=service-bridge-v2-create-config-rev&version=xanadu&pubname=xanadu-service-bridge&ft:locale=en-US).
...
For additional information on configuration revisions and their associated states, see [Create configuration revisions](https://www.servicenow.com/docs/access?context=service-bridge-v2-create-config-rev&version=xanadu&pubname=xanadu-service-bridge&ft:locale=en-US).
...
A virtual field is a field that exists in a target task table but doesn’t exist in the source task table. You use this method to pass the values for a virtual field from the source instance to the target instance. This method stores the specified values for the virtual field as JSON in the form section of the specified [remote task](https://www.servicenow.com/docs/access?context=service-bridge-v2-remote-task-overview&version=xanadu&pubname=xanadu-service-bridge&ft:locale=en-US). This remote task is then synced to the target instance where the values are stored in the mapped field in the target task table.
...
For information on how to create a remote task definition, see [Create remote task definitions in Service Exchange for Providers](https://www.servicenow.com/docs/access?context=service-bridge-v2-create-remote-tasks-defs&version=xanadu&pubname=xanadu-service-bridge&ft:locale=en-US).

For information on how to create a Service Exchange transform, see [Transform data with the Service Exchange transform framework](https://www.servicenow.com/docs/access?context=service-bridge-v2-transform-about&version=xanadu&pubname=xanadu-service-bridge&ft:locale=en-US).
