...
For details and instructions about using the SPWidgetAccessControl API in your server script, see [Configure widget security](https://www.servicenow.com/docs/access?context=configure-widget-security&version=zurich&pubname=zurich-platform-user-interface&ft:locale=en-US) \(Restrict which tables public widgets can access for guest users\).
