...
Error occurred while checking if the task is compatible with the template. See also Detecting duplicate CIs.
...
Optional \(required if the value of **master\_ci\_option** is `conditional`\). Encoded query to specify how the main CI is selected. If multiple CIs or no CIs match the query, the method selects the most recently updated CI as the main CI. The encoded query is evaluated for the template table. This table is identified in the **table** property.For details, see Encoded query strings.
...
Optional. Specifies which related tables for duplicate CIs within a de-duplication task are merged to the main CI when applying this template. See also Remediate a de-duplication task \(manual\).Possible options:
...
Specifies which related tables for duplicate CIs within a de-duplication task are merged to the main CI when applying this template. See also Remediate a de-duplication task \(manual\).Possible options:
