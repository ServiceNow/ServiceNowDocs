...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/api-reference/server-api-reference/GlideQueryGlobalAPI.html
release: xanadu
...
-   Queries with ambiguous conditional logic. For example, the following query is unclear because the system does not know whether to execute `(active = true AND name != null) OR last_name = Luddy` or `active = true AND (name != null OR last_name = Luddy)`.
    ...
    See the [where\(\)](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/api-reference/server-api-reference/GlideQueryGlobalAPI.md) method to understand how to nest a child query instead.
...
|encoded\_query|String|[Encoded query](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/platform-user-interface/configure-user-experiences/c_EncodedQueryStrings.md) to apply to the records in the specified table.|
...
**Note:** Use a terminal method in the Stream class to get the result of the query. For more information, see [Stream](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/api-reference/server-api-reference/StreamGlobalAPI.md).
