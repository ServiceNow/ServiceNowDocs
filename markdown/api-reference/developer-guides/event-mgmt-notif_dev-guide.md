...
-   [`TMFTopicEventAPIUtilOOB`](../../../../app-store/dev_portal/API_reference/TMFTopicEventAPIUtilOOB/concept/tmftopiceventapiutiloobScopedAPI.md#): Contains functions to handle API requests that are triggered by [external trigger definition flows](https://www.servicenow.com/docs/access?context=set-up-flow-ihub&version=zurich&pubname=zurich-integrate-applications&ft:locale=en-US) that have the actions `AlarmCreateNotification`, `AlarmChangeNotification`, `AlarmDeleteNotification` which create, update, and delete events.
