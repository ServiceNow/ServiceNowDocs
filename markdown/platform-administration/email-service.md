...
By default, Email Service only allows admin users to access the [Email API](https://www.servicenow.com/docs/access?context=email-api&version=australia&pubname=australia-api-reference&ft:locale=en-US). Administrators can [grant access to the Email API](../task/grant-access-to-email-api.md) by:
...
Non-admin users with the email\_api\_send role can access the Email API from the [REST API Explorer](https://www.servicenow.com/docs/access?context=use-REST-API-Explorer&version=australia&pubname=australia-api-reference&ft:locale=en-US).
