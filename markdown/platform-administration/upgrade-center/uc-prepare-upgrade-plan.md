...
4.  Apply an existing upgrade plan to an instance anytime, without being limited to during upgrades only.
    ...
    You can now also export the upgrade plan in a batch install format by using the **Export** button. See [CI/CD - POST /sn\_cicd/app/batch/install](https://www.servicenow.com/docs/access?context=cicd-api&version=zurich&pubname=zurich-api-reference&ft:locale=en-US) for more information.
