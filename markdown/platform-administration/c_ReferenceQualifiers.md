...
-   Creating reference qualifiers requires knowledge of the underlying ServiceNow [data model](../../../administer/table-administration/concept/c_TableAdministration.md) \(tables and fields\) and knowledge of the [Web services](https://www.servicenow.com/docs/access?context=r_AvailableWebServices&version=xanadu&pubname=xanadu-api-reference&ft:locale=en-US) and [Scripts](https://www.servicenow.com/docs/access?context=c_Script&version=xanadu&pubname=xanadu-application-development&ft:locale=en-US).
...
Dynamic reference qualifiers enable you to use a [Create a dynamic filter option](https://www.servicenow.com/docs/access?context=t_DynamicFilterOptions&version=xanadu&pubname=xanadu-platform-user-interface&ft:locale=en-US) to run a query against a reference field to filter the returned data set. Dynamic filter options are stored filters that can contain [Encoded query strings](https://www.servicenow.com/docs/access?context=c_EncodedQueryStrings&version=xanadu&pubname=xanadu-platform-user-interface&ft:locale=en-US), JavaScript, or script includes, and can be used in multiple dynamic reference qualifiers. Changes made to a dynamic filter option automatically apply to all reference qualifiers that use the same dynamic filter option. Use this type of reference qualifier when you want to use the same filter on multiple forms or to provide filter functionality to "non-code savvy" implementers.
...
All fields within the currently loaded form \(tables\) are available for use with the `current` object. Use [dot-walking](https://www.servicenow.com/docs/access?context=c_DotWalking&version=xanadu&pubname=xanadu-platform-user-interface&ft:locale=en-US) to access values in a table, including the referenced table. For example, on the Incident form, the **Assigned To** field references the User table. To access the email address of the user, use the following syntax: `javascript:"emailAddress=" + current.assigned_to.email`.

-   **[Configure reference qualifiers](../task/t_ConfigureReferenceQualifiers.md)**  
System administrators can configure reference qualifiers that enable filtering of the associated reference field.
-   **[The INSTANCEOF operator in reference qualifiers](c_UseTheINSTANCEOFOperator.md)**  
You can use the INSTANCEOF operator in a reference qualifier to shorten or simplify a complex class qualifier.

**Parent Topic:**[Reference field type](../../../administer/field-administration/concept/c_ReferenceField.md)

**Related topics**  


[Create a dynamic filter option](https://www.servicenow.com/docs/access?context=t_DynamicFilterOptions&version=xanadu&pubname=xanadu-platform-user-interface&ft:locale=en-US)

[Create scripted filters](https://www.servicenow.com/docs/access?context=t_ScriptedFilters&version=xanadu&pubname=xanadu-platform-user-interface&ft:locale=en-US)

[CMDB classifications](https://www.servicenow.com/docs/access?context=c_CMDBClassifications&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US)
