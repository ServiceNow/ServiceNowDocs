...
You must be familiar with the ServiceNow table and column names to export data directly from the URL. See [Navigate to a record or a module using a URL](https://www.servicenow.com/docs/access?context=navigate-using-url&version=xanadu&pubname=xanadu-platform-user-interface&ft:locale=en-US).
...
4.  Query for specific records by adding the **sysparm\_query** parameter.
    ...
    **Note:** All queries use a column name, an operator, and a value. See [Condition Builder](https://www.servicenow.com/docs/access?context=c_ConditionBuilder&version=xanadu&pubname=xanadu-platform-user-interface&ft:locale=en-US) for a list of available operators.
