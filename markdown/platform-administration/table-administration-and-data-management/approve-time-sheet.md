...
During out-of-office time, the user manager can [Delegate time sheet approvals to another user](https://www.servicenow.com/docs/access?context=delegate-time-sheet-approvals&version=zurich&pubname=zurich-it-business-management&ft:locale=en-US) with the required roles as the time sheet approver. For the specified time period, the delegated user also receives all the time sheets submitted by the time card users to process.
