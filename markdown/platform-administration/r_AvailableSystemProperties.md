...
-   **com.glide.acl\_check\_all\_filter\_on\_new**
    ...
    -   Type: true \| false
    -   Default value: false
    -   Location: [Add the property](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/t_AddAPropertyUsingSysPropsList.md) to the System Property \[sys\_properties\] table.
...
-   **com.glide.par.pae.drilldown\_to\_core\_ui**
    ...
    For more information, see [Chart interactions in a data visualization](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/now-intelligence/dv-chart-interactions.md).
...
-   **com.snc.pa.dc.flush\_snapshot\_count**
    ...
    Maximum number of Performance Analytics [snapshots](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/glossary/now-platform-glossary.md) to store in the memory during data collection. When this number is reached, the snapshots are offloaded to the database, freeing memory.
...
-   **com.snc.pa.dc.max\_row\_count\_indicator\_source**
    ...
    Maximum number of rows that are allowed to be fetched from an [indicator source](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/glossary/now-platform-glossary.md). This property only applies to jobs that do not use [Optimized data collection](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/now-intelligence/optimized-data-collection.md).
...
-   **com.snc.pa.dc.hsql.max\_row\_count\_indicator\_source**
    ...
    Maximum number of rows that are allowed to be fetched from an Indicator Source. This property only applies to jobs that use [Optimized data collection](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/now-intelligence/optimized-data-collection.md).
...
-   **com.snc.pdf.whitelisted\_urls**
    ...
    Stores a list of URLs allowed to be rendered in a PDF generated by the [PDFGenerationAPI - Scoped, Global](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/api-reference/server-api-reference/PDFGenerationAPIBothAPI.md). This list is only verified if the glide.pdf.url.whitelisting.enabled property is set to true.
...
-   **glide.custom.ip.authenticate.allow**
    ...
    -   Default value: none
    -   Location: [Add the property](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/t_AddAPropertyUsingSysPropsList.md) to the System Property \[sys\_properties\] table.
    -   Instance Security Hardening Settings: [Restrict performance monitoring access](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-security/sc-performance-monitoring-acl.md)
...
-   **glide.db.loguser**
    ...
    Display \(**true**\) or suppress display \(**false**\) of database errors such as `Unique Key violation detected by database (Duplicate entry 'ABC' for key 'name').` to logged-in user.
...
-   **glide.hosts.allowlist**
    ...
    Enables control over what hosts that a Fetch action can access. This affects HTTP APIs, like [RESTMessageV2 - Scoped, Global](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/api-reference/server-api-reference/c_RESTMessageV2API.md) and Fetch [Fetch - Scoped, Global](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/api-reference/server-api-reference/FetchAPI.md). Features of `glide.hosts.allowlist`:
...
-   **glide.html.sanitize\_all\_fields**
    ...
    -   Default value: true
    -   Location: System Property \[sys\_properties\] table
    -   Instance Security Hardening Settings: [HTML sanitizer \(instance security hardening\)](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-security/sc.md-sanitizer.md)
...
-   **glide.import\_excel.use\_only\_user\_session\_date\_format**
    ...
    -   Type: true \| false
    -   Default value: true
    -   Location: [Add the property](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/t_AddAPropertyUsingSysPropsList.md) to the System Property \[sys\_properties\] table if you want to change the value to false.
...
-   **glide.script.dot\_walk.enforce\_cross\_scope\_access\_all\_tables**
    ...
    **Important:** If **glide.script.dot\_walk.log\_and\_allow\_violations** is set to **true**, the value of this property is ignored.

    See [KB2793170: Dot-Walk Scoping Security Enhancement](https://support.servicenow.com/kb?id=kb_article_view&sysparm_article=KB2793170) for more information.
...
-   **glide.script.dot\_walk.log\_and\_allow\_violations**
    ...
    Allows dot-walk cross-scope access to unauthorized tables and logs the violation.

    When set to **true**:

    -   JavaScript accessing a caller restricted table without the proper Cross-Scope Privilege \(CSP\) or Restricted Caller Access \(RCA\) records via dot-walk will be allowed to proceed
    -   A log message will be written to the application node log file
    -   The message may be displayed to the end user if the session is interactive
    When set to **false**, system behavior is determined by the **glide.script.dot\_walk.enforce\_cross\_scope\_access\_all\_tables** property and the presence and value of the table attribute **enforce\_dot\_walk\_cross\_scope\_access** on an individual table.
...
-   **glide.script.dot\_walk.log\_stack\_trace\_when\_blocked**
    ...
    When this property is set to **true**, a full stack trace is included with the log message.

    When this property is set to **false**, no stack trace is included with the log message.
...
-   **glide.security.csrf.strict.validation.mode**
    ...
    -   Default value: false
    -   Location: System Property \[sys\_properties\] table
    -   Instance Security Hardening Settings: [CSRF strict validation \(instance security hardening\)](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-security/sc-csrf-strict-validation.md)
-   **glide.security.diag\_txns\_acl**
    ...
    -   Default value: false
    -   Location: [Add the property](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/t_AddAPropertyUsingSysPropsList.md) to the System Property \[sys\_properties\] table.
    -   Instance Security Hardening Settings: [Performance monitoring \(ACL\) \(instance security hardening\)](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-security/sc-performance-monitoring-acl.md)
...
-   **glide.security.file.mime\_type.validation**
    ...
    -   Default value: false
    -   Location: System Property \[sys\_properties\] table
    -   Instance Security Hardening Settings: [Restrict uploaded MIME types](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-security/sc-upload-mime-type-restriction.md)
...
-   **glide.security.use\_csrf\_token**
    ...
    Enables usage or a secure token to identify and validate incoming requests. This token is used to prevent Cross Site Request Forgery \(CSRF\) attacks. For more information, see [Enable anti-CSRF token](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-security/sc-anti-csrf-token.md).
...
-   **glide.service\_portal.search\_as\_you\_type\_behavior**
    ...
    Select search suggestions \(Suggestions\) or type-ahead functionality \(Typeahead\). For more information about search suggestions, see [Enable search suggestions](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/search-administration/enable-search-suggestions.md).
...
-   **glide.set\_x\_frame\_options**
    ...
    -   Default value: true
    -   Location: System Property \[sys\_properties\] table
    -   Instance Security Hardening Settings: [Implement the x-frame-options: SAMEORIGIN security header](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-security/sc-x-frame-options-sameorigin.md)
...
-   **glide.stax.allow\_entity\_resolution**
    ...
    Determines whether XML entities can be expanded during parsing by the streaming parser. For more information, see [Disable Entity Expansion within the XMLDocument2 Streaming Parser](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-security/sc-disable-entity-expansion.md).
...
-   **glide.sys.domain.use\_record\_domain\_for\_client\_scripts**
    ...
    -   Default value: false
    -   Location: System Property \[sys\_properties\] table
-   **glide.sys.fencing.restricted\_caller\_access.invalidation\_mode**

    Controls how the system invalidates Restricted Caller Access records when a source code record, such as a script include, in a cross-scope access table is committed in an update set. For more information about RCA privileges, see [Restricted caller access privilege settings](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/application-development/restricted-caller-access-privilege.md).

    -   Type: string
    -   Default value: DB\_LISTENER
    -   Valid values:
        -   DB\_LISTENER: Enables a database listener that monitors update set commits on source code tables and invalidates RCA records at the time of the commits so that cross-scope access is based on the latest source code updates.
        -   SCRIPT\_ENGINE: If an update set contains a script with changes, the script engine doesn't invalidate associated RCA records.
    -   Location: [Add the property](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/t_AddAPropertyUsingSysPropsList.md) to the System Property \[sys\_properties\] table.
...
-   **glide.ui.attachment.force\_download\_all\_mime\_types**
    ...
    -   Default value: true for new instances, false for upgraded instances
    -   Location: System Property \[sys\_properties\] table
    -   Instance Security Hardening Settings: [Restrict Downloadable MIME types](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-security/sc-downloadable-mime-type-denylist.md)
...
-   **glide.ui.chart.pie.labels.max\_items**
    ...
    -   Default value: 8
    -   Location: [Add the property](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/t_AddAPropertyUsingSysPropsList.md) to the System Property \[sys\_properties\] table.
-   **glide.ui.choice.display\_none**

    Controls the label displayed for the empty option in choice fields. Set to **true** to display **None**. When set to **false**, displays the default **--None--** label.
...
-   **glide.ui.clickthrough.popup**
    ...
    -   For document ID fields, enables \(**true**\) or disables \(**false**\) displaying the information icon and opening a pop-up window with the document's form. To learn more [Document ID field](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/t_CreatingADocumentIDField.md).
...
-   **glide.ui.m.clear\_pasteboard\_when\_backgrounded**
    ...
    -   Type: true \| false
    -   Default value: false
    -   Location: [Add the property](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/t_AddAPropertyUsingSysPropsList.md) to the System Property \[sys\_properties\] table. In Categories, add the mobile\_post\_auth category.
...
-   **glide.ui.polaris.experience**
    ...
    Enables the Next Experience user interface, landing page, and Unified Navigation. For more information, see [Next Experience UI](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-user-interface/next-experience-landing-page.md).
...
-   **mid.version.override**
    ...
    Sets an override condition for the current version for all MID Servers in your environment. For more information on version control, see [MID Server version control properties](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/servicenow-platform/c_UpgradeAndTestMIDServer.md).
...
-   **sn\_global\_searchui.hide\_results\_count**
    ...
    Hides \(**true**\) or displays \(**false**\) the number of matching records for search groups and tables on the [Global search](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/search-administration/global-search-polaris-ui.md) page.

    To modify this property, select **Enhanced Global Search UI** from the [application picker](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/t_SelectAnAppFromTheAppPicker.md) before opening the System Properties table.
...
-   **sn\_nowassist\_va.nass\_animated\_avatar\_enabled**
    ...
    An animated sparkle \[Omitted image "icon-ai-sparkle.png"\] Alt text: Animated AI sparkle icon. icon appears while the LLM response streams to the end user. The animated sparkle \[Omitted image "icon-ai-sparkle.png"\] Alt text: Animated AI sparkle icon. icon changes to the static branded Virtual Agent icon after the response has fully loaded. If this system property is set to `false`, the animated ellipsis \[Omitted image "ellipsis-horizontal-outline-24.svg"\] Alt text: Ellipsis icon. icon appears after the loading messages.
...
-   **sn\_query\_gen.hidden\_insights.groupby.min\_fields**
    ...
    -   Default value: 5
    -   Location: System Property \[sys\_properties\] table
    -   Learn more: [Extended analysis](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/now-intelligence/hidden-insights.md)
-   **sn\_query\_gen.segments.disable.filter**
    ...
    -   Default value: false
    -   Location: System Property \[sys\_properties\] table
    -   Learn more: [Segments in the Query Generation semantic layer](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/now-intelligence/querygen-segments.md)
...
-   **sys\_par\_insights\_enabled**
    ...
    -   More information: [Add a system property](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/t_AddAPropertyUsingSysPropsList.md)
-   **tablet.footer.text**
