...
Any table that extends Task can use this field. To add the field, [Personalize a form](https://www.servicenow.com/docs/access?context=t_PersonalizeAForm&version=zurich&pubname=zurich-platform-user-interface&ft:locale=en-US).

As the record is viewed, the timer counts upward. To pause the timer, select the **Stop icon** \(![Stop icon](../image/TimerStop.png)\).

To resume the timer, select the **Start icon** \(![Start icon](../image/TimerStart.png)\).
