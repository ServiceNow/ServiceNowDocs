...
-   **glide.security.auto.resubmit.ajax**: Automatically resubmits timed-out Ajax requests when set to **true** and the [Log in to an instance](https://www.servicenow.com/docs/access?context=t_LoggingIn&version=xanadu&pubname=xanadu-platform-user-interface&ft:locale=en-US) check box is selected or [Change the default value of the Remember me check box](https://www.servicenow.com/docs/access?context=c_ChSetRemMeChkbxCookie&version=xanadu&pubname=xanadu-platform-security&section=t_ChDefValRemMeChckBx&ft:locale=en-US). A pop-up appears to users asking them to continue.
