...
6.  Configure the qualifier based on the type of reference qualifier.
    ...
    -   **Simple**: [Build the condition](https://www.servicenow.com/docs/access?context=c_ConditionBuilder&version=zurich&pubname=zurich-platform-user-interface&ft:locale=en-US) using the choice lists.
    -   **Dynamic**: Either select an existing dynamic filter option or [Create a dynamic filter option](https://www.servicenow.com/docs/access?context=t_DynamicFilterOptions&version=zurich&pubname=zurich-platform-user-interface&ft:locale=en-US).
    -   **Advanced**: In the **Reference qual** field, enter an [Encoded query strings](https://www.servicenow.com/docs/access?context=c_EncodedQueryStrings&version=zurich&pubname=zurich-platform-user-interface&ft:locale=en-US), JavaScript that returns a query string, or the name of a [script include](t_ExJSConstrainAssignmentGroupField.md) or [business rule](t_ExJavaScriptLimitAssignedToField.md) to run.
...
Flag that determines whether to [Configure the edit option](https://www.servicenow.com/docs/access?context=t_ConfigureTheEditOption&version=zurich&pubname=zurich-platform-user-interface&ft:locale=en-US) for one-to-many relationships.
