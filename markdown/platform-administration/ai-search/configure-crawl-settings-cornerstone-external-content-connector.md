...
4.  Select one of the following **\[\[\[ SOMETHING \]\]\]** options:

    -   To crawl all  from the source system, select **Crawl all \[\[\[ CRAWL LOCATION \]\]\]s**.
    -   To crawl only a specified set of  from the source system, select **Include only these \[\[\[ CRAWL LOCATION \]\]\]s**, then use the **Add \[\[\[ IDENTIFIER \]\]\]s to include** field and **Add** button to enter  for  you want the connector to include when crawling.

        As an example, you might enter `` to only retrieve searchable content from this .

    -   To crawl all but a specified set of  from the source system, select **Exclude only these \[\[\[ CRAWL LOCATION \]\]\]s**, then use the **Add \[\[\[ IDENTIFIER \]\]\]s to exclude** field and **Add** button to enter  for  you want the connector to exclude when crawling.

        As an example, you might enter `` to exclude searchable content from this .
