...
Number of sentences to overlap between chunks when indexing text from semantic index field values. Higher overlap values increase recall for semantic vector search at the expense of performance.
...
Textual unit to use as the basis for chunk size when indexing semantic field values for semantic vector search.
...
Maximum number of words or sentences \(depending on your **Chunk Unit** selection\) to include in a chunk when indexing semantic field values for semantic vector search.
...
Maximum number of words to index for semantic vector search from the concatenated values of all semantic index fields.
