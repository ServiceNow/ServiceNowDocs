...
3.  Add the API permissions required by the Microsoft Teams external content connector.
    ...
    3.  For each of the following permissions, enter the permission name into the **Select permissions** search field, then locate and select the option for the permission.
        ...
        -   Sites.Read.All
            ...
            **Note:** The Microsoft Teams external content connector only uses the Sites.Read.All permission to perform automatic site discovery for crawl configuration and one-time crawls. If you plan to specify the sites to crawl manually, you don't need to select this permission. For more information on specifying sites for the connector to crawl, see [Configure crawl settings for the Microsoft Teams external content connector](configure-crawl-settings-msteams-ext-cont-connector.md). To learn more about one-time crawls, see [Run a one-time full or partial document crawl for an external content connector](run-doc-crawl-ext-cont-connector.md).
...
-   The DER-encoded binary X.509 format public key certificate's SHA1 thumbprint hash that you recorded in step [5.f](cfg-src-sys-settings-msteams-ext-cont-connector.md#certificate-thumbprint-step).
