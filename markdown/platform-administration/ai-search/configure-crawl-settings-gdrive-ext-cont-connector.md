...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/platform-administration/ai-search/configure-crawl-settings-gdrive-ext-cont-connector.html
release: xanadu
product: AI Search
...
4.  Specify the connector's crawl scope by selecting one of the following **Drives to crawl** options:

    -   To crawl all shared drives from the source system, select **All**, then optionally select **Exclude drives** and use the **Add URL** field and **Add** button to enter URLs for any shared drives that you want to exclude from the crawl.
    -   To crawl only a specified set of shared drives from the source system, select **Specify**, then use the **Add URL** field and **Add** button to enter URLs for the shared drives that you want to include in the crawl.
    ...
    **Note:** The Google Drive external content connector only crawls shared drives from your source system. It doesn't crawl user drives from your source system.
