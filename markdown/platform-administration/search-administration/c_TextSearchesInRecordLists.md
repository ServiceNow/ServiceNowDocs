...
The list search field accepts [Boolean operators](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/search-administration/c_BooleanOperators.md) \(AND, OR, and NOT\) in search queries. When a user adds a Boolean operator to a search query, the system only returns records that match all search conditions of the query.
