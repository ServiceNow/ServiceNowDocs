...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/intelligent-experiences/nlu-service/add-model-update-set.html
release: xanadu
product: NLU Service
...
For more information on update sets, see [System update sets](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/application-development/system-update-sets/system-update-sets.md).

If you want to add the model to a different update set, you must make that update set the current one. See [Create and select an update set as the current set](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/application-development/system-update-sets/create-select-update-set.md).
...
For information about parent-child update sets, see [Update set batching](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/application-development/system-update-sets/us-hier-overview.md). The following video provides a demonstration of this process.
...
8.  If the model is in a non-Global scope: In the Update Sets table sys\_update\_set, locate the parent update set and its child update sets \(refresh the page if necessary\).
    ...
    \[Omitted image "add-model-update7V.png"\] Alt text: In the Update Sets table, a parent-child update set with the parent row highlighted.
