...
Provides suggestion to Enterprise Graph on which tables to prioritize when retrieving information, to improve the accuracy of results.Tag up to 30 tables for regular mode as hints to prioritize while searching.
...
Provides a list of tables to run the search operation. Enterprise Graph \(Small\) will only search on the tables provided by tags.
 ...
 It is mandatory to add a tag when Enterprise Graph \(Small\) is used with any of the prebuilt integration with AI agent, Virtual Agent or ServiceNow Otto panel.
...
Tags are created by kg\_admins using the Knowledge Graph Tags section. Tags has to be linked to schema when selecting the Enterprise Graph or Enterprise Graph \(Small\).

Link Knowledge Graph schema for your use case in Virtual Agent, ServiceNow Otto panel or AI agent.
...
2.  ServiceNow OttoPANEL DEFAULT TAG: A prebuilt tag listing common to core, ITSM and HR tables which can be leveraged as general tag on ServiceNow Otto panel. Admins can edit this tag to add additional important table for answering questions on ServiceNow Otto panel.
...
-   Navigate to the admin center for consuming apps.
    -   For the Virtual Agent or ServiceNow Otto panel, go to your virtual agent’s setup, select Information Sources, and open the Knowledge Graph.
    -   For AI agent, proceed to its setup and open the Knowledge Graph tool for the AI agent.
-   Select Enterprise graph or Enterprise Graph \(Small\) as the Knowledge Graph schema and select the required tag from Tag drop down to link it to graph schema.

**Note:** In this release, the Enterprise Graph option is hidden by default for Virtual Agent and ServiceNow Otto panel, and only Enterprise Graph \(Small\) can be selected as schema option. To use Enterprise Graph, Admins can enable it as a schema option by setting the property `sn_kg.va.enable_global_graph_all_modes` to true. The role required is Admin.

## Recommended tables for different use cases

-   Virtual Agent: For Virtual Agent, include the tables necessary to answer the expected user queries. For example if you want to deploy a new virtual agent on employee center portal, you should add a tag having tables relevant for answering employee questions.
