...
The integration of Workflow Data Fabric \(WDF\) with the ServiceNow Knowledge Graph \(KG\) allows organizations to seamlessly connect external and internal data sources. This integration extends the functionalities of the Knowledge Graph to external data, ensuring real-time access and enabling advanced insights, workflow automation, and personalization using external data.
...
-   Natural Language Query Support: Users can ask natural language queries \(NLQ\) in Virtual Agents and AI Agents, leveraging both external \(WDF\) and internal data.
...
-   Admins: Can create a KG schema with WDF tables and link it to the Now Assist Panel, AI Agents, or Now Assist Virtual Agents.
...
-   Meaningful Naming: Use clear and meaningful names when creating WDF tables. Add synonyms to your tables in the KG schema as needed to improve the accuracy of natural language queries.
...
![Snowflake Google example.](../Images/snowflake-googl-example-kg.png)
