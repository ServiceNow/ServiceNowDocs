...
An agentic resource refers to an element in an agentic AI system that supplies data, tools, or functions allowing autonomous AI agents to perform tasks and achieve set objectives. Agentic resources give AI agents the tools to act independently, make decisions, and handle complex tasks with little human input.
...
A feature in Now Assist that translates the user input so that Now LLM can understand and provide a response back in the user's preferred language.

## episodic memory

Episodic memory is a type of AI memory that serves as a historical record of interactions and events, such as a sequence of chats or a task flow.
...
A feature in Now Assist that uses the LLMs provided by Now LLM Service, Azure OpenAI, Google Gemini, and AWS Claude to generate responses in the user's preferred language. You can enable and configure it in the Now Assist Admin console.
...
To learn more about Now Assist, see [Exploring Now Assist for Customer Service Management \(CSM\)](https://www.servicenow.com/docs/access?context=now-assist-csm-exploring&version=zurich&pubname=zurich-customer-service-management&ft:locale=en-US).
...
Semantic memory is a type of long-term memory \(LTM\) that allows AI agents to store and access factual and general knowledge across sessions.
