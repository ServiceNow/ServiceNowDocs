---
title: Activate the ServiceNow Otto panel standard chat
description: Activate the ServiceNow Otto panel standard chat to enable your agents to use AI skills, such as task summarization or navigation, in a side panel on the user interface.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/australia/intelligent-experiences/activate-now-assist-panel.html
release: australia
topic_type: task
last_updated: "2025-07-31"
reading_time_minutes: 1
keywords: [Activate, Now Assist, panel, skills, admin, experiences]
breadcrumb: [Configuring AI skills, AI Admin Hub, Enable AI experiences]
---

# Activate the ServiceNow Otto panel standard chat

Activate the ServiceNow Otto panel standard chat to enable your agents to use AI skills, such as task summarization or navigation, in a side panel on the user interface.

## Before you begin

You must install at least one AI application before you can turn on the ServiceNow Otto panel.

Role required: sn\_generative\_ai.nsa\_admin

## About this task

You must have the now\_assist\_panel\_user role to have access to the ServiceNow Otto panel once you turn it on.

To learn more about the ServiceNow Otto panel, and how it can assist your agents, see [ServiceNow Otto panel](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/australia/intelligent-experiences/now-assist-panel-overview.md).

## Procedure

1.  Navigate to **Admin** &gt; **AI Admin Hub**.

    If you’re already in AI Admin Hub, select the **AI Experiences** tab.

2.  Select **ServiceNow Otto panel**.

3.  Select **Turn On**.

    \[Omitted image "config-now-assist-2.png"\] Alt text: On the Settings tab, select Now Assist panel, and then select Turn on.

4.  In the Turn on ServiceNow Otto Panel dialog box, select **Turn on**.

    \[Omitted image "config-now-assist-3.png"\] Alt text: On the Settings tab, select Now Assist panel, and then select Turn on.

5.  If you want to use assistants, you must activate them.

    See [Activate ServiceNow Otto panel assistants](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/australia/intelligent-experiences/now-assist-panel-activate-assistant.md) for information on activating assistants.


## What to do next

For help with installation, see [Solving installation and configuration issues with ServiceNow AI features](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/australia/intelligent-experiences/now-assist-install-config-checklist.md).

For examples of the ServiceNow Otto panel in action for ServiceNow Otto applications, see the following topics:

-   [Summarize a chat conversation by using ServiceNow Otto for Customer Service Management \(CSM\)](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/customer-service-management/now-assist-csm-summarize-chat.md)
-   [Summarize a Sidebar discussion by using ServiceNow Otto for IT Service Management \(ITSM\)](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/it-service-management/now-assist-itsm-sidebar-discussion.md)
-   [Summarize an issue using ServiceNow Otto for Integrated Risk Management \(IRM\)](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/governance-risk-compliance/summarize-an-issue.md)
-   [Generate a knowledge article from HR Agent Workspace with ServiceNow Otto for HRSD](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/employee-service-management/gen-kb-now-assisthr.md)

**Parent Topic:**[Configuring AI skills](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/australia/intelligent-experiences/configuring-na-landing.md)

