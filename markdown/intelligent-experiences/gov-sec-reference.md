...
Shows AI agents that haven't been active for over 90 days. Review dormant AI agent permissions to reduce security risk.Select **Create Asset Task** to create a task to investigate why the agent has been inactive.
...
For more details on configuring integrations to show external AI agent provider metrics, see [Configure overview security metrics](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/intelligent-experiences/gov-sec-configure-overview.md) and [Configuring integrations](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/intelligent-experiences/aict-configuring-integrations.md).

Select **Create Asset Task** to create a task to review the agent's elevated permissions.
...
|------|------------|-----------|
|AI assets with critical findings|
|Agents|Base system|Number of AI agents with potential critical configuration issues, out of all agents evaluated.|
|MCP servers|Base system|Number of MCP servers with potential critical configuration issues.|
|Tools|Base system|Number of tools with potential critical configuration issues.|
|System prompts|Base system|Number of system prompts with potential critical configuration issues.|
|Findings approaching remediation target|Available with plugin|Potential configuration issues nearing the deadline your vulnerability management team set for remediation. Only shown if the AI Security Exposure Management plugin \(`com.sn_sec_ai`\) is installed.|
|Findings with remediation overdue|Available with plugin|Potentical configuration issues that have passed their remediation deadline. Only shown if the AI Security Exposure Management plugin \(`com.sn_sec_ai`\) is installed.|
|Findings deferred|Available with plugin|Potential configuration issues deferred to a later date under an approved exception request. Only shown if the AI Security Exposure Management plugin \(`com.sn_sec_ai`\) is installed.|
|Posture findings by risk rating|Available with plugin|Risk rating for potential configuration issues, calculated in AI Security Exposure Management based on factors you specify. Only shown if the AI Security Exposure Management plugin \(`com.sn_sec_ai`\) is installed.|
|Critical findings by platform|Available with plugin|Potential critical configuration issues by platform \(for example, Cisco AI Defense or HiddenLayer\).|
|Critical findings by OWASP top ten|Available with plugin|Potential configuration issues by OWASP LLM Top 10 threat type.|
|Critical findings by MITRE ATLAS technique|Available with plugin|Potential configuration issues mapped to MITRE ATLAS techniques.|
...
To activate these metrics, see [Activate design-time security metrics](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/intelligent-experiences/gov-sec-configure-design-time.md).

|Metric|Availability|Description|
|------|------------|-----------|
|Approaching remediation target|Available with plugin|Number of potential vulnerability issues approaching the deadline for remediation. This is a rule defined in AI Security Exposure Management. Only shown if the AI Security Exposure Management plugin \(`com.sn_sec_ai`\) is installed.|
...
|Remediation overdue|Available with plugin|Number of potential vulnerability issues past the deadline for remediation. This is a rule defined in AI Security Exposure Management. Only shown if the AI Security Exposure Management plugin \(`com.sn_sec_ai`\) is installed.|
...
|Vulnerabilities by risk|Available with plugin|Risk is a combination of severity and other factors defined in AI Security Exposure Management. Only shown if the AI Security Exposure Management plugin \(`com.sn_sec_ai`\) is installed.|
|Vulnerabilities by severity|Base system|Severity data is gathered from third-party AI security tools \(for example, Cisco AI Defense and HiddenLayer\). Not shown if the AI Security Exposure Management plugin \(`com.sn_sec_ai`\) is installed.|
...
|Metric|Availability|Description|
|------|------------|-----------|
|Approaching remediation target|Available with plugin|Number of potential validation issues approaching the deadline for remediation. This is a rule defined in AI Security Exposure Management. Only shown if the AI Security Exposure Management plugin \(`com.sn_sec_ai`\) is installed.|
|Findings by risk rating|Available with plugin|Risk is a combination of severity and other factors defined in AI Security Exposure Management. Only shown if the AI Security Exposure Management plugin \(`com.sn_sec_ai`\) is installed.|
...
|Remediation overdue|Available with plugin|Number of potential validation issues past the deadline for remediation. This is a rule defined in AI Security Exposure Management. Only shown if the AI Security Exposure Management plugin \(`com.sn_sec_ai`\) is installed.|
...
Each cell in the Top AI asset security events heat map contains the number of AI asset security events belonging to that category \(row\) and the timeframe \(column\), based on Coordinated Universal Time \(UTC\).

To configure these metrics, see [Configure post-runtime security metrics](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/intelligent-experiences/gov-sec-configure-event-metrics.md).
...
-   **[System properties for AI asset security](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/intelligent-experiences/gov-sec-reference-system-properties.md)**  
