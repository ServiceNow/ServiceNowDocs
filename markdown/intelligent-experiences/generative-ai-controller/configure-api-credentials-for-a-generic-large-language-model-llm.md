...
13. In the **Credential** field, select the Credential record you created with the model's API key.
    ...
    You can search the list of Credentials by typing the name of the Credential in the field. You can also select the lookup icon \(![This image shows the list lookup icon used in the Now Assist Admin console.](../../now-assist-admin/image/na-list-lookup-icon.png)\) to open a modal with the full list.
