...
The client ID from the OAuth inbound integration on the server instance.**Note:** For clients that support Client ID Metadata Document \(CIMD\) authentication, you can register a CIMD client and provide a CIMD URL instead of a client ID. For more information, see 
