...
# Gemini Cloud Assist A2A investigation AI agent
...
The agent runs a Gemini Cloud Assist investigation for a Google Cloud alert and reports the findings.

1.  Identify the alert record to investigate from the information provided by the calling agent.
2.  Start a Gemini Cloud Assist investigation for that alert and capture the resulting task and any ownership labels found on the alert's resource and policy metadata.
3.  Poll the investigation task until it completes.
4.  Retrieve the completed investigation's full result, including root-cause hypotheses and recommended next steps.
5.  Report the findings, along with any ownership information identified, back to the calling agent.
...
Get GCA Investigation Result

Get GCA Investigation Status

Start GCA Investigation
