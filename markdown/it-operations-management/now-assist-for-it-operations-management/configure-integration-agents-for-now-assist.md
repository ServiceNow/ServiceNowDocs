...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/it-operations-management/now-assist-for-it-operations-management/configure-integration-agents-for-now-assist.html
release: zurich
...
# Configure observability agents for ServiceNow Otto
...
Summary, findings, root-cause hypotheses, next steps, impact on affected Google Cloud resources, ownership, recommended actions, supporting evidence, and sources
...
Google Gemini Cloud Assist Agent
...
Google Cloud IAM permissions:-   Assign the Investigation Creator role to the service account
...
-   Enable the required APIs for your Google Cloud project: `logging.googleapis.com`, `monitoring.googleapis.com`, `cloudresourcemanager.googleapis.com`, and `geminicloudassist`
-   Gemini Cloud Assist must be set up in the Google Cloud project
...
<table id="table_vbw_mql_rjc"><thead><tr><th>
...
Alert details, metric trend analysis, device properties and classification, collector health, SDT/maintenance windows, related entity topology, CMDB service associations, recent ServiceNow Change records, top processes, and root cause analysis with verdict \(degraded / healthy / cleared-and-sustained / likely-false-positive / inconclusive\) and recommended next steps.
...
`https://<account>.logicmonitor.com/santaba/rest`
 ...
 Replace `<account>` with your LogicMonitor portal subdomain.
...
API bearer token: An LMv2 personal access token
...
LogicMonitor API v3 read-only access: Devices \(view\), Alerts \(view\), Dashboards/Metrics \(view\), Collectors \(view\), SDTs \(view\), Device Groups \(view\). The API token inherits the creating user's role. Any user with the default read only or administrator role has sufficient access.

</td></tr></tbody>
</table>## Microsoft Azure

<table id="table_pry_z4z_1kc"><thead><tr><th>
...
Alert details, resource health status, activity logs, Log Analytics query results, metric trend analysis, CMDB resource context, correlated service metrics, and root cause analysis with recommended next steps
...
-   Reader role on target Azure subscriptions
-   Log Analytics Reader role on target Log Analytics workspaces

</td></tr><tr><td>

Required Microsoft Azure settings

</td><td>

-   Microsoft Azure MCP Server must be deployed and reachable
-   Log Analytics workspaces must exist in the target subscription
-   Azure Monitor must be enabled for target resources
...
The agents are now ready to be used by the [analyze alert impact agentic workflow](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/it-operations-management/now-assist-for-it-operations-management/now-assist-itom-agentic-aia.md)
