...
7.  Service Mapping adds new application services to the Tag-Based Application Service \[cmdb\_ci\_service\_by\_tags\] table.
    ...
    **Important:** Application services mapped using tags might not include relevant CIs if those CIs do not have the correct tags.
...
You can create single tag-based application services using the Common Service Data Model \(CSDM\) flow as described in [populate application services using tags](https://www.servicenow.com/docs/access?context=populate-app-service-tags&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US).
