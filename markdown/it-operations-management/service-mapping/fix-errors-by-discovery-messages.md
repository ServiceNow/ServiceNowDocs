...
-   For an application service converted from an application service, perform the following actions:
    ...
    -   Decide how many levels of related CIs you must include into this application service. If necessary, [change the number of levels used in conversion](https://www.servicenow.com/docs/access?context=update-services-from-cmdb&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US) to reduce the service size.


</td></tr><tr><td>

The following discovery message is displayed in a tag-based application service: **This tag-based application service reached the maximum number of CIs. For more information, refer to the product documentation.**

</td><td>

Perform the following actions: -   [Modify tag definitions used for mapping](modify-tag-category-family.md) to make sure Service Mapping includes only relevant CIs in the application service.
-   If necessary, [Tag-based discovery configuration](../concept/tag_discovery_configuration.md) to change which CIs Service Mapping includes in tag-based application services.
