...
description: After the Service Mapping administrator maps IT services and fixes errors in them, the administrator and the owner collaborate to review and approve the service maps. The review and approval process is available only for discovered and manually created application services.
locale: en-US
release: xanadu
...
1.  After the initial mapping of application services, the administrator fixes errors and sends individual application services for review. The system creates a service process task assigned to the application service owner and sends an email notification about it.
...
5.  If the revised application service maps are satisfactory, the application service owner approves them. If not, the owner requests further fixes, which the administrator must address.
