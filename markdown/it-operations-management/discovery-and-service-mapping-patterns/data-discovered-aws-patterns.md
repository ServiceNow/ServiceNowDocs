...
-   **IAM user policy on the AWS Management Console**
    ...
    Typically, you create the IAM user policy for provisioning AWS resources in Cloud Provisioning and Governance, as described in [Control AWS access and permissions using policies](../../it-operations-management/task/aws-create-user-policy-cloud-mgt.md). Ensure that the IAM user policy covers the following AWS resources:
...
-   **\(optional\) Optimize discovery by including only datacenters with resources**
    ...
    Starting with Discovery and Service Mapping Patterns version 1.29.0, you can optimize discovery by limiting it to only AWS datacenters with resources. For more information, see the [AWS resources discovery by datacenters](data-discovered-aws-patterns.md#active-datacenter-discovery) section.
