---
title: Oracle Forms pattern-based discovery
description: Discovery and Service Mapping Patterns uses the Oracle - Forms pattern to find Forms instances running on Windows servers. Discovering some of these resources might require updating to the latest version of the Discovery and Service Mapping Patterns application from the ServiceNow Store.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/australia/it-operations-management/discovery-and-service-mapping-patterns/oracle-forms-pattern.html
release: australia
...
keywords: [Oracle Forms, application pattern, Windows]
breadcrumb: [Available on-premise discovery patterns, Discovery patterns used by ITOM Visibility, ITOM Visibility, IT Operations Management]
---

# Oracle Forms pattern-based discovery

Discovery and Service Mapping Patterns uses the Oracle - Forms pattern to find Forms instances running on Windows servers. Discovering some of these resources might require updating to the latest version of the Discovery and Service Mapping Patterns application from the ServiceNow Store.
...
-   **Verify read access to the version file**
    ...
    Verify you have permission for the following command: `powershell -Command "Select-String -Path '{process.environmentVariables.MW_HOME.value}\inventory\ContentsXML\comps.xml' -Pattern 'version' | Select-Object -First 5"`.
...
Discovery populates the data in the CMDB when running the Oracle - Forms pattern.
...
|Field|Description|
|-----|-----------|
|Name \[name\]|Name of the application in the following format: **Forms@\{hostname\}**.|
|Product \[product\]|Product name. The value is set to **Forms**.|
...
The Oracle - Forms pattern creates the following relationships and references to support Oracle Forms discovery. References link to records in other tables and don't appear in the CI Relationship \[cmdb\_rel\_ci\] table.
