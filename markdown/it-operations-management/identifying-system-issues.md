...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/it-operations-management/identifying-system-issues.html
release: xanadu
topic_type: concept
...
Use the Configuration card to understand the basic configuration parameters of the monitor. If a monitor is in an `unknown` state, you can use the endpoint link to verify that the correct endpoint is selected for the monitor. An `unknown` state often occurs due to an issue with the related Agent Client Collector \(ACC\) proxy.
