...
This procedure shows how to create escalation triggers in SRM. If your organization manages on-call scheduling using IT Service Management \(ITSM\), you can also use ITSM On-Call Scheduling to define escalation triggers. See [Create an escalation trigger rule](https://www.servicenow.com/docs/access?context=create-trigger-rule-oncall&version=zurich&pubname=zurich-it-service-management&ft:locale=en-US) for more information. While that procedure lists ITSM roles, SRM users only need the srm\_manager or srm\_admin role.

For a broader overview of escalation processes in ITSM, see [Designing an escalation process](https://www.servicenow.com/docs/access?context=designing-escalation-process-oncall&version=zurich&pubname=zurich-it-service-management&ft:locale=en-US).
