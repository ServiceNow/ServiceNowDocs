...
Select this proxy cluster to monitor CIs specified in an agent policy. For details, see [Configure an agent on a proxy server](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/it-operations-management/agent-client-collector/configure-agent-proxy-server.md).
