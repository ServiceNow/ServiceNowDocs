...
During this time, the MID Server also serves any relevant Assets or Plugins that the ACC requests. The MID Server ensures that the Assets and Plugins are relevant to the particular Operating System, platform, OS version, and architecture on which the ACC is running.
