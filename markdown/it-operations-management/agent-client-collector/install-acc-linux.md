...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/it-operations-management/agent-client-collector/install-acc-linux.html
release: yokohama
...
During Linux installation \(and upgrade\), the agent executable file is enabled with Linux capabilities \(CAP\_SETFCAP, CAP\_SETPCAP\) by default. Store apps such as Agent Client Collector Log Analytics \(ACC-L\) can use this to grant capabilities to read the entire file system \(CAP\_DAC\_READ\_SEARCH\). The system undergoes various security measures to verify that granting capabilities does not pose a security risk. These measures include double verification of content origination and the plugin verification process. This procedure assumes that you are familiar with commands for Linux capabilities.
...
3.  Install the Agent Client Collector package using the package manager associated with Linux distribution.
    ...
    **Note:** Some file systems may have restrictions enabled; for example, `/var/` may be mounted with a **noexec** flag. Because the agent must execute Agent Client Collector plugins that are normally stored in the `/var/cache` directory, you must deploy the application into specific folders by customizing the installation paths using the **--relocate** option as an `.rpm` parameter.
    ...
    After relocating an installation path, update the relevant `ACC_UPGRADE_CMD` string described in step [7](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/it-operations-management/agent-client-collector/install-acc-linux.md).
...
**Parent Topic:**[Agent Client Collector installation on a Linux OS system](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/it-operations-management/agent-client-collector/acc-install-linux-concept.md)
