...
Runs on the agent when file-based discovery is invoked. For details, see [Agent Client Collector File-Based Discovery](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/it-operations-management/agent-client-collector/file-based-discovery-overview.md).
...
Software discovery via running processes

</td><td>

A background policy which discovers software without executable files in the configuration table \(**sn\_acc\_vis\_content\_process\_based\_sw\_config**\) and stores them in agent's local DB. The SAM discovery policy \(for Windows and macOS\) and Software Installed Policy \(for Linux\) gathers the information collected by this policy and saves it in the **cmdb\_sam\_sw\_install** table.
...
For details on web usage data system properties, see [Web usage data collection tables and fields](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/it-operations-management/agent-client-collector/web-usage-collection-tables.md).
