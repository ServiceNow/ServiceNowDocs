...
**Important:** Windows credentials aren't necessary for vCenter Discovery when valid [VMware credentials](https://www.servicenow.com/docs/access?context=r_VMwareCredentialsForm&version=zurich&pubname=zurich-platform-security&ft:locale=en-US) are used.

## Updating the CMDB with vCenter event collector

In addition to finding vCenter data through the standard discovery process, Discovery can also update the CMDB by detecting vCenter events through a MID Server extension called the [vCenter event collector](https://www.servicenow.com/docs/access?context=c_VCenterEventProcessorExtension&version=zurich&pubname=zurich-servicenow-platform&ft:locale=en-US).
...
For instructions on configuring vCenter events, see [Configure and run the vCenter event collector extension](https://www.servicenow.com/docs/access?context=c_VCenterEventProcessorExtension&version=zurich&pubname=zurich-servicenow-platform&section=t_ConfigVCenEvntProcExt&ft:locale=en-US).
...
-   When Discovery runs, it creates an audit record in the [Components installed with CMDB Health](https://www.servicenow.com/docs/access?context=r_TablesInstalledCMDBHealth&version=zurich&pubname=zurich-servicenow-platform&ft:locale=en-US) table for the missing CI and marks the CI "stale".
