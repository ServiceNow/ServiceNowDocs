...
[Configure Service Graph Connector for GCP using the guided setup](https://www.servicenow.com/docs/access?context=sgc-config-gcp-integration&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US)
...
If an entry is marked with one asterisk, the resource type is supported by Patterns only by utilizing the **Google Asset inventory** pattern's inclusion list. For more information, see [Google Cloud Platform \(GCP\) resource inventory discovery](../../service-mapping/reference/gcp-resource-inventory-discovery.md).
...
\* These resource types are supported in Patterns only by utilizing the **Google Asset inventory** pattern's inclusion list. For more information, see [Google Cloud Platform \(GCP\) resource inventory discovery](../../service-mapping/reference/gcp-resource-inventory-discovery.md).
