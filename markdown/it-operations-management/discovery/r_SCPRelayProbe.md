...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/it-operations-management/discovery/r\_SCPRelayProbe.html
release: zurich
product: Discovery
...
The SCP Relay probe uses the same parameters as SSHCommand. Commands can be sent in or out of the context of a terminal \(tty\). Commands can be sent with or without sudo. Some commands, such as lsof, require execution in the context of root to return the required information. When commands are sent in the context of a terminal, the path is automatically widened to include a set of default paths. You can further widen the path with the path\_override parameter. If the target machine is the local machine, SSH is not used; instead, a local shell is run to execute the command.
...
</table>**Parent Topic:**[List of Discovery probes](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/it-operations-management/discovery/r_ListOfDiscoveryProbes.md)
