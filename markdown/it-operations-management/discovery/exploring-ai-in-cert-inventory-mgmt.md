---
title: AI in Certificate Inventory and Management
description: AI Agents for Discovery provides an AI agent in Certificate Inventory and Management that helps PKI administrators automatically renew TLS certificates through a conversational prompt.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/it-operations-management/discovery/exploring-ai-in-cert-inventory-mgmt.html
release: brazil
product: Discovery
classification: discovery
topic_type: concept
last_updated: "2026-09-10"
reading_time_minutes: 1
keywords: [Now Assist, generative AI, certificate, certificate renewal, AI agent]
breadcrumb: [Certificate Inventory and Management, ITOM Visibility, IT Operations Management]
---

# AI in Certificate Inventory and Management

AI Agents for Discovery provides an AI agent in Certificate Inventory and Management that helps PKI administrators automatically renew TLS certificates through a conversational prompt.

Certificate Inventory and Management includes an AI-powered certificate renewal agent. This feature requires ServiceNow Otto for IT Operations Management \(ITOM\) to be installed.

**Note:** Depending on your license, you will have access to certain application features, generative AI skills, agentic workflows, and AI agents. For more information, see [ServiceNow product tiers](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/intelligent-experiences/ai-native-sku-overview.md).

|AI feature|Description|Use case|Resources|
|----------|-----------|--------|---------|
|Certificate renewal AI agent|Uses AI to automatically renew a single TLS certificate through a conversational prompt. The agent guides the user through the renewal process and provides a link to the task record when complete.|A PKI administrator wants to renew a certificate without manually navigating the renewal workflow.|[Renew certificates using the AI agent](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/it-operations-management/discovery/automatically-renew-cert-now-assist.md)|

