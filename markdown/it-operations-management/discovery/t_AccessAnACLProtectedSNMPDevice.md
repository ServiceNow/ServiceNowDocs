...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/it-operations-management/discovery/t\_AccessAnACLProtectedSNMPDevice.html
release: zurich
product: Discovery
...
1.  Install a MID Server on a host permitted by the ACL.
    ...
    Install the MID Server on a host machine specified by IP address in the ACL. The ACL must grant this host access to the SNMP devices. Because this MID Server is scanning SNMP devices only, it can be installed on any supported operating system. The other MID Server, configured to discover WMI and SSH, can be installed on any Windows host that has visibility to the specified IP ranges.
...
3.  Create the behavior record.
    ...
    3.  To complete this behavior \(as shown in [Figure 1](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/it-operations-management/discovery/t_AccessAnACLProtectedSNMPDevice.md)\) we must create functionalities using the MID Servers we installed for that purpose.
...
4.  Define the functionalities.
    ...
    2.  **Note:** We configure the MID Server installed on the privileged host machine to scan for the SNMP protocol only.
        ...
        \[Omitted image "DiscoveryFunctionalitySNMPOnly.png"\] Alt text: Scan for SNMP Only
    ...
    3.  **Note:** In the second functionality, Discovery will scan our domain for WMI and SSH protocols. Because WMI is one of the protocols, the MID Server for this functionality must be installed on a Windows machine and must have criteria.
        ...
        \[Omitted image "DiscoveryFunctionalityAllExceptSNMP.png"\] Alt text: Scan for All Except SNMP
...
Be sure to enable the criteria by selecting this check box \(**true**\).
        ...
        \[Omitted image "WMICriteria.png"\] Alt text: Functionality Criteria for a WMI Scan
...
**Parent Topic:**[Discovery behaviors](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/it-operations-management/discovery/c_DiscoveryBehaviors.md)
