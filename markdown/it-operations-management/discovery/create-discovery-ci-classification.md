...
</table>    This example shows a completed CI classification form with the Horizontal Pattern probe defined. For instruction on creating probes, see [Discovery probes and sensors](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/it-operations-management/discovery/c_DiscoveryProbesAndSensors.md). Probes that are defined are launched when the device is properly classified, unless Discovery is configured to stop after classification.
