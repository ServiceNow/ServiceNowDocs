...
When monitoring the status of a certificate order, a configuration-related issue may cause the task to be stuck in work in progress status. The PKI Admin is responsible for resolving such issues. For example, if the MID Server is down during the scheduled job tracking the certificate order status, the system waits for the next scheduled job. It does not log an error. If the MID Server is brought back up before the subsequent job, the certificate retrieval proceeds accordingly.
