...
-   Verify that the Log Export Service \(LES\) is installed on your ServiceNow instance. LES is a ServiceNow Platform tool under a different license than Health Log Analytics that enables you to export your system logs seamlessly into HLA. For more information, see [Log Export Service \(LES\)](https://www.servicenow.com/docs/access?context=les-intro&version=zurich&pubname=zurich-platform-security&ft:locale=en-US).
-   Verify that the Hermes Messaging Service is up and running:
    1.  Navigate to **All** &gt; **Hermes Messaging Service** &gt; **Diagnostics**.
    2.  Ensure that the status icon is green, signifying that the service is active: ![Green Hermes Diagnostics status icon](../image/hla-hermes-diagnostics-status.png).
