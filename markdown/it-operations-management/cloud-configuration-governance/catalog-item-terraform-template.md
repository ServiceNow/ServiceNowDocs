...
-   If you are using Terraform Enterprise or Terraform Cloud, ensure that the MID Server can connect with Version Control System \(VCS\) accounts provisioned in the Terraform organization.
    ...
    For more information on the API credentials, see [Cloud Provisioning and Governance: Terraform Connector API Key Credentials form reference](../reference/cpg-terraform-connector-api-key-credentials-form-reference.md).
