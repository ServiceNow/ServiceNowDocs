...
4.  From the JSON key file that is associated with the service account, copy/paste the **project\_id** value into the **Account ID** field.

    ![Copy the project_id value into the Account ID field](../image/gcp-add-service-acct.png)

5.  In the **Discovery credentials** field, select the appropriate credentials for the service account.

    These are the credentials that you generated in the [Specify the credentials that Cloud Provisioning and Governance: Google Cloud Connector uses to access Google Cloud Platform data](gcp-create-creds-cloud-mgt-1.md) procedure.
