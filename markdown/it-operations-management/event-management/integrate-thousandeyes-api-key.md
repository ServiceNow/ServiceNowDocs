...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/australia/it-operations-management/event-management/integrate-thousandeyes-api-key.html
release: australia
...
3.  On the form, fill the fields.
    ...
    **Note:** The user needs to be created with the evt\_mgmt\_integration role. Create a new one on the sys\_user table to use here to facilitate the right access.
    ...
    |Token|The REST API key generated. Copy the key to use as part of the REST API call within the Header or Query parameter.|
...
Now you are ready to start sending events. In your monitoring tool you can send events either with the header or the query `param: x-sn-apikey`. For example:

```
https://[INSTANCENAME].service-now.com/api/sn_em_connector/em/inbound_event?source=[SOURCENAME]&sys_id=[SYSID OF PUSH CONNECTOR INSTANCE]&x-sn-apikey=[AUTH TOKEN]
```
