...
Without setting at least one of these properties to true, the alerts will not be properly segregated. If you do not define any grouping logic, the scale-out will not work. This occurs when both the assignment group and domain separation are set to false. All alerts will be processed by a single scheduled job.
...
6.  In the new jobs, call the queryS0 function and pass the job number as input.

    In the Run this script section, call queryS0 instead of the old query\(\) function.
