...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/it-operations-management/event-management/run-multiple-alert-group-scheduled-jobs.html
release: zurich
...
-   **sa\_analytics.agg.group\_alert\_with\_same\_group\_by\_fields**: “Group by” property, with comma-separated list of field names that need to have matching values across alerts to allow alerts to be grouped together. The property can contain alert field names \(such as assignment\_group\) or CI field names \(such as alert\_cmdb\_ci.location\). It can also contain alert additional info field names \(such as additional\_info.state\) or alert tags \(such as t\_data\_center\). Alerts are grouped when the specified field values match.
...
6.  In the new jobs, call the new JS function queryS0 in the Run this script section and pass the job number as input parameter.
