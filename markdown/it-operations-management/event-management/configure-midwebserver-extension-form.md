...
-   **Keybased** \(deprecated and kept only for compatibility with releases prior to the Zurich release\): The value in the **Secret Key** field is used to generate an authentication token sent in the **Authorization** header of incoming client requests. For details, see [Configure key-based MID Web Server authentication](../task/configure-mid-web-server-extension-metric-data.md).
