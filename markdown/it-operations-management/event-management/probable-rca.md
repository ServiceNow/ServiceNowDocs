...
Each probable root cause includes an explanation how RCA correlated the alert with the change request. The reason can indicate that the change request was applied directly to the CI that generated the alert. It can also indicate changes to a CI in the affected CIs list or to a related CI. Additionally, it can indicate changes to an application service or to software installed on the CI or related infrastructure. RCA identifies installed software by using the `cmdb_software_instance` table.
...
-   **[Customize RCA settings](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/it-operations-management/event-management/set-rca-change-query-filters.md)**  
