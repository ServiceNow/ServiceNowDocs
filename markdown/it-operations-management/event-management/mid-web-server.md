...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/it-operations-management/event-management/mid-web-server.html
release: yokohama
...
The MID Web Server extension supports MID Server clusters that are configured for failover. When selecting a MID Server cluster option, an algorithm determines which MID Server in the cluster runs the extension. The extension can run on only one MID Server. If the primary MID Server that runs the MID Web Server extension goes down, the secondary MID Server is activated. The extension automatically starts to run on the secondary MID Server.
