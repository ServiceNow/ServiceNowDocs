...
-   With two exceptions, approval and rollback activities are only available when the workflow runs on a table that extends Task. The exceptions are the [**Approval - User**](https://www.servicenow.com/docs/access?context=r_ApprovalUser&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US) and [**Approval Action**](https://www.servicenow.com/docs/access?context=r_ApprovalAction&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US) activities, which are available globally.
