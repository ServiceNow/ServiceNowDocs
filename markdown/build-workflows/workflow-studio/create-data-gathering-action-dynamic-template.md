...
**Note:** Dynamic inputs are not available in the base system. To use dynamic inputs in Workflow Studio, you must [Request an Integration Hub plugin](https://www.servicenow.com/docs/access?context=request-integrationhub&version=xanadu&pubname=xanadu-integrate-applications&ft:locale=en-US).
...
6.  In the Action Outline under the REST step, select the add a new step icon \(![Add a new step icon](../images/add-new-step-icon.png)\) and select the **Script** step.
    ...
    3.  In Script, enter a script to create a JSON output from the input variable.
        ...
        -   **Parse the input variable as JSON**
            ...
            You can use the [JSON - Global](https://www.servicenow.com/docs/access?context=c_JSONAPI&version=xanadu&pubname=xanadu-api-reference&ft:locale=en-US) API to transform the input variable from a string to a JSON object.
