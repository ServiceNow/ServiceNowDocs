...
**Note:** Integration Hub See [Request Integration Hub](https://www.servicenow.com/docs/access?context=request-ih-overview&version=australia&pubname=australia-integrate-applications&ft:locale=en-US) for information about Integration Hub usage and subscriptions.
...
-   **Vanilla \(Core JavaScript\)**: The action step runs the script from either the instance or MID Server. Select this option when the script only needs the core JavaScript APIs and not the ServiceNow API or instance data.
 ...
 The runtime you select determines the JavaScript objects and methods displayed in the [Context-sensitive help](https://www.servicenow.com/docs/access?context=c_ScriptingAssistance&version=australia&pubname=australia-api-reference&ft:locale=en-US).
