...
Workflow Studio sets the action status when the SLA state matches an [SLA condition](https://www.servicenow.com/docs/access?context=c_SLAConditions&version=xanadu&pubname=xanadu-it-service-management&ft:locale=en-US) or when certain UI actions are selected.
