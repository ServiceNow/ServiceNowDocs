---
title: Turn on AI skills for Playbooks
description: Turn on the ServiceNow Otto for Creator skills for using generative AI for creating and managing Playbooks.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/build-workflows/workflow-studio/turn-on-playbook-generation-skill.html
release: brazil
product: Workflow Studio
classification: workflow-studio
topic_type: task
last_updated: "2026-09-10"
reading_time_minutes: 1
breadcrumb: [Configure, Playbooks, Workflow Studio, Build workflows]
---

# Turn on AI skills for Playbooks

Turn on the ServiceNow Otto for Creator skills for using generative AI for creating and managing Playbooks.

## Before you begin

-   Install the ServiceNow Otto for Creator application. For more information, see .
-   Role required: admin

## About this task

You can use AI to generate a playbook, a summary of a playbook, or recommendations for placeholder activities. To enable these features, activate the corresponding AI skill in AI Admin Hub.

<table id="table_cmj_fxg_lkc"><thead><tr><th>

AI capability

</th><th>

AI skill name

</th></tr></thead><tbody><tr><td>

[Generate a playbook from text or image](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/build-workflows/workflow-studio/generate-a-playbook-outline.md)

</td><td>

Playbook generationPlaybook generation with images

</td></tr><tr><td>

[Generate a playbook from a knowledge base article](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/build-workflows/workflow-studio/generate-playbook-from-kb.md)

</td><td>

Playbook generation with KB

</td></tr><tr><td>

[Generate recommendations for placeholder activities](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/build-workflows/workflow-studio/generate-playbook-recommendations.md)

</td><td>

Playbook recommendations

</td></tr><tr><td>

[Generate a playbook summary](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/build-workflows/workflow-studio/generate-playbook-summary.md)

</td><td>

Playbook summarization

</td></tr></tbody>
</table>**Important:** The playbook generation skill requires a separate subscription to ServiceNow Otto for Creator.

## Procedure

1.  Navigate to **Admin** &gt; **AI Admin Hub** &gt; **AI Skills**.

2.  In the workflow list, select **Creator**.

3.  In the search box, search for `Playbook` skills.

4.  On the skill tile, select **Turn on**.

    If a skill is already turned on, you see the option to deactivate the skill.

    \[Omitted image "ai-skills-playbook.png"\] Alt text: Activate or deactivate generative AI skills for Playbooks.


## Result

Generative AI skills for Playbooks are active on your instance.

**Parent Topic:**[Configuring Playbooks](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/build-workflows/workflow-studio/setting-up-process-automation-designer.md)

**Related topics**  


[Generate a playbook from text or image](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/build-workflows/workflow-studio/generate-a-playbook-outline.md)

[Change the default LLM for playbook generation](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/build-workflows/workflow-studio/change-default-llm-playbook-generation.md)

