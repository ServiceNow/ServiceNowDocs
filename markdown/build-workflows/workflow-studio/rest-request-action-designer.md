...
REST API calls return data in the Response Body. The response body data is usually structured in JSON or XML format. You can use a [Script step](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/build-workflows/workflow-studio/javascript-step-action-designer.md) to parse the structured data into variables to use elsewhere in the action or in a flow. There is also an  to parse a response body that's in an XML format.
