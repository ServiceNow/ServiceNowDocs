...
**Note:** This feature requires an Integration Hub Enterprise subscription. For more information, see [Request Integration Hub](https://www.servicenow.com/docs/access?context=request-ih-overview&version=xanadu&pubname=xanadu-integrate-applications&ft:locale=en-US).

|Trigger|Description|
|-------|-----------|
|REST API - Asynchronous|Start a flow from an inbound API call or webhook from an external system. Configure the trigger start conditions without having to write or maintain custom code. For more information, see [REST API trigger](https://www.servicenow.com/docs/access?context=rest-trigger&version=xanadu&pubname=xanadu-integrate-applications&ft:locale=en-US).|
...
1.  [Set up a flow](https://www.servicenow.com/docs/access?context=set-up-flow-ihub&version=xanadu&pubname=xanadu-integrate-applications&ft:locale=en-US)
2.  [Set up external trigger endpoints](https://www.servicenow.com/docs/access?context=set-up-external-webhook-endpoints&version=xanadu&pubname=xanadu-integrate-applications&ft:locale=en-US).
...
-   **When to run the flow**
    ...
    |Option|Description|
    |------|-----------|
    |Only Run for Non-Interactive Session|Flow that is only triggered in non-interactive sessions. See [Non-interactive sessions](https://www.servicenow.com/docs/access?context=c_NonInteractiveSessions&version=xanadu&pubname=xanadu-platform-administration&ft:locale=en-US).|
    ...
    |Option|Description|
    |------|-----------|
    |Do not run if triggered by the following users|Flow that does not trigger for a selected list of users. Click the Add User icon \(![Add User Icon](../images/icon-select-users.png)\) to add users to the list.|
    |Only run if triggered by the following users|Flow that triggers only for a selected list of users. Click the Add User icon \(![Add User Icon](../images/icon-select-users.png)\) to add users to the list.|
...
Flow that runs asynchronously in the background. Use this option for flows that do not require immediate updates and to allow other system processes to run at the same time.
