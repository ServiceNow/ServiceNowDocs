...
3.  Under **TRIGGER**, select **Add a trigger**.
    ...
    If the flow already contains a trigger and you want to replace with a saved trigger, select **Delete trigger**.

4.  From the **Trigger** list, under **INSTALLED APPLICATIONS**, select the application under which the trigger was created.

    For example, Global. Workflow Studio displays a list of triggers present in the selected application.

5.  Select the trigger.

    **Tip:** Select the info icon to see more details about the trigger.
...
6.  Depending on the trigger configurations, you can view the trigger properties and add additional conditions for the trigger.
    ...
    Adding additional conditions affects only the flow, it doesn't alter the saved trigger.
...
The selected saved trigger is added to the flow.
