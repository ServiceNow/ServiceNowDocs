...
3.  Tap to select the **Select Campus** tab.
    ...
    2.  Use the zoom in \(![Zoom in control icon to focus on the map artifacts.](../images/wsd-mobile-zoom-in-icon-new-loc-directory.jpeg)\) controls on the map to bring into focus the location pin marker icon \(![Location pin marker on map.](../../workplace-service-delivery/images/wsd-location-pin-icon.png)\) to select a campus.
