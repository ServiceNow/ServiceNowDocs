---
title: View move tasks
description: Monitor the move-related tasks that are created in the application to fulfill a move request. View the move tasks assigned to you, the tasks that are currently open, and all the move tasks created in the application.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/employee-service-management/view-move-tasks.html
release: yokohama
topic_type: task
last_updated: "2025-01-30"
reading_time_minutes: 1
breadcrumb: [Analytics and Reporting, Workplace Move Management, Workplace Service Delivery, Employee Service Management]
---

# View move tasks

Monitor the move-related tasks that are created in the application to fulfill a move request. View the move tasks assigned to you, the tasks that are currently open, and all the move tasks created in the application.

## Before you begin

Role required: sn\_wsd\_move.manager

## Procedure

1.  Navigate to **All** &gt; **Workplace Move Management** &gt; **Move tasks**.

2.  To view move tasks, select any of the following options:

    -   **Assigned to me**: Select this option to view the move tasks that are assigned to you.
    -   **Open tasks:** Select this option to view open move tasks.
    -   **All move tasks**: Select this option to view all the move tasks.

**Parent Topic:**[Analytics and reporting solutions for Workplace Move Management](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/employee-service-management/analytics-reporting-solution.md)

**Related topics**  


[View move requests]()

