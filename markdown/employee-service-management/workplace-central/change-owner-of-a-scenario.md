---
title: Change owner of a scenario
description: Change the ownership of a scenario that is published and is in the Ready state. Enable other space planners also to make their changes to the scenario that you created. A can switch a scenario between multiple owners one at a time.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/employee-service-management/workplace-central/change-owner-of-a-scenario.html
release: yokohama
product: Workplace Central
classification: workplace-central
topic_type: task
last_updated: "2025-01-30"
reading_time_minutes: 3
breadcrumb: [Working with Space Optimization, Use, Workplace Central, Workplace Service Delivery, Employee Service Management]
---

# Change owner of a scenario

Change the ownership of a scenario that is published and is in the Ready state. Enable other space planners also to make their changes to the scenario that you created. A can switch a scenario between multiple owners one at a time.

## Before you begin

Ensure the following:

1.  [Create a scenario](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown).
2.  [Publish a scenario](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/employee-service-management/workplace-central/publish-a-scenario.md)

Role required: sn\_wsd\_spcmgmt.space\_planner

## About this task

Change the ownership of a published scenario to enable other space planners to implement their changes. A scenario can be worked upon by multiple space planners one at a time.

## Procedure

1.  Navigate to any one of the following locations:

    -   **All** &gt; **Workplace Central** &gt; **Workplace Central**.
    -   **All** &gt; **Scenario Planning** &gt; **My Scenario Plans**
    You can also open Workplace Central from the Employee Center directly. Navigate to **Workspaces** &gt; **Workplace Central**.

    The Workplace Analytics dashboard opens.

2.  On the left pane, select **Space Optimization** icon \(\[Omitted image "space-optimization-icon.png"\] Alt text: Space optimization icon.\).

    The Space optimization dashboard opens.

3.  Open the scenario for which you want to change the ownership.

    Follow any one of the following paths.

<table id="choicetable_vtv_5nj_3vb"><thead><tr><th align="left" id="d394626e157">

Path

</th><th align="left" id="d394626e160">

Action

</th></tr></thead><tbody><tr><td id="d394626e166">

**From the Scenarios list section**

</td><td>

1.  At the end of the scenarios list, click **View all**.

A list of all the scenarios created in the application are displayed.

2.  Select the scenario for which you want to change the ownership.

You can use the search and filter options on the list.

</td></tr><tr><td id="d394626e191">

**From the Buildings list section**

</td><td>

1.  Select the building of which you want change the scenario ownership.

The stack plan view of the building opens.

2.  Select **Related scenarios**.
3.  Select the scenario for which you want to change the ownership.


</td></tr></tbody>
</table>    By default, the Stack plan of the selected scenario opens.

4.  Select the **Details** tab of the scenario.

    The Scenario details form opens.

5.  In the **Owned by** field, select the space planner's name to whom you want to move the ownership.

6.  Click **Save**.


## Result

The scenario is moved to the selected space planners. You can no longer change the scenario. You can make changes to a scenario only if you are assigned as the owner of the scenario.

**Parent Topic:**[Working with Space Optimization](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/employee-service-management/workplace-central/working-with-space-optimization.md)

**Related topics**  


[Create a scenario]()

[Viewing or editing a scenario]()

[Review a scenario]()

[Publish a scenario]()

[Send a scenario for approval]()

[Deploy a scenario]()

[View scenario change details]()

[Create a copy of an existing scenario]()

[View or edit space allocations of a building]()

[Work on a space assist request]()

[Map based space administration]()

