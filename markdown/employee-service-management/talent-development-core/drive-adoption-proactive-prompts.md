...
The following example shows how proactive prompts are used to drive easy updates to employees in their flow of work.

![Screen shot that shows a proactive prompt on an employee's mobile screen.](../image/skills-intelligence-proactive-prompts.png "Drive easy updates with Proactive Prompts")

As you can see in the example, the employee receives a prompt to update the suggestions that the Skills Foundation application detected. The prompt sent to the employee clearly communicates the information. The employee just has to select a button to add the suggested skills to their skills profile.
