...
Header for the prompt. For information on adding a new header, see [Add a new prompt header to identify the prompt](proactive-prompts-add-header.md).The header is an icon and a label that is displayed on the prompt to identify the type of prompt on Employee Portal. For example, you could have headers for Skills, Learning, Roles, and so on.
