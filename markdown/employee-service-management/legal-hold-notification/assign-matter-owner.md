---
title: Assign a matter owner
description: Assign a matter owner \(fulfiller\) for the legal hold matter to be responsible for managing it.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/employee-service-management/legal-hold-notification/assign-matter-owner.html
release: yokohama
product: Legal Hold Notification
classification: legal-hold-notification
topic_type: task
last_updated: "2025-07-08"
reading_time_minutes: 1
breadcrumb: [Using Legal Hold Notification, Legal Hold Notification, Legal Service Delivery Practice Applications, Legal Service Delivery, Employee Service Management]
---

# Assign a matter owner

Assign a matter owner \(fulfiller\) for the legal hold matter to be responsible for managing it.

## Before you begin

Role required: admin

## Procedure

1.  Navigate to **All** &gt; **Legal Request** &gt; **Legal Counsel Center**.

2.  Select the List icon \(\[Omitted image "list-icon-hs.png"\] Alt text: List icon\).

3.  In the **Default lists** tab, navigate to **Matters** &gt; **All**.

4.  Select the legal matter from the list.

5.  Update the **Assignment group permission** field value to **Read**.

6.  In the **Matter owner** field, select the Search for Record icon \(\[Omitted image "lookup-using-list.png"\] Alt text:\) and search for and select the fulfiller to be responsible for managing this legal hold matter.

7.  Select **Save**.


## Result

The legal matter state changes from New to Assigned.

A matter owner \(fulfiller\) is assigned to the legal hold matter.

**Parent Topic:**[Using Legal Hold Notification](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/employee-service-management/legal-hold-notification/use-lg-hold-notif.md)

