...
8.  In the **Notification preferences** section, select the channel to be notified on when the visitor checks into the building.
    ...
    You can link the Virtual Agent with other channels like Microsoft Teams or Slack. For more information, see [Integrating Virtual Agent with other channels](https://www.servicenow.com/docs/access?context=integrate-virtual-agent&version=australia&pubname=australia-conversational-interfaces&ft:locale=en-US).
