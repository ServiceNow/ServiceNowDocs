---
title: Ask a Health and Safety question from Employee Center
description: Ask a health and safety question from the safety department to request information such as health and safety procedures, training, or return to work.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/employee-service-management/health-and-safety-case-management/hs-ask-health-safety-question.html
release: xanadu
product: Health and Safety Case Management
classification: health-and-safety-case-management
topic_type: task
last_updated: "2025-07-31"
reading_time_minutes: 1
breadcrumb: [Submit cases, Health and Safety Case Management, Health and Safety, Employee Service Management]
---

# Ask a Health and Safety question from Employee Center

Ask a health and safety question from the safety department to request information such as health and safety procedures, training, or return to work.

## Before you begin

Role required: sn\_hs\_cm.case\_reporter

## About this task

Employees can also submit safety related questions from their mobile device using the ServiceNow® Now Mobile app.

## Procedure

1.  Navigate to **All** &gt; **Self-Service** &gt; **Employee Center**.

2.  In the Employee Center, select the **Health and Safety** menu and then **Ask a Health and Safety Question**.

3.  On the form, fill in the fields.

    1.  Select an appropriate category for your question, such as **General inquiry** and **Injury / Incident related leave**.

    2.  Add a related safety incident, if any.

4.  If necessary, attach an image related to your question using the add attachments icon \(\[Omitted image "icon-add-attachment.png"\] Alt text: Add attachment icon.\).

5.  Select **Submit**.


## Result

-   Based on the selected category of your question, a safety case is submitted in the Open state. To view and track the updates to your question, navigate to the **My Requests** list in the Employee Center and open the record.

    **Note:** If you selected someone else in the **Requested For** field to submit this question for, the case won't appear in your **My Requests** list. Instead, it appears in the **My Requests** list of the person you selected.

-   The safety case is also listed in the Cases list on Health and Safety Workspace. A safety case manager \[sn\_hs\_cm.case\_manager\] can view all the filed questions and requests by navigating to the Health and Safety Workspace and selecting the cases icon \(\[Omitted image "icon-hs-cases.png"\] Alt text: Health and safety cases icon.\). For more information, see [Work on a Health and Safety case](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/employee-service-management/health-and-safety-case-management/hs-work-health-safety-case.md).

**Parent Topic:**[Submitting Health and Safety cases](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/employee-service-management/health-and-safety-case-management/hs-submitting-health-safety-cases.md)

