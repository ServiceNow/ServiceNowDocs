---
title: Configure approval workflows
description: Customize an approval workflow according to your organizational requirements.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/australia/employee-service-management/recruitment-workspace/set-up-approval-ta.html
release: australia
product: Recruitment Workspace
classification: recruitment-workspace
topic_type: task
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Configure, Recruitment Workspace, Hiring Experiences, HR Service Delivery, Employee Service Management]
---

# Configure approval workflows

Customize an approval workflow according to your organizational requirements.

## Before you begin

Role required: admin

## About this task

Hiring Experiences provides an approval flow, where the approval request goes to the manager of the hiring manager, as part of the base system configuration. You can customize the approval flow, to add a different role for the approval, or add more conditions to it, with the following procedure.

## Procedure

1.  Navigate to **All** &gt; **Process Automation** &gt; **Workflow Studio**.

2.  Select the **Subflows** tab.

3.  Locate and select **Submit for approval subflow**.

4.  On the Submit for approval subflow page, select **Ask for Approval**.

5.  Modify the conditions in **Rules**.

6.  Select **Done** and **Save**.

7.  Select **Publish**.


**Parent Topic:**[Configuring Recruitment Workspace](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/australia/employee-service-management/recruitment-workspace/config-recruitment.md)

