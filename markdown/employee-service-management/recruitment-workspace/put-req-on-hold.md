---
title: Put a job requisition on hold
description: Put a job requisition on hold if the requirement changes.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/employee-service-management/recruitment-workspace/put-req-on-hold.html
release: zurich
product: Recruitment Workspace
classification: recruitment-workspace
topic_type: task
last_updated: "2025-07-31"
reading_time_minutes: 1
breadcrumb: [Manage the job requisitions as a recruiter, Use, Recruitment Workspace, Hiring Experiences, HR Service Delivery, Employee Service Management]
---

# Put a job requisition on hold

Put a job requisition on hold if the requirement changes.

## Before you begin

Role required: sn\_ta\_hiring\_core.recruiter

## Procedure

1.  Navigate to **Workspaces** &gt; **Recruitment workspace**.

2.  Go to **Active requisitions** from the recruitment dashboard.

    1.  Choose an active requisition from the list.

3.  Select **On hold**.

    1.  In the Pause the job requisition pop-up window, select a reason for the action.

    2.  Add comments, if any.

        **Note:**

        -   If you choose the other reason option from the list of reasons, providing comments in the notes is required.
        -   If a requisition is closed while it was on hold, the on hold state won't be retained.
    You can put a Job requisition on hold regardless of its state. For more information on the requisition state maangement, see [Job requisition state management](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/employee-service-management/recruitment-workspace/job-req-state-mngmnt.md).


## Result

The requisition is put on hold and displays the last state along with the on hold badge when searched.

**Parent Topic:**[Manage the job requisitions as a recruiter](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/employee-service-management/recruitment-workspace/manage-job-reqs.md)

**Related topics**  


[Withdraw hold on a job requisition]()

[Duplicate a job requisition]()

[Close a job requisition]()

[Reopen a job requisition]()

[Submit a job requisition for approval]()

[Cancel a submitted approval request]()

[Resend a job requisition for approval]()

[Open a job requisition in your ATS]()

