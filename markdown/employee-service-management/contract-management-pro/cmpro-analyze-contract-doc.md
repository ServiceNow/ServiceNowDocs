...
2.  Select **Analyze contract**.
    ...
    2.  A card is created for each document in analyzed contracts within the Now Assist contextual panel. The contract request state updates to the Awaiting review status.
    3.  When the analysis of the documents is complete, the status changes to Ready for review. The details are listed in the following table.

        |Section|Description|
        |-------|-----------|
        |Pending reviews|Displays the number of deviations reported and pending for review.|
        |Completed|Displays the number of reviewed deviations and any actions taken.|
