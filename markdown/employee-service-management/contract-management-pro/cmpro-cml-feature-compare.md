---
title: Contract Management solutions
description: Explore the differences between the different contract management solutions offered in ServiceNow - ITAM Contract Management, \(Deprecated\) Legal Simple Contracts, and Contract Management Pro \(CM Pro\)
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/employee-service-management/contract-management-pro/cmpro-cml-feature-compare.html
release: zurich
product: Contract Management Pro
classification: contract-management-pro
topic_type: reference
last_updated: "2025-10-27"
reading_time_minutes: 4
breadcrumb: [Reference, Contract Management Pro, Legal and Contract Operations, Employee Service Management]
---

# Contract Management solutions

Explore the differences between the different contract management solutions offered in ServiceNow - ITAM Contract Management, \(Deprecated\) Legal Simple Contracts, and Contract Management Pro \(CM Pro\)

This resource helps you review the strengths of each application and guide you in selecting the most suitable option for your organization’s contract management needs.

-   Contract Management Pro: Management of contract creation, execution, storage, and reporting
-   \(Deprecated\) LSD Legal Simple Contracts: Creation of standard language contracts
-   ITAM Contract Management: Software and hardware asset management

<table><thead><tr><th>

 

</th><th>

Features

</th><th>

ITAM Contract Management

</th><th>

LSD Legal Simple Contracts\(Deprecated\)

</th><th>

Contract Management Pro

</th></tr></thead><tbody><tr><td rowspan="4">

Contract Request

</td><td align="center">

Self-serve with configurable Intake

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Third party contracts

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Third party contracts with multiple documents

</td><td>

-

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

NOW Integrations \(Legal, Procurement, Sales\)

</td><td>

-

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td rowspan="12">

Contract Authoring

</td><td>

PDF, HTML Templates

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

-

</td></tr><tr><td>

MS Word Templates

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

MS Word Add-in

</td><td>

-

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Clause Library for Contract Generation

</td><td>

-

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Versions

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Versions with multiple documents

</td><td>

-

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

External storage \(OneDrive and GoogleDrive\)

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Create contract record

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

\[Omitted image "bus-ai-sparkle.svg"\] Alt text: AI-assisted Review

</td><td>

-

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Contract Workspace

</td><td>

-

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Clause Library for Negotiation

</td><td>

-

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Amendment workflow

</td><td>

-

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td rowspan="2">

Signatures

</td><td>

Wet Signature

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Electronic signature \(AdobeSign, DocuSign integration\)

</td><td>

 

</td><td>

 

</td><td>

 

</td></tr><tr><td rowspan="2">

Review and Approvals

</td><td>

Ad-hoc approvals

</td><td>

 

</td><td>

 

</td><td>

 

</td></tr><tr><td>

Internal reviews

</td><td>

 

</td><td>

 

</td><td>

 

</td></tr><tr><td rowspan="5">

Repository

</td><td>

Document storage in ServiceNow

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

OneDrive, Google Drive storage integration

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Contract Relationships \(parent/child\)

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Contract Metadata

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td align="center">

\[Omitted image "bus-ai-sparkle.svg"\] Alt text:AI Metadata Extraction

</td><td>

-

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td rowspan="2">

Obligation Management

</td><td>

Expiry / Renewal notifications

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

\[Omitted image "bus-ai-sparkle.svg"\] Alt text: AI Obligation Extraction

</td><td>

-

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td rowspan="2">

Analytics

</td><td>

Dashboard for Volume &amp; Work distribution

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr><tr><td>

Dashboard for asset contract renewals

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td><td>

-

</td><td>

\[Omitted image "icon-check-mark-green.png"\] Alt text: Yes

</td></tr></tbody>
</table>## Migrating from Legal Simple Contracts to Contract Management Pro

Legal Simple Contracts is being deprecated.

-   For new customers, it is now hidden and no longer available.
-   For existing customers, it will continue to function, but is no longer supported.

For details, see the [Deprecation Process \[KB0867184\]](https://support.servicenow.com/kb?id=kb_article_view&sysparm_article=KB0867184) article in the Now Support knowledge base.

You can migrate your data from Legal Simple Contracts to Contract Management Pro to start using the additional features available.

For more information on migration, see [Migrating to Contract Management Pro for Legal Service Delivery](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/employee-service-management/legal-simple-contracts/migration-landing-page.md).

**Important:** The migration script is designed only for migrating data from Legal Simple Contracts to the Legal Contract Repository, which applies specifically to Contract Management Pro for Legal Service Delivery.

Contract Management Pro for Legal Service Delivery is a licensed application that must be installed separately.

**Parent Topic:**[Contract Management Pro reference](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/employee-service-management/contract-management-pro/cncore-ref.md)

**Related topics**  


[Components installed with Contract Management Pro]()

[Components installed with Contract Workspace]()

[Components installed with Analytics Pack for Contract Management Pro]()

[Contract request State and Contract document status in Contract Management Pro]()

[Clause Variation form]()

[Signatory roles]()

[Contract Configuration form]()

[Properties installed to configure expiry notifications]()

[Properties installed to configure contracts integrations]()

[Expiring Contracts Condition form fields]()

[Action assignment form]()

[UFX Add on Event mapping form]()

[Obligation form]()

[Obligation Management notifications]()

[Contract Management Pro glossary]()

