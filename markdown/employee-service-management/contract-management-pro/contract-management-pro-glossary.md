...
A Now Assist skill that is used to extract the metadata from signed contracts and add the information to the system, optimizing the data entry process.

### Microsoft Word Add-in

A program that integrates with Microsoft Word that enables a user to add the content controls for the metadata, clauses, signature blocks, and dynamic tables.
...
A catalog item that allows end users to create records from Service Catalog.
