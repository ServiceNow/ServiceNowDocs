---
title: Use Guided Self-Service
description: Use the Guided Self-Service experience to self-serve, leading to improved productivity, and operational efficiencies for the organization.​
locale: en-US
release: xanadu
product: Employee Experience Foundation
classification: employee-experience-foundation
topic_type: concept
last_updated: "2024-08-01"
reading_time_minutes: 3
breadcrumb: [Use, Employee Center, Employee Service Management]
---

# Use Guided Self-Service

Use the Guided Self-Service experience to self-serve, leading to improved productivity, and operational efficiencies for the organization.​

With an in-context visual way to navigate the complex work scenarios, you can enhance productivity, engagement, and contribute to the overall organizational efficiency.

-   Self-serve with easy access to information, processes, and resolutions for day-to-day work.
-   Discover and use the available GSS process from the search.
-   Navigate to the right article or catalog form through a visual QnA experience.
-   Access information, services, and technology intuitively​.

As an employee, you can select options, view or modify the choices, restart activities, and start entire playbook all over again. Here is the intuitive step-by-step flow for the use cases.

-   Start afresh with context when you return to a partially completed GSS flow.
-   Ensure you complete the entire GSS flow in one-go.
-   Go back, resume, view, and revise your selections.
-   Access a catalog or a KB article based on your permissions.

**Note:** Your incomplete selections are cancelled daily by a schedule job.

Here is the intuitive step-by-step flow for the use cases.

![Know your benefits - Guided Self-Service in Employee Center and its intuitive uses.](../images/gss-guided-help-employee.gif "Guided Self-Service: Know your benefits flow")

![Leave request in Guided Self-Service in Employee Center and its intuitive uses.](../images/gss-guided-help-employee-leave-flow.gif "Guided Self-Service Start over")

For more information on how to configure the Guided Self-Service experience, see [Guided Self-Service in Employee Center](gss-guided-self-service-overview.md).

**Parent Topic:**[Using Employee Center](using-emp-center.md)

**Related topics**  


[Use the Employee Center topic pages](ec-topic-use.md)

[Use the integrated service and experience feedback](using-ex-fdback.md)

[Topic managers and contributors for topic page management](topic-managers-and-contributors-for-topic-page-management.md)

[Use the My To-dos page](../task/ec-to-dos-use.md)

[View advanced portal navigation](adv-portal-navigation-employee.md)

[Use approval experience](../task/ec-to-dos-use-approval-hub.md)

[Manage approvals from Microsoft Teams](../../sn-teams/task/va-concur.md)

[Manage approvals from a Microsoft Outlook email](../../sn-ms-outlook/task/oam-concur.md)

[Task filters on My tasks](configurable-filters-experience.md)

[View the Recommended for you content](rfy-employee.md)

[Use Employee Center from Zoom](../task/zoom-create-email-notification.md)

[View employee profile](update-employee-profile.md)

[Use Personalized Answers](using-personalized-search-on-employee-center.md)

[Employee Profile org chart widget](../task/view-org-chart.md)

[RTL support for Employee Center](ec-rtl-language-support.md)

[Manage favorites](../task/web-configure-favorites-employee.md)

[Access applications from App Launcher](web-application-employee.md)

[Use Employee Center Pro Kiosk](deskless-eck-homepage.md)

