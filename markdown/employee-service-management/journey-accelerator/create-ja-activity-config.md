...
Use the application picker to set the appropriate scope. Employee Center. See, [Application picker](https://www.servicenow.com/docs/access?context=c_ApplicationPicker&version=zurich&pubname=zurich-application-development&ft:locale=en-US)

The name of the activity to configure. Use the name of the activity created in the [Add a Journey Accelerator action plan to My active items](add-ja-action-plan-myitems-list.md) task example `Action plan items`.
