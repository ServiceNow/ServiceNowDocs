---
title: List view in Recruitment workspace
description: The List view in the Recruitment workspace displays all assigned job requisitions, interviews, tasks, and their key details.
locale: en-US
release: zurich
product: Recruiter Workspace
classification: recruiter-workspace
topic_type: concept
last_updated: "2025-07-31"
reading_time_minutes: 1
breadcrumb: [Explore, Recruitment Workspace, Hiring Experiences, HR Service Delivery, Employee Service Management]
---

# List view in Recruitment workspace

The List view in the Recruitment workspace displays all assigned job requisitions, interviews, tasks, and their key details.

Using the list categories and filters, you can quickly find the required job requisition, interview, or task that you must work on.

To get to the list view, you must select the list icon \(![List icon.](../../ohs-incident-management/image/list-icon-hs.png)\). When you select a record in a list view, the record opens in a new tab.

## Lists

The Lists view enables you to manage your Recruitment workspace and all your own entities. You can view the following items in the **Recruitment workspace** &gt; **List** view:

-   Job requisitions
-   Applications
-   Interviews
-   Scheduling preferences
-   Interview feedback
-   Tasks

## My Lists

Any list that you create appear in the My Lists section. Lists in this section are only visible to you.

