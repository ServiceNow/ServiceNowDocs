...
6.  Compose the SMS message, add an attachment if necessary, and send to the employee.
    ...
    You can also select the ![HR Agent Workspace - Quick action icon](../reference/images/quick-action-icon.png) quick action icon, or enter a back slash \(/\) in the chat area, to use quick actions for SMS messaging. See [Agent Chat](https://www.servicenow.com/docs/access?context=ci-agent-chat-exploring&version=australia&pubname=australia-conversational-interfaces&ft:locale=en-US) for more information.
    ...
    **Note:** For complete details regarding initiating SMS conversations in Agent Workspace, refer to [Agent initiated SMS conversation](https://www.servicenow.com/docs/access?context=agent-initiated-sms-store-app&version=australia&pubname=australia-conversational-interfaces&ft:locale=en-US). For details about using Agent Workspace with HR Service Delivery, refer to .
