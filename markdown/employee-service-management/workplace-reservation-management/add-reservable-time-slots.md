...
Add the time slot to the reservable module. The time slot will be displayed in the selection list when an employee wants to reserve a workplace item from the reservable module that has this time slot. For more information, see [Add a time slot to a reservable module](add-timeslot-to-reservable-module.md).
