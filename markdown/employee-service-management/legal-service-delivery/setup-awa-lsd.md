...
4.  Define or change a queue so that you can determine which work items are routed automatically to agents through a given service channel.
    ...
    -   Ethics Complaint
    **Note:** A queue is a unique alphanumeric identifier assigned to an application. If the application does not appear in the queue table, verify that its queue is not being used by another application. If it is already in use, create a new queue. For more information, see Create a work item queue.
...
6.  Use Skill Determination Rules as an alternative way to map skills to an legal request case.
    ...
    If you are going to use skill determination rules to map a skill to an legal request, activate Skills Management. For more information, see Skills Management.
...
7.  Configure the following parameters to improve your Advanced Work Assignment functionality:
    ...
    1.  Create or modify the availability states that agents use to indicate whether they can receive work or are offline or away. Agents set these states in their Workspace Inbox. For more information, see Configure agent presence states. Add the service channel to
