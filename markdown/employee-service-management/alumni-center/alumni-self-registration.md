---
title: Register on the Alumni Center website
description: As an external user, sign up using a personal email address and other details to self-register as alumni.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/employee-service-management/alumni-center/alumni-self-registration.html
release: zurich
product: Alumni Center
classification: alumni-center
topic_type: task
last_updated: "2025-07-31"
reading_time_minutes: 1
breadcrumb: [Use, Alumni Center, HR Service Delivery, Employee Service Management]
---

# Register on the Alumni Center website

As an external user, sign up using a personal email address and other details to self-register as alumni.

## Before you begin

Role required: sn\_asc.alumni

## Procedure

1.  Navigate to Alumni Center website.

2.  Select **Register as alumni** on the login page.

3.  Fill in the fields on the alumni self-registration form.

<table id="table_cg3_jbf_5fc"><thead><tr><th>

Fields

</th><th>

Description

</th></tr></thead><tbody><tr><td>

First name

</td><td>

Your first name.

</td></tr><tr><td>

Last name

</td><td>

Your last name.

</td></tr><tr><td>

Personal email address

</td><td>

Your personal email address.**Note:** The First name, Last name, and Personal email address fields are required. The email address that you provide here is verified with the existing personal email addresses in your HR profile to create the alumni record.

</td></tr><tr><td>

Employee number

</td><td>

Your employee number in your previous organization.

</td></tr><tr><td>

Employment start year

</td><td>

Beginning year of your employment.

</td></tr><tr><td>

Employment end year

</td><td>

Terminating year of your employment.

</td></tr><tr><td>

Date of birth

</td><td>

Your date of birth.

</td></tr><tr><td>

Country code

</td><td>

The country code for your mobile number.

</td></tr><tr><td>

Mobile number

</td><td>

Your mobile phone number.

</td></tr><tr><td>

Street

</td><td>

Your street name.

</td></tr><tr><td>

City

</td><td>

Your city name.

</td></tr><tr><td>

State / Province

</td><td>

Your state or province.

</td></tr><tr><td>

Zip / Postal code

</td><td>

Your postal or zip code.

</td></tr><tr><td>

Country

</td><td>

Your country.

</td></tr></tbody>
</table>4.  Select the check box to agree to the privacy policies and terms and conditions.

5.  Select **Sign Up**.


## Result

A mail is sent to your personal email, which contains a verification link. After you select the link, a staging record is created for you so that the admin can verify your details and move the record to Approved or Rejected status.

-   If approved, the user ID and password are communicated to you via the personal email address provided.
-   If rejected, you receive an email to fill in the required details by selecting to register again, and the verification process is repeated.

**Parent Topic:**[Using your Alumni Center](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/employee-service-management/alumni-center/asc-use.md)

**Related topics**  


[Alumni job preferences]()

[Add other employment details]()

[Edit personal details]()

