...
Set the application scope to **Legal Request Management** in the application picker. For more information, see [Application picker](https://www.servicenow.com/docs/access?context=c_ApplicationPicker&version=xanadu&pubname=xanadu-application-development&ft:locale=en-US).
...
Cross-scope record that is accessing a restricted application resource.-   Select Lookup Documents by using the list icon \(![Lookup Documents using list icon](../../contact-tracing/image/icon-lookup.png)\).
