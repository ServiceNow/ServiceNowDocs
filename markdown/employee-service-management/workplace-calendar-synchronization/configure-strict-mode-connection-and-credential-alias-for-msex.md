...
2.  Check if the OAuth entity profile is set as follows:
    ...
    2.  To generate the OAuth token, select the **Get OAuth Token** related link.
        ...
        In the Microsoft Exchange login page that appears, login with the strict mode user email credentials. You must be prompted for credential. When asked, specify the strict mode email credentials.
