...
-   Only the reservations made on spaces and rooms that have Reservable sync configuration are exported. For more information on how to add a Reservable sync configuration for a room or a space, refer to [Add a Reservable Sync Configuration](add-reservable-sync-config.md).
