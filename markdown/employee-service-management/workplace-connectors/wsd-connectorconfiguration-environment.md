...
Target or destination table to store the environmental sensor data.Select Occupancy and Environment Data \(sn\_wsd\_wc\_occ\_env\_data\) table. For more information, see [Configure environmental sensor data for a workplace location](config-connectors-env-sensor.md).
...
Search for extension point definition for the environmental data. Search using the lookup list icon \(![Look up list icon to search for required extension point definition.](../images/wsd-lookup-list-icon.png)\) to locate the extension point definition **OccupancyEnvironmentDataHandler**.
