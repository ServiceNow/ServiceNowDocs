...
Actions are reusable operations to automate processes. Actions are defined in the Flow designer Metrikus spoke integration. For more information, see[Metrikus spoke](https://www.servicenow.com/docs/access?context=metrikus-spoke&version=yokohama&pubname=yokohama-integrate-applications&ft:locale=en-US)Select **Query Space Occupancy**: The script for action configurations checks the workplace space for occupancy status. For example, **Occupied**, **Unoccupied**, **Invalid**, and so on.
...
Invokes sub flows from the Flow designer for Metrikus spoke integration. For more information, see [Metrikus spoke](https://www.servicenow.com/docs/access?context=metrikus-spoke&version=yokohama&pubname=yokohama-integrate-applications&ft:locale=en-US)Select **Subflow**.
