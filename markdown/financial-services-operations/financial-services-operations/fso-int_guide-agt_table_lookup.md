...
1.  The agent enters the required information on the FSO form. Using this information, formulate the REST call /api/getTransactions using the [RESTMessageV2](https://www.servicenow.com/docs/access?context=c_RESTMessageV2API&version=xanadu&pubname=xanadu-api-reference&ft:locale=en-US) API and send it to the remote bank application to obtain the customer's financial transaction details.
2.  The remote bank application processes the request and returns the specified customer's financial transaction details.
3.  The financial transaction details are cached on the ServiceNow instance in remote tables using the [v\_table](https://www.servicenow.com/docs/access?context=v_tableAPI&version=xanadu&pubname=xanadu-api-reference&ft:locale=en-US) API.
4.  The transaction details are then displayed on the FSO form.

</td></tr></tbody>
</table>## Example code

The follow code example shows how to use the [RESTMessageV2](https://www.servicenow.com/docs/access?context=c_RESTMessageV2API&version=xanadu&pubname=xanadu-api-reference&ft:locale=en-US) API to create and execute the REST call to the external bank application. It then store the return results in a [remote table](https://www.servicenow.com/docs/access?context=remote-tables&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US) using the [v\_table](https://www.servicenow.com/docs/access?context=v_tableAPI&version=xanadu&pubname=xanadu-api-reference&ft:locale=en-US) API. Before you can use the v\_table API you must activate the Remote Tables \(com.glide.script.vtable\) plugin.
