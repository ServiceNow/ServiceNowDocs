...
|-----------------|----------------|---------------|-----------------------|----------------|
|Stage 1: Pre-Dispute|Submit Transaction Inquiry|Enables you to search for transactions using specific criteria. Requires a tokenization solution \(see Token Provider note above\).|`SISubmitTranInquiryRequest`|`/rsrv_rolti/api/SISubmitTranInquiry`|
...
|Batch Queues|Batch Queues Flows Adapter|Adapter that includes all batch queue processes. See Batch Queue APIs section below for the individual queue types.|`SIGetBatchQueueRequest`|`/rsrv_rolsi/api/SIGetBatchQueue`|
