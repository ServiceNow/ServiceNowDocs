...
User name that the proxy uses to log in to the instance. The user must have the edge\_encryption role. See [Set up an Edge Encryption user account](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-security/edge-encryption/t_SetupEdgeEncryptionUser.md).
