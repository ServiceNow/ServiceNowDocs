...
Calling this function on a JSON node tells the proxy that the value is an [Encoded query strings](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/platform-user-interface/c_EncodedQueryStrings.md) for the specified table. The proxy parses the encoded query and encrypts the values for fields in the encoded query that must be encrypted.
