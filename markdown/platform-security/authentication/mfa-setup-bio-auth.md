---
title: Register a biometric authenticator
description: Register a biometric authenticator to use as part of your multi-factor authentication login.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/platform-security/authentication/mfa-setup-bio-auth.html
release: xanadu
product: Authentication
classification: authentication
topic_type: task
last_updated: "2024-08-01"
reading_time_minutes: 1
breadcrumb: [Web Authentication, Using Multi-factor authentication \(MFA\), Multi-factor authentication \(MFA\), Authentication, Access Management]
---

# Register a biometric authenticator

Register a biometric authenticator to use as part of your multi-factor authentication login.

## Before you begin

Role required: none

## Procedure

1.  Navigate to **All** &gt; **Self-Service** &gt; **My Profile**.

2.  Under **Related Links**, click **Multi-factor Authentication**.

    The multi-factor authentication page opens.

3.  Click **Register biometric authentication**.

    \[Omitted image "mfa-authenticator.png"\] Alt text: MFA Authenticators

4.  Enter a nickname for your authenticator and click **Register**.

    \[Omitted image "mfa-biometric.png"\] Alt text: MFA Biometric setup

5.  When prompted, follow the instructions on the screen to authenticate with your biometric authenticator.

    This message will vary based on your specific authenticator.

    \[Omitted image "bio-request.png"\] Alt text: Request for biometric authentication

    After authenticating successfully, you see a confirmation window. Click X to close the confirmation.


## Result

Your biometric authenticator is registered. You can see the biometric authenticator listed on the Multi-factor authentication page.

