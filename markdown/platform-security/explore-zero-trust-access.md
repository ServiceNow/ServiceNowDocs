...
-   Reduce access based on IDP response for a user session, if the user is using an untrusted device. For more information, see [Configure IDP attribute for Session Access](../task/configure-idp-session-access.md).
...
For more information, see [Configure Zero Trust Access for mobile](https://www.servicenow.com/docs/access?context=configure-zero-trust-access-mobile&version=xanadu&pubname=xanadu-mobile&ft:locale=en-US).
