---
title: Sign the JDBC data source records in the production instance
description: Use update sets to sign and validate the JDBC data sources by enabling the code signing in production and trusted non-production instances.Use update sets to bring mass signing jobs to the production instance.Use update sets to bring the signed update set to the production instance.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/platform-security/cs-jdbc.html
release: xanadu
topic_type: concept
...
1.  In the trusted non-production instance, configure the KMF signing job to sign the data sources.
...
1.  In the trusted non-production instance, start an update set.
