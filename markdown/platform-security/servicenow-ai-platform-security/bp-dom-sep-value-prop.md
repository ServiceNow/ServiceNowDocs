---
title: Domain separation value proposition
description: With domain separation, service providers can have a multitenant instance architecture that delivers offerings efficiently and securely to their clients. Strong universal process standards, data-driven process design, strict governance, and centralized administration help to maximize these benefits.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/platform-security/servicenow-ai-platform-security/bp-dom-sep-value-prop.html
release: yokohama
product: ServiceNow AI Platform Security
classification: servicenow-ai-platform-security
topic_type: reference
last_updated: "2025-01-30"
reading_time_minutes: 1
breadcrumb: [Domain separation explained, Domain separation recommended practices for service providers, Domain separation for service providers, Access Management]
---

# Domain separation value proposition

With domain separation, service providers can have a multitenant instance architecture that delivers offerings efficiently and securely to their clients. Strong universal process standards, data-driven process design, strict governance, and centralized administration help to maximize these benefits.

\[Omitted image "sp-landing-gif-6.gif"\] Alt text: Opportunities for service providers

## Benefits of domain separation

The tenants of a domain benefit from a quick return on investment, lower administrative overhead, and leverage of business services provided by instance owners.

Here is an quick view of these benefits.

|Instance Owner|Domain Tenants|
|--------------|--------------|
|Service provider employee productivity|Increased security|
|Only process deltas maintained|Pre-built processes and features|
|Administration efficiencies|Reduced staff required|
|Fewer client integrations|Faster onboarding|
|Upgradability and scalability|Leverage of the latest releases|
|Data segregation|Services provided by the instance owner|
|Global reporting| |

**Parent Topic:**[Domain separation explained](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/platform-security/servicenow-ai-platform-security/bp-what-is-domain-separation.md)

