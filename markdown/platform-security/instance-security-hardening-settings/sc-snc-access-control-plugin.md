...
1.  To request the plugin, follow the steps in [Activating ServiceNow access control](../../security/task/t_ActivateSNCAccessControl.md). Customers must request the SNC Access Control plugin \(com.snc.snc\_access\_control\) from HI.
2.  To enable SNC access control, follow the steps in [Configuring ServiceNow access control](../../security/task/t_ConfigureAccessControl.md). Configure an access control record to specify one or more Customer Service and Support employees that have permission to log in your instance.
