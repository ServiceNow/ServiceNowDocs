...
-   Configure Session Access role to either reduce or remove roles for the users logging based on the policy inputs and conditions. To learn more about the configuration, see [Configure Session Access role](../task/configure-session-access-role.md).

The configuration evaluates the login to reduce or remove the roles of the users who access your ServiceNow® instance based on the policy filters and conditions. For more information, see [Configure Zero Trust Access for mobile](https://www.servicenow.com/docs/access?context=configure-zero-trust-access-mobile&version=zurich&pubname=zurich-mobile&ft:locale=en-US).
