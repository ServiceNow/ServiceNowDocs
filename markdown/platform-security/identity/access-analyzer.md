---
title: Access Analyzer
description: ServiceNow Access Analyzer is an access diagnostic tool that helps to view the permissions of an identity for a resource.
locale: en-US
release: xanadu
product: Identity
classification: identity
topic_type: concept
last_updated: "2024-08-01"
reading_time_minutes: 1
breadcrumb: [Identity]
---

# Access Analyzer

ServiceNow® Access Analyzer is an access diagnostic tool that helps to view the permissions of an identity for a resource.

<table id="table_iwv_lpv_klb" class="nav-card"><tbody><tr><td>

[Explore ![Explore](../../../common/image/icon-explore.png)](explore-access-analyzer.md)

 [Learn the features and business value of Access Analyzer.](explore-access-analyzer.md)

</td><td>

[Use ![Use](../../../common/image/icon-set-up.png)](../task/use-access-analyzer.md)

 [Understand how to use Access Analyzer.](../task/use-access-analyzer.md)

</td></tr><tr><td>

[Permission Evaluation ![Evaluation](../../../common/image/icon-manage.png)](permission-evaluation.md)

 [Know how the permissions are evaluated.](permission-evaluation.md)

</td><td>

[Frequently asked questions ![Reference](../../../common/image/icon-reference.png)](frequently-asked-questions.md)

 [Get details about commonly asked questions about Access Analyzer.](frequently-asked-questions.md)

</td></tr></tbody>
</table>