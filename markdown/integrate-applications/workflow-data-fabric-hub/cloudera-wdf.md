---
title: Cloudera
description: The Cloudera connector provides read-only access to data and metadata in Cloudera.
locale: en-US
release: zurich
...
The Cloudera connector provides read-only access to data and metadata in Cloudera.

A connection admin can set up a connection to Cloudera in the Workflow Data Fabric Hub and grant data stewards access to this connection. Data stewards can then use the established connection to create a data fabric table and map data from Cloudera. This allows users to access Cloudera data through the table list view or by using GlideRecord scripts. For details on creating data fabric tables and mapping data, see [Managing data fabric tables in Workflow Data Fabric Hub](managing-data-fabric-tables-wdf.md).
...
**Important:** Cloudera data types not included in the table aren't supported for data mapping in Workflow Data Fabric Hub.
...
[Create a Cloudera connection](../task/create-cloudera-connection.md)
