...
The Import Log tab displays the logs for the import set. It shows when each log was created, its level, the log message, and the source of the log. For details, see [Import logs](https://www.servicenow.com/docs/access?context=r_ImportLogs&version=zurich&pubname=zurich-platform-security&ft:locale=en-US).
