...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/integrate-applications/rpa-hub/use-user-certificates-rpa.html
release: xanadu
product: RPA Hub
...
If user certificates are generated by your organization, then you can skip this procedure. You can proceed with [installing the user certificate.](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/integrate-applications/rpa-hub/use-user-certificates-rpa.md)
...
You install the KeyStore Explorer application in your Windows machine and then generate a client authentication and code signing certificate from the KeyStore Explorer application. Skip this task, if you already have these certificates.
...
3.  From your desktop, double-click the KeyStore Explorer icon and select **Create a new KeyStore**.

4.  In the New KeyStore Type dialog box, select **JKS** as a type of the new KeyStore and select **OK**.

    The new KeyStore appears as an additional **Untitled** tab.

5.  On the **Untitled** tab, right-click the screen and select **Generate Key Pair**.

6.  In the Generate Key Pair dialog box, select **OK**.

7.  In the Generate Key Pair Certificate dialog box, enter a name for the certificate.

8.  In the Generate Key Pair Certificate dialog box, select the book icon against the **Name** field, and then do the following actions:

    1.  In the Name dialog box, fill in the details of your organization.

    2.  Select **OK**.

9.  In the Generate Key Pair Certificate dialog box, select **OK**.

10. In the New Key Pair Entry Alias dialog box, enter an alias name in the **Enter Alias** field and select **OK**.

11. In the New Key Pair Entry Password dialog box, do the following actions to generate a key pair:
...
12. On the **Untitled** tab, right-click this key, select **View Details**, and then select **Certificate Chain Details**.

13. In the Certificate Details for Entry dialog box, select the **PEM** button.

14. In the Certificate PEM dialog box, select **Export**.

15. Select a location to save this file as a `.pem` file and select **Save**.

    Use this file to upload in the instance for creating a Certificate Authority \(CA\) chain record and user record.

16. After the PEM is exported successfully, select **OK** and close all the tabs.

17. On the **Untitled** tab, do the following actions to generate a `.p12` file:
...
Add the user to manage private keys. For more information, see [Add user to manage private keys](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/integrate-applications/rpa-hub/use-user-certificates-rpa.md).
...
-   To set up the Unattended Robot. For more information, see [Set up Unattended Robot](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/integrate-applications/rpa-hub/set-up-rpa-runtime.md).
-   To publish an automation project in RPA Desktop Design Studio. For more information, see [Publish an automation project in RPA Desktop Design Studio](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/xanadu/markdown/xanadu/integrate-applications/rpa-hub/publish-automation-project.md).
