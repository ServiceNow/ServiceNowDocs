...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/integrate-applications/rpa-hub/switch-providers-rpa-bot-skill.html
release: xanadu
product: RPA Hub
...
1.  Create a connection record and add its associated credential record before you switch the provider.
    ...
    5.  In the **Credential** field, select the search icon \(\[Omitted image "List\_SearchIcon.png"\] Alt text: Search icon\), select **New**, and then select **Custom Header API Key Credential**.
...
2.  Clone the RPA bot generation skill.
    ...
    3.  On the RPA bot generation card, select the three dots icon \(\[Omitted image "three-dot-icon-vertical-rpa.png"\] Alt text: Vertical three-dots icon.\) and select **Make a copy**.

        \[Omitted image "clone-rpa-bot-gen-skill.png"\] Alt text: RPA bot generation card shows the procedure of cloning the RPA bot generation skill by selecting the three dots next to the skill and selecting the Make a copy option.
...
3.  Switch your provider.
    ...
    3.  On the **Skill settings** tab, in the Providers \(Provider API\) section, toggle the Custom LLM switch to make it the default provider.
        ...
        \[Omitted image "switch-custom-llm-rpa.png"\] Alt text: Providers section that shows the Custom LLM provider and the Make default toggle is selected to make it the default provider.
