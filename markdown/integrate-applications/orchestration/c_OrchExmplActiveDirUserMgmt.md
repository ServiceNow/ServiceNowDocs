...
The activities in the [Active Directory activity pack](https://www.servicenow.com/docs/access?context=c_OrchActiveDirectoryActivities&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US) are designed to manage user accounts and reset user passwords. The following activities cover the most common use cases administrators encounter when managing Active Directory user accounts:
...
11. Attach both activity outcomes \(Success and Failure\) to the end point.
    ...
    **Note:** In a normal workflow, some type of alternate action is desirable upon failure. For example, you might send an [Email and SMS notifications](https://www.servicenow.com/docs/access?context=c_EmailNotifications&version=xanadu&pubname=xanadu-platform-administration&ft:locale=en-US) if the workflow failed to update the record.
