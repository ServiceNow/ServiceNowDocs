...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/australia/integrate-applications/erp-integration-framework/exploring-erp-models.html
release: australia
...
-   Platform models map ERP input and output fields to existing ServiceNow platform tables, standardizing ERP data into the data structure of the ServiceNow AI Platform for tighter integration.
...
Through input and output parameter mapping, you define which ERP fields are used and what the model sends back to the ERP system on reads, updates, and creates. This means sensitive fields never reach remote or extraction tables. The fields are excluded at the model level before any data flows.

Models support a range of underlying connection methods \(table reads, BAPIs, RFC, OData, IDoc, and REST APIs\). You can match the method to what's available and appropriate for the ERP module and operation type you're working with.
...
A developer is building a vendor management application and needs to retrieve vendor data from SAP, enable updates to vendor payment terms, and create vendor records.

Instead of building three separate integrations, the developer creates a single vendor model connected to the appropriate SAP system. In the model manager, they add three operations:

-   A read operation using an RFC/BAPI entity to retrieve vendor details.
-   An update operation using OData to write payment term changes back to SAP.
