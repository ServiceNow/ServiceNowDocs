...
|Idle CPU|Percentage of the CPU's capacity that is not being actively utilized or is in an idle state.|
|Memory usage|Amount of computer memory \(RAM\) currently being utilized by running processes and applications.|
...
Process ID \(PID\) is a unique numerical identifier assigned to each running process. **Note:** The PID can be used to monitor and manage running processes. The PID can also be used to terminate or kill a specific process if necessary.
