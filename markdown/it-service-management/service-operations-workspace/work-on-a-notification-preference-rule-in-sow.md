---
title: Manage delivery channels
description: Add, edit, and delete delivery channels for notification preference rules in Service Operations Workspace.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/it-service-management/service-operations-workspace/work-on-a-notification-preference-rule-in-sow.html
release: zurich
product: Service Operations Workspace
classification: service-operations-workspace
topic_type: task
last_updated: "2025-01-30"
reading_time_minutes: 1
breadcrumb: [Create notification preference rule, Configuring On-Call Scheduling in Service Operations Workspace, On-Call Scheduling in Service Operations Workspace, Manage, Service Operations Workspace for ITSM, IT Service Management]
---

# Manage delivery channels

Add, edit, and delete delivery channels for notification preference rules in Service Operations Workspace.

## Before you begin

Role required: rota\_manager, rota\_admin

## Procedure

1.  Navigate to **Workspaces** &gt; **Service Operation Workspace**.

2.  Select the **Schedules** icon.\[Omitted image "image.schedules"\] Alt text: schedules icon

3.  On **Schedules** page, select **On-call Notification preferences** and select **Edit** to add or edit or delete Delivery channels.

4.  Select **Add New** to add channel and provide name and email ID.

5.  Select **Phone numbers** tab to add and edit phone numbers.

6.  To delete a delivery channel, select the channel from the list and select **Delete** icon.

    **Important:**

    Deleting a delivery channel removes it from all notification preference rules that reference it. Verify that no active rules depend on the channel before deleting it.

7.  Select **Save** to apply your changes.


**Parent Topic:**[Create notification preference rule](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/it-service-management/service-operations-workspace/create-notification-preference-rule.md)

