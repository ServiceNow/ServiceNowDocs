...
Depending on the remedial action chosen from the list, specific input parameters are listed which will fetch a static input. To fetch data from the table, enable and configure the advanced settings of the input parameters to capture the data during the run time. Find the following details of all the advanced settings of one of the out-of-the-box remedial action input parameters for Clear application cache. On saving the resolution, we should save parameter information in the `sn_pren_resolution table` remedial\_action\_parameters field
