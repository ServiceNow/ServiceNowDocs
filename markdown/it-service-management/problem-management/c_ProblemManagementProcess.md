...
If the Problem Management team has a problem model process for dealing with certain problems, they can be added to the system with [workflows](https://www.servicenow.com/docs/access?context=c_WorkflowOverview&version=zurich&pubname=zurich-build-workflows&ft:locale=en-US) . Workflows standardize and automate a process.

As a problem is updated, [email notifications](https://www.servicenow.com/docs/access?context=c_EmailNotifications&version=zurich&pubname=zurich-platform-administration&ft:locale=en-US) are sent to concerned parties. If inbound email actions are specified, the problem can be updated via email.
...
-   [Inactivity monitors](https://www.servicenow.com/docs/access?context=t_SetAnInactivityMonitor&version=zurich&pubname=zurich-platform-administration&ft:locale=en-US) : The inactivity monitors prevent problems from being overlooked by generating an event. When a problem has not been updated within a certain amount of time, the event can create an email notification or trigger a script.
