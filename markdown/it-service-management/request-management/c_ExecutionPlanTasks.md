...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/it-service-management/request-management/c\_ExecutionPlanTasks.html
release: brazil
...
4.  Fill in the fields for that approval task.
    ...
    \[Omitted image "image.ExecutionPlanApprovalTasks1"\] Alt text:
...
7.  Select **Update** to add the task to the execution plan.
    ...
    \[Omitted image "image.Rollback1"\] Alt text: Screenshot for Rollback1

    Next, complete the first two steps, which leads to the approval task.

    \[Omitted image "image.Rollback2"\] Alt text: Screenshot for Rollback2

    Reject the request to roll back the execution plan to a previous task, and reset any intermediate tasks to pending:

    \[Omitted image "image.Rollback3"\] Alt text: Screenshot for Rollback3
