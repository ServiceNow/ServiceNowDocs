...
1.  Create a Performance Analytics \(PA\) indicator and its data collection job that measures the daily score \(count\) of the metric that you want to measure.
    ...
    For information on permissions required to create a PA indicator, see [Performance Analytics roles](https://www.servicenow.com/docs/access?context=r_PARoles&version=xanadu&pubname=xanadu-now-intelligence&ft:locale=en-US) for more details.
