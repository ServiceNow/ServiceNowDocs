...
The create outages for a change request agentic workflow lets users quickly create outages for a change request from the ServiceNow Otto panel.
...
To modify the create outages for a change request agentic workflow, [duplicate it](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/intelligent-experiences/clone-aia-usecase.md), and adjust the settings according to your requirements.
...
4.  Access and use the create outages for a change request agentic workflow from the ServiceNow Otto panel.

    You can query in the ServiceNow Otto panel with the change request number. For the query instructions, read the steps specified in the **List of steps** field of the corresponding AI agent \(change outage assistant AI agent\).

    \[Omitted image "now-assist-change-outage-nap.png"\] Alt text: Create outages for a change request agentic workflow output in the ServiceNow Otto panel.
