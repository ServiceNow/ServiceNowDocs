...
|-------|---------------|
|Upload file formats such as text, PDF, Word \(.doc, .docx\) and image files such as JPEG and PNG.|Upload documents in a chat.|
|Combine AI Search with generative AI chat skills to speed up issue resolution and reduce deflection to a live agent using Now Assist for ITSM in Virtual Agent.|Enhanced chat|
|Use the structured and unstructured data from ServiceNow records, knowledge bases, and external sources to enhance the performance of Now Assist in Virtual Agent, AI agents, and generative AI skills using Knowledge Graph.|Knowledge Graph|
|Define crawl settings for external content connectors to crawl source systems and send it to AI Search for indexing.|Configuring External Content Connectors|
...
2.  Set up Now Assist in Virtual Agent.
    ...
    For more information on how to turn on the skills and set up generative AI for Virtual Agent, see Configuring assistants overview.
