---
title: Open an IT ticket in Employee Center
description: Resolve a user's issue by opening an IT ticket.
locale: en-US
release: australia
topic_type: task
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Employee experience, Simplified IT Service Management, IT Service Management]
---

# Open an IT ticket in Employee Center

Resolve a user's issue by opening an IT ticket.

## Before you begin

Role required: none

## Procedure

1.  Navigate to the Employee Center portal.

2.  Select **Open IT Ticket**.

    The AI-enabled record producer, Create incident with Now Assist, is displayed.

3.  In the **Please describe your issue below** field, enter the issue information.

    While you’re specifying the issue, Now Assist generated solutions are displayed.

4.  In the **Add attachments** field, select a file related to the issue.

    ![Create incident in employee portal](../image/AINativeCreateIncident.png)

5.  Select **Save as Draft** to save the ticket as a draft request to update and submit later.

6.  Perform one of the following actions.

<table id="choicetable_mky_ybb_xhc"><thead><tr><th align="left" id="d239210e121">

Choice

</th><th align="left" id="d239210e124">

Description

</th></tr></thead><tbody><tr><td id="d239210e130">

**If the related content is relevant**

</td><td>

Select **Solution found**.

</td></tr><tr><td id="d239210e142">

**If the related content is not relevant**

</td><td>

1.  Select **Proceed with incident creation**.
2.  Specify the relevant information such as urgency and select **Submit**.


</td></tr></tbody>
</table>
**Parent Topic:**[Employee experience in Simplified IT Service Management](../concept/employee-experience-native-ai-itsm.md)

