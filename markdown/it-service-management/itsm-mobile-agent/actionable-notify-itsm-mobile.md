...
4.  Click the lock icon \(![Lock icon](../image/lock-icon.png)\) next to the **Actions** field.

5.  Click the reference icon \(![Reference icon](../image/reference-lookup-icon.png)\) to display the **Push Actions** list and select the actions for the push action category.
...
3.  Create a function for one of your push actions.
    ...
    -   URL
    -   Chat launcher
    For information on function types, and detailed steps for creating functions, see [Mobile functions](https://www.servicenow.com/docs/access?context=sg-studio-mobile-button-types&version=xanadu&pubname=xanadu-mobile&ft:locale=en-US).
