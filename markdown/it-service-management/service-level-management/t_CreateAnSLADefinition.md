...
description: Create SLA definitions to set the conditions under which SLAs attach, pause, stop, and reset on task records. Use them to track and enforce service commitments across your organization.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/australia/it-service-management/service-level-management/t\_CreateAnSLADefinition.html
release: australia
...
When you create an SLA definition, avoid using dot-walked fields that change frequently in any of the SLA conditions \(start, stop, pause, and reset\). The SLA engine features, such as SLA Timeline and SLA Repair, replay the audit history of the Task record that the task\_sla is attached to. The SLA engine does not replay the audit history of any dot-walked fields from that Task, rather only considers the final state of the dot-walk fields.

For example, a pause condition is set to an incident dot-walk field `incident.company.cost_center`. The value of the Cost Center is first set to Finance and later changed to Engineering. When the SLA repair runs, only the final value of the Cost Center is considered. So, if the SLA pause condition is: `incident.company.cost_center is Engineering`, SLA Repair pauses immediately, because SLA Repair does not consider the previous Cost Center value: Finance. This condition also applies to SLA Timeline \(and when executing the SLA Engine asynchronously\).

If the requirement is to pause when incident.company.cost\_center is Engineering and not Finance, then the audit history must be on the Task record using a custom field. Create a custom field \(in this example on the incident table\) such as: u\_company\_cost\_center and a business rule to populate the field when either the company changes or the Cost Center of the company changes. Instead of setting the SLA pause condition to the dot-walked field, set it against the custom field. This approach helps that the task\_sla, SLA Repair, and SLA Timeline show consistent results.
...
Table that determines the records tracked by the SLA. SLAs can be defined for any table that extends the task table, including incident, change request, and service catalog tasks. You can also create an SLA definition for configuration items \(CIs\) and business services for Event Management.
...
Option to activate debug logging for this SLA definition only.The debug logging information includes details of the conditions that have matched or not matched, and the before and after values for the task SLA and task records.
...
The length of time the SLA runs before it is marked **Breached**. This field appears only when the duration is **User specified duration**.
...
This field appears only when the relative duration is selected in the **Duration type** field. For more information, see [SLA duration types](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/australia/it-service-management/service-level-management/c_SLADuration.md).
...
-   **Task field**: The option label reflects the table selected in the **Table** field. For example, if **Incident** is selected, this option appears as **Incident field**. If the **Task table field** option is selected, the **Schedule source field** list appears.
...
For more information, see [Time zones in SLAs](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/australia/it-service-management/service-level-management/c_TimeZonesInSLAs.md).
...
**Parent Topic:**[Configure Service Level Agreement \(SLA\)](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/australia/markdown/australia/it-service-management/service-level-management/c_ConfigureSLAs.md)
