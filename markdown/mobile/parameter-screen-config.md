...
Tapping the icon opens a page that lists the validation errors, enabling users to correct these errors and submit an error-free input form. The list refreshes only after it is resubmitted.![Error indicator page in an input form screen listing 3 error messages and 3 info messages.](../image/ifs-error-list.png)
...
Option to display the page progress menu after tapping the page indicator button![Page indicator button showing that the user is on page one of three.](../image/ifs-page-progress.png).The page indicator button displays when an input form contains more than 2 pages. Tapping the button opens a page progress menu that lists all available pages and their level of completeness. The level of completeness is measured by the number of completed mandatory fields. This list helps users track their progress when completing an input form.
...
![Page progress menu showing items that need completing on six pages of an input form screen.](../image/ifs-progress-menu.png)
