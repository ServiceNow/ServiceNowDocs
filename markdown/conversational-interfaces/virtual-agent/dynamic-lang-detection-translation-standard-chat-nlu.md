---
title: Language detection and dynamic machine translation in Virtual Agent standard chat conversations
description: Language detection and machine translation work together in Virtual Agent to improve the chat experience for users across languages. You can designate fallback languages for locales or dynamically translate languages that aren't professionally localized or aren't supported in topic discovery.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/conversational-interfaces/virtual-agent/dynamic-lang-detection-translation-standard-chat-nlu.html
release: brazil
...
Results in Virtual Agent LLM standard chat conversations

</th></tr></thead><tbody><tr><td>

Language detection and Dynamic Translation aren't configured.

</td><td>

For topics that haven't been translated into other languages, the language they appear in depends on the language they were authored in. Typically, the default system language is used for topics and topic discovery.
...
Language responses can show up differently depending on whether those topics have been translated. The conversation language may continue in the detected language if translated content for those topics already exists. If translated content doesn't exist, the conversation continues in the preferred conversational language.
 ...
 If the user selects **No**, Virtual Agent does not prompt them again during the conversation.
...
If the detected language is enabled for dynamic translation in Virtual Agent Chat Settings, Virtual Agent asks the user if they want to switch the conversation to the detected language.
 ...
 For example, the preferred conversational language is in English but the user types an utterance in Spanish. The user receives a prompt asking whether they'd like to switch the conversation to Spanish. The prompt reads, `I have detected you're typing in Spanish. Would you like to switch the conversation to Spanish? (Yes/No)`

 If the user selects **No**, Virtual Agent does not prompt them again during the conversation and the conversation continues in English.

 If the user selects **Yes**, the user prompt is translated to the default language. The default language for LLM conversations is within the **com.glide.cs.llm.default.Language** system property. All topic matches are translated dynamically into the user's conversation language and displayed. In this example, the conversation continues in Spanish and Virtual Agent expects the user to continue chatting in Spanish. After the conversation ends and the user restarts a new conversation by reloading their browser or initially navigating to a portal with no conversation in progress, the prior conversational language \(Spanish in this example\) continues.
...
-   After the conversation closes and restarts, the language used in the next conversation varies depending on how the user restarted the conversation. If the user restarts the conversation by reloading the browser, the preferred language is restored in the next conversation. If the user restarted the conversation by using the chat widget's restart feature, the previous conversational language is restored in the next conversation. Regardless of how a conversation has closed and restarted for standard chat conversations, if a user did not accept to switch languages after a different language was detected in the previous conversation, the preferred language is restored in the next conversation.
...
If Dynamic Translation is enabled on Virtual Agent but turned off for Agent Chat, the live agent sees what the user sees. If the user started the conversation in another language and the conversational language switches, then the live agent will see the conversation in the user's detected language. This may cause some confusion for both the agent and the customer. To resolve this issue, consider turning on Dynamic Translation for Agent Chat as well. For details, see [Configuring Dynamic Translation for Agent Chat](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/conversational-interfaces/agent-chat/configure-dtac.md).

## Supported languages

Install the ServiceNow language plugins for each language that uses dynamic translation. For a detailed list of language plugins, see [Activate a language](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/t_ActivateALanguage.md).

You can enable dynamic translation for languages that aren't currently supported on the ServiceNow platform. For details, see [Enable Dynamic Translation for an unsupported language](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/conversational-interfaces/virtual-agent/enable-dt-unsupported-language.md).
...
**Note:** The Virtual Agent topic test window may identify user inputs from Microsoft Teams or other adapters as `sysparm_search_text`. Dynamic Translation will translate this content as well.
