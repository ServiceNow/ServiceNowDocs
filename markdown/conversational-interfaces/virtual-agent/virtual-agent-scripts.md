...
Evaluates whether `myvar` matches an expected value. **Note:** Values for user input variables are assigned using user input controls. These values can’t be changed in your scripts.
 ...
 -   The Zurich release supports additional data types. For more information, see [Input data types in Virtual Agent topics](../reference/va-data-types.md).
 ...
 **Note:** NLU must be enabled on the topic. In the Virtual Agent Designer, select the **Properties** tab to set up NLU. For information about defining NLU entities, see [Entities](https://www.servicenow.com/docs/access?context=entities&version=zurich&pubname=zurich-intelligent-experiences&ft:locale=en-US).
...
![Sidebar in theVirtual Agent Designer canvas for manually adding script variables that are used in topic scripts.](../images/ScriptVariablesExample.png "Variables sidebar")
