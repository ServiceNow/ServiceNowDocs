...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/conversational-interfaces/virtual-agent/va-date-time-input.html
release: xanadu
...
Name that identifies this Date Time user input control node in the topic flow.
...
\[Omitted image "va-date-time-llm-2.png"\] Alt text: Basic properties include node name, detail description, and input format. Advanced options include Enforce user prompt, Additional instructions to LLM, and Hide or skip this node.
...
Name of the variable that stores the user response to this prompt. The variable name is automatically created from the **Name** property.
