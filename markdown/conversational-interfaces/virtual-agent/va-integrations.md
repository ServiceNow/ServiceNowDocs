...
description: When creating a topic or topic block, you can specify the chat channels in which the topic or topic block will run. During conversation design, Assistant Designer Asset library provides built-in guardrails to help you tailor your conversations to those channels.
locale: en-US
...
During conversation design, Virtual Agent checks the various controls used to build the conversation flow. This is to confirm that the control is valid for the channels in which the topic or block will run.

## Design checks for specified channels during conversation design

When you're designing your conversation, built-in guardrails identify the Assistant Designer Asset library controls that aren't suited for the custom channels that you specified in the Topic Properties page. These messages are displayed in the following views:
...
-   **Properties** tab \(Topic Properties page\)
    ...
    \[Omitted image "channel-support-prop-example1.png"\] Alt text: Assistant Designer Asset library topic properties page with defined channels.

-   **Flow** tab \(Assistant Designer Asset library canvas\)

    In the following example flow, the banner message identifies the controls that not supported for the specified channels. The property sheet for the affected nodes also displays a message indicating that the control is not supported. For example, when you highlight a particular node, the property sheet for the node indicates that the control is unsupported in Skype and SMS channels.
...
To see a list of the available channels in your instance, use the filter icon \(\[Omitted image "filter-icon.png"\] Alt text: Filter icon.\) in Assistant Designer Asset library. The Messaging Channels \[sys\_cs\_channel\] table also shows the available channels.
