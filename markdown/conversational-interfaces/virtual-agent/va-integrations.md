...
To see a list of the available channels in your instance, use the filter icon \(![Filter icon.](../images/filter-icon.png)\) in the Assistant Designer Asset library \(**Conversational Interfaces** &gt; **Virtual Agent** &gt; **Designer**\). The Messaging Channels \[sys\_cs\_channel\] table also shows the available channels.
