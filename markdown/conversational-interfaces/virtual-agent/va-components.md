---
title: Components installed with Virtual Agent
description: Several types of components are installed with activation of the Virtual Agent plugin, including tables, user roles, and scheduled jobs.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/conversational-interfaces/virtual-agent/va-components.html
release: brazil
...
# Components installed with Virtual Agent

Several types of components are installed with activation of the Virtual Agent plugin, including tables, user roles, and scheduled jobs.
...
Virtual Agent adds several roles for users operating as Virtual Agent administrators. The parent administrator role, Virtual Agent administrator \[virtual\_agent\_admin\], contains all the sub-roles available.
...
-   For multilanguage support, the localization\_requestor role is included by default. For more information, see [Localization Framework roles](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/platform-administration/roles-localization-framework.md).
-   Automated Customer Experiences \(ACE\) user \[sn\_ace.ace\_user\] role can access and read ACE records.
-   Workflow Studio \[action\_designer\] role launches the Workflow Studio app, to automate tasks with sequences of related steps.
-   Activity subscriptions administrator \[actsub\_admin\] role creates subscribable objects and subscribes to activities on those objects.
-   External application installation administrator \[external\_app\_install\_admin\] role installs external apps such as Slack and Microsoft Teams.

    Any user assigned this role can alter or delete topics created in Assistant Designer, including topics that other topics refer to or have as dependencies.

-   The flow designer \[flow\_designer\] role launches the Workflow Studio app, to create multi-step workflows.
-   Image administrator \[image\_admin\] role manages image files on the Images \[db\_image\] table.
-   Search application administrator \[search\_application\_admin\] role inserts data into, updates, and deletes search-UX related configuration tables including: \[sys\_search\_context\_config\], \[sys\_search\_source\], \[m2m\_search\_context\_config\_search\_source\], \[sys\_search\_facet\], and \[sys\_search\_filter\]. The AI Search administrator \[ais\_admin\] role is included in this role to perform AI Search configuration.
-   Self-service manager \[sn\_ssa\_core.self\_service\_manager\] role performs CRUD operations on Pattern and Activity configuration and the Deflection data tables.
-   Virtual Agent branding \[va\_branding\] role, fetches branding settings via scripted rest API.

**Note:** Avoid granting an admin role when more specialized roles are available.

## Users installed

The Glide Conversation Server plugin \(com.glide.cs\) automatically installs a virtual agent user \(virtual.agent\) in the User \[sys\_user\] table. Ongoing [Virtual Agent interactions](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/brazil/markdown/conversational-interfaces/virtual-agent/va-interactions.md) are assigned to this virtual agent user. If you're using Advanced Work Assignment \(AWA\), interactions assigned to the virtual agent user aren't routed by AWA. The virtual agent user is also used in the default web chat branding settings to associate a virtual agent user profile with a bot avatar.
...
Stores mapping of Virtual Agent conversations and Workspace conversations.
