...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/yokohama/conversational-interfaces/virtual-agent/test-llm-topics.html
release: yokohama
...
\[Omitted image "va-llm-test-window-assistants-2.png"\] Alt text: Add assistants in the Properties tab, followed by the Test skill discovery check box in the topic testing window, to select an assistant in the drop-down window.
...
If instructions in a node include potentially offensive content, a warning badge appears on the node. A yellow warning icon \[Omitted image "yellow-warning-icon.png"\] Alt text: appears next to the node name. Selecting **Apply and restart** inputs those instructions and removes the badge and icon, and the `Modified` status reverts to `Original`.

The following image shows the **Modify instructions** tab showing offensive content warning badge, icons, and hover message.

\[Omitted image "va-prompt-discovery-llm-3-2.png"\] Alt text: Modify instructions tab showing offensive content warnings.
...
\[Omitted image "image.va-variables-llm-2 "\] Alt text: Variables tab that shows the input variables and values, next to a screen capture of the conversation flow.
...
-   **[Testing Now Assist enhanced chat conversations](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/yokohama/markdown/yokohama/conversational-interfaces/virtual-agent/testing-enhanced-chat-conversations.md)**  
