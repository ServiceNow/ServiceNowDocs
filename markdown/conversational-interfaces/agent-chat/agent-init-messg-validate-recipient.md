...
3.  On the Extension Point form, either modify a script include to use the global.ChannelSendToValidation extension point or create and register a custom script include.
    ...
    -   Modify the existing script include by going to the Implementations related list and selecting a script include in the **Class** column.
        ...
        **Note:** The PhoneNumberValidationExtPointImpl script include that use the global.ChannelSendToValidation extension point is available for validating recipient IDs profiles associated with the [Conversational SMS service channel](https://www.servicenow.com/docs/access?context=conversation-sms-service-channel-store-app&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US) application.
