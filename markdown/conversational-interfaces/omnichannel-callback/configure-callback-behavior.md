...
7.  Provide the **Time between callback attempts \(in minutes\)**.

    When a callback request is attempted, the next callback request can be attempted only after the time that you've set between multiple attempts.
