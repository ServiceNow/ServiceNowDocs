...
Callback Actions API helps you fulfill web-based callback requests. By using Callback Actions API, you can request, reschedule, and cancel callbacks from Virtual Agent, Web Embeddable, Portal, Engagement Module \(EM\), Agent Workspace, and route the callback requests to external CCaaS providers. For more information, see [Configure callback routing](../task/configure-callback-route-ccaas-api.md).

To learn more about the Omnichannel Callback APIs, see [Omnichannel Callback API](https://www.servicenow.com/docs/access?context=omichannel-callback-api&version=zurich&pubname=zurich-api-reference&ft:locale=en-US).
