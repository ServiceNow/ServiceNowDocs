...
1.  To configure the Now Assist for Virtual Agent application that installs the Now Assist in Conversational Catalog Request application, see [Configuring Now Assist in Virtual Agent](configure-now-assist-va.md).
...
2.  Enable the generative AI experience for a catalog item request submission in Virtual Agent.
    ...
    2.  Select a catalog item that must be requested in the conversation mode.
        ...
        For information about catalog items that are supported in the conversation mode, see [Service Catalog topic blocks in Virtual Agent powered by Now LLM](https://www.servicenow.com/docs/access?context=request-topic-blocks-va-llm&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US).
    ...
    4.  On the catalog item form, confirm that the **Turn off Now Assist \(LLM\)** check box isn't selected.
        ...
        **Note:**
        ...
        -   If you clear the **Make the item non-conversational in VA** check box, the item will become conversational only if all the conditions are met for making an item conversational. For more information about such conditions, see [Service Catalog topic blocks in Virtual Agent powered by Now LLM](https://www.servicenow.com/docs/access?context=request-topic-blocks-va-llm&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US).
    5.  Select **Update**.
...
<table id="choicetable_omj_3fl_lzb"><thead><tr><th align="left" id="d141697e311">

Option

</th><th align="left" id="d141697e314">

Description

</th></tr></thead><tbody><tr><td id="d141697e320">
...
</td></tr><tr><td id="d141697e359">

**Build additional logic in a new subflow. For example, to refer to a new record producer for each business.**

</td><td>

1.  Create a subflow to return the sys\_id of a record producer. For information about Flow Designer subflows, see [Building subflows](https://www.servicenow.com/docs/access?context=subflows&version=xanadu&pubname=xanadu-build-workflows&ft:locale=en-US).
...
[Service Catalog topic blocks in Virtual Agent powered by NLU](https://www.servicenow.com/docs/access?context=request-topic-blocks-va&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US)

[Service Catalog topic blocks in Virtual Agent powered by Now LLM](https://www.servicenow.com/docs/access?context=request-topic-blocks-va-llm&version=xanadu&pubname=xanadu-servicenow-platform&ft:locale=en-US)
