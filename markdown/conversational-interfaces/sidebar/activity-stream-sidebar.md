...
When a Sidebar discussion is created, a corresponding Sidebar discussion tile is automatically added to the activity stream. Users who have access to the record can also access the discussion from the activity stream. Sidebar users can post messages from discussions into the activity stream as Sidebar posted messages. These messages can be posted to the activity stream as editable entries that can be added and removed as necessary. For generalized information about activity streams, see [Activity streams](https://www.servicenow.com/docs/access?context=c_activity-streams&version=zurich&pubname=zurich-platform-user-interface&ft:locale=en-US).
