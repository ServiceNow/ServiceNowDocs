...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/xanadu/healthcare-life-sciences/vaccine-administration-management/managing-high-number-of-concurrent-booking.html
release: xanadu
...
To avoid performance-related issues, configuration changes are required to pin to a specific node. As a user with the admin role, you can choose which thread to point to a specific node so that the load is distributed evenly across all the nodes. For example, if you are using a multi-node instance, you can change the configuration to pin to a specific node by using the system ID field to select the specific node where you want your mass booking event processor to hit. This configuration change can improve the system performance.
